...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_throat_middle_interior_streamwise_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_throat_middle_interior_streamwise_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.420063`
- Max h/u/v/hu/hv delta: `0.355242` / `0.410862` / `0.402237` / `0.420063` / `0.412687`
...
- Final-frame `hu` field remains 1.68x over threshold at `upstream_approach/interior` cell 6,9.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/interior` (max u/hu delta `0.26848`/`0.420063`).
- Cross-stream shear/momentum mismatch is still active in `recovery/lower_edge` (max v/hv delta `0.306653`/`0.412687`).

## Next Levers

- Start with `upstream_approach/interior` cell 6,9; `hu` delta is `-0.420063` with reference h `1.64011` m and C++ h `1.65212` m.
