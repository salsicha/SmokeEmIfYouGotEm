...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_lower_middle_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_lower_middle_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.397734`
- Max h/u/v/hu/hv delta: `0.355242` / `0.389623` / `0.33579` / `0.397734` / `0.386887`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `upstream_approach` | `interior` | 54 | -0.0472677 / 0.263487 | 0.0265168 / 0.254771 | -0.0267023 / 0.220581 | 0.0259524 / 0.397734 | -0.0535672 / 0.386887 | -2.55245 | 0 | 1.59094 |
...
- Final-frame `hu` field remains 1.59x over threshold at `upstream_approach/interior` cell 5,9.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/interior` (max u/hu delta `0.254771`/`0.397734`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/interior` (max v/hv delta `0.220581`/`0.386887`).

## Next Levers

- Start with `upstream_approach/interior` cell 5,9; `hu` delta is `-0.397734` with reference h `1.71983` m and C++ h `1.74325` m.
