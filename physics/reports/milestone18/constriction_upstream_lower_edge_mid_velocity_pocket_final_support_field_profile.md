...
- Max field delta: `0.537514`
- Max h/u/v/hu/hv delta: `0.466456` / `0.537514` / `0.502471` / `0.533837` / `0.52695`
...
- Final-frame `u` field remains 2.15x over threshold at `recovery/interior` cell 4,19.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.466456` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.537514`/`0.532979`).
- Cross-stream shear/momentum mismatch is still active in `downstream_constriction/interior` (max v/hv delta `0.463802`/`0.52695`).

## Next Levers

- Start with `recovery/interior` cell 4,19; `u` delta is `-0.537514` with reference h `1.20208` m and C++ h `1.57884` m.
