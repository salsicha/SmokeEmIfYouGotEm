...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_middle_upper_shelf_cross_stream_balance_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_middle_upper_shelf_cross_stream_balance_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.263487`
- Max h/u/v/hu/hv delta: `0.263487` / `0.262286` / `0.239044` / `0.250292` / `0.258701`
- Max profile mass delta: `3.37637` m3
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `upstream_approach` | `interior` | 54 | -0.0472677 / 0.263487 | 0.0225891 / 0.254233 | -0.00483912 / 0.177068 | 0.0196583 / 0.229556 | -0.0154316 / 0.254026 | -2.55245 | 0 | 1.05395 |
...
| `recovery` | `interior` | 46 | 0.0733994 / 0.261945 | -0.0541368 / 0.245967 | -0.00960214 / 0.189581 | 0.0638089 / 0.250292 | -0.00670219 / 0.226973 | 3.37637 | 0 | 1.04778 |
...
| `upstream_approach` | `upper_shelf` | 18 | -0.00913431 / 0.0899976 | -0.0284828 / 0.251359 | 0.0247186 / 0.222589 | -0.0192516 / 0.229945 | 0.0174464 / 0.20115 | -0.164418 | 0.0555556 | 1.00544 |
...
- Final-frame `h` field remains 1.05x over threshold at `upstream_approach/interior` cell 4,6.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.263487` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/upper_edge` (max u/hu delta `0.262286`/`0.237098`).
- Cross-stream shear/momentum mismatch is still active in `recovery/lower_edge` (max v/hv delta `0.189954`/`0.258701`).

## Next Levers

- Start with `upstream_approach/interior` cell 4,6; `h` delta is `-0.263487` with reference h `1.86864` m and C++ h `1.60515` m.
