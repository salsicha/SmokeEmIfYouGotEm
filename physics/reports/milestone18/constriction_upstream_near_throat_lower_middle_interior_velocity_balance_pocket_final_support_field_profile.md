...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_near_throat_lower_middle_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_near_throat_lower_middle_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.389623`
- Max h/u/v/hu/hv delta: `0.355242` / `0.389623` / `0.33579` / `0.388646` / `0.386887`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `upstream_approach` | `upper_edge` | 10 | 0.00561358 / 0.103644 | -0.111086 / 0.389623 | 0.0102379 / 0.105336 | -0.013632 / 0.237095 | -0.0270507 / 0.239362 | 0.0561358 | 0 | 1.55849 |
| `upstream_approach` | `upper_shelf` | 18 | -0.00913431 / 0.0899976 | -0.0661867 / 0.389058 | -0.0100128 / 0.300528 | -0.0311484 / 0.291333 | 0.0169122 / 0.272483 | -0.164418 | 0.0555556 | 1.55623 |
| `upstream_approach` | `interior` | 54 | -0.0472677 / 0.263487 | 0.0312371 / 0.254233 | -0.0302668 / 0.220581 | 0.0341809 / 0.388646 | -0.0597809 / 0.386887 | -2.55245 | 0 | 1.55458 |
...
- Final-frame `u` field remains 1.56x over threshold at `upstream_approach/upper_edge` cell 9,5.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/upper_edge` (max u/hu delta `0.389623`/`0.237095`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/interior` (max v/hv delta `0.220581`/`0.386887`).

## Next Levers

- Start with `upstream_approach/upper_edge` cell 9,5; `u` delta is `-0.389623` with reference h `0.417236` m and C++ h `0.4375` m.
