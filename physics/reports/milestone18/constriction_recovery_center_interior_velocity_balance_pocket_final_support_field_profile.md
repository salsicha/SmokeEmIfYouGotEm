...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_recovery_center_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_recovery_center_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.410862`
- Max h/u/v/hu/hv delta: `0.355242` / `0.410862` / `0.402237` / `0.399788` / `0.400549`
...
- Final-frame `u` field remains 1.64x over threshold at `downstream_constriction/interior` cell 5,15.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `downstream_constriction/interior` (max u/hu delta `0.410862`/`0.360888`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/lower_shelf` (max v/hv delta `0.402237`/`0.294638`).

## Next Levers

- Start with `downstream_constriction/interior` cell 5,15; `u` delta is `-0.410862` with reference h `1.28827` m and C++ h `1.3875` m.
