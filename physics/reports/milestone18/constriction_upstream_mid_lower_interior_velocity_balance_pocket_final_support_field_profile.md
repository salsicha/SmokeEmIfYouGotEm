...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_mid_lower_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_mid_lower_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.386887`
- Max h/u/v/hu/hv delta: `0.355242` / `0.353905` / `0.33579` / `0.383335` / `0.386887`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `upstream_approach` | `interior` | 54 | -0.0472677 / 0.263487 | 0.0274295 / 0.254233 | -0.0275156 / 0.220581 | 0.0271869 / 0.383335 | -0.0547273 / 0.386887 | -2.55245 | 0 | 1.54755 |
...
- Final-frame `hv` field remains 1.55x over threshold at `upstream_approach/interior` cell 3,0.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/interior` (max u/hu delta `0.254233`/`0.383335`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/interior` (max v/hv delta `0.220581`/`0.386887`).

## Next Levers

- Start with `upstream_approach/interior` cell 3,0; `hv` delta is `-0.386887` with reference h `1.76401` m and C++ h `1.69732` m.
