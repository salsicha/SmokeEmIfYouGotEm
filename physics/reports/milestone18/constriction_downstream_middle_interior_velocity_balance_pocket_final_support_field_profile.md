...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_downstream_middle_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_downstream_middle_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.324043`
- Max h/u/v/hu/hv delta: `0.309993` / `0.324043` / `0.300528` / `0.323404` / `0.322086`
...
- Final-frame `u` field remains 1.30x over threshold at `upstream_approach/lower_shelf` cell 0,2.
- Depth/profile mismatch is still active in `upstream_approach/lower_edge` (max h delta `0.309993` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/lower_shelf` (max u/hu delta `0.324043`/`0.323404`).
- Cross-stream shear/momentum mismatch is still active in `constriction_throat/upper_edge` (max v/hv delta `0.103293`/`0.322086`).

## Next Levers

- Start with `upstream_approach/lower_shelf` cell 0,2; `u` delta is `-0.324043` with reference h `0.171864` m and C++ h `0.3` m.
