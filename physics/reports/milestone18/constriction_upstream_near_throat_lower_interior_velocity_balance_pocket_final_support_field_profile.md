...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_near_throat_lower_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_near_throat_lower_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.33791`
- Max h/u/v/hu/hv delta: `0.309993` / `0.329199` / `0.33579` / `0.336402` / `0.33791`
...
- Final-frame `hv` field remains 1.35x over threshold at `recovery/interior` cell 3,21.
- Depth/profile mismatch is still active in `upstream_approach/lower_edge` (max h delta `0.309993` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.309551`/`0.336402`).
- Cross-stream shear/momentum mismatch is still active in `recovery/interior` (max v/hv delta `0.278258`/`0.33791`).

## Next Levers

- Start with `recovery/interior` cell 3,21; `hv` delta is `0.33791` with reference h `1.30213` m and C++ h `1.32873` m.
