...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_throat_interior_streamwise_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_throat_interior_streamwise_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.440334`
- Max h/u/v/hu/hv delta: `0.355242` / `0.439552` / `0.402237` / `0.436356` / `0.440334`
...
- Final-frame `hv` field remains 1.76x over threshold at `constriction_throat/lower_edge` cell 4,11.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.439552`/`0.436356`).
- Cross-stream shear/momentum mismatch is still active in `constriction_throat/lower_edge` (max v/hv delta `0.286263`/`0.440334`).

## Next Levers

- Start with `constriction_throat/lower_edge` cell 4,11; `hv` delta is `0.440334` with reference h `1.58606` m and C++ h `1.53972` m.
