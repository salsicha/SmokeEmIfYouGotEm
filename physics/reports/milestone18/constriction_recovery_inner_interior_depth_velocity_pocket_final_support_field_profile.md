...
- Max field delta: `0.532979`
- Max h/u/v/hu/hv delta: `0.466456` / `0.52177` / `0.502471` / `0.532979` / `0.52695`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `recovery` | `interior` | 46 | 0.09473 / 0.350468 | -0.0637315 / 0.52177 | 0.0290209 / 0.37979 | 0.0917078 / 0.532979 | 0.052841 / 0.508163 | 4.35758 | 0 | 2.13192 |
...
- Final-frame `hu` field remains 2.13x over threshold at `recovery/interior` cell 5,23.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.466456` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.52177`/`0.532979`).
- Cross-stream shear/momentum mismatch is still active in `downstream_constriction/interior` (max v/hv delta `0.463802`/`0.52695`).

## Next Levers

- Start with `recovery/interior` cell 5,23; `hu` delta is `-0.532979` with reference h `1.41822` m and C++ h `1.48595` m.
