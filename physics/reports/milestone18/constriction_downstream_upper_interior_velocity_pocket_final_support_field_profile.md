...
- Max field delta: `0.550955`
- Max h/u/v/hu/hv delta: `0.466456` / `0.550955` / `0.502471` / `0.544494` / `0.549948`
...
- Final-frame `u` field remains 2.20x over threshold at `upstream_approach/upper_edge` cell 9,1.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.466456` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/upper_edge` (max u/hu delta `0.550955`/`0.452691`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/interior` (max v/hv delta `0.502471`/`0.549948`).

## Next Levers

- Start with `upstream_approach/upper_edge` cell 9,1; `u` delta is `-0.550955` with reference h `0.380904` m and C++ h `0.492325` m.
