...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_recovery_upper_interior_to_lower_shelf_depth_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_recovery_upper_interior_to_lower_shelf_depth_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.339519`
- Max h/u/v/hu/hv delta: `0.309993` / `0.329199` / `0.33579` / `0.339519` / `0.33791`
...
- Final-frame `hu` field remains 1.36x over threshold at `upstream_approach/interior` cell 7,3.
- Depth/profile mismatch is still active in `upstream_approach/lower_edge` (max h delta `0.309993` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/interior` (max u/hu delta `0.254233`/`0.339519`).
- Cross-stream shear/momentum mismatch is still active in `recovery/interior` (max v/hv delta `0.278258`/`0.33791`).

## Next Levers

- Start with `upstream_approach/interior` cell 7,3; `hu` delta is `-0.339519` with reference h `1.7897` m and C++ h `1.83259` m.
