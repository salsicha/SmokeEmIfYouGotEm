...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_throat_interior_streamwise_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_throat_interior_streamwise_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.329199`
- Max h/u/v/hu/hv delta: `0.309993` / `0.329199` / `0.300528` / `0.323404` / `0.322086`
...
- Final-frame `u` field remains 1.32x over threshold at `downstream_constriction/interior` cell 6,15.
- Depth/profile mismatch is still active in `upstream_approach/lower_edge` (max h delta `0.309993` m).
- Streamwise shear/momentum mismatch is still active in `downstream_constriction/interior` (max u/hu delta `0.329199`/`0.248846`).
- Cross-stream shear/momentum mismatch is still active in `constriction_throat/upper_edge` (max v/hv delta `0.103293`/`0.322086`).

## Next Levers

- Start with `downstream_constriction/interior` cell 6,15; `u` delta is `-0.329199` with reference h `1.27281` m and C++ h `1.3875` m.
