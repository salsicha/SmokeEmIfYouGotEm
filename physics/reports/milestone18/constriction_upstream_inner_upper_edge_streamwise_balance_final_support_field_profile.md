...
- Max field delta: `0.261945`
- Max h/u/v/hu/hv delta: `0.261945` / `0.251359` / `0.239044` / `0.250292` / `0.258701`
...
| `hv` | `upstream_approach` | `lower_edge` | `2,5` | 5 | -3.5 | 1.00936 | 0.765399 | -0.243958 | 0.243958 | 0.25 | 0.975834 |

## Blocked Reasons

- Final-frame `h` field remains 1.05x over threshold at `recovery/interior` cell 8,21.
- Depth/profile mismatch is still active in `recovery/interior` (max h delta `0.261945` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/upper_shelf` (max u/hu delta `0.251359`/`0.229945`).
- Cross-stream shear/momentum mismatch is still active in `recovery/lower_edge` (max v/hv delta `0.189954`/`0.258701`).

## Next Levers

- Start with `recovery/interior` cell 8,21; `h` delta is `0.261945` with reference h `1.42473` m and C++ h `1.68667` m.
