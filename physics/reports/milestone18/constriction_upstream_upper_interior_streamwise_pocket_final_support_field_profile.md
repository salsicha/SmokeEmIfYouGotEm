...
- Max field delta: `0.52177`
- Max h/u/v/hu/hv delta: `0.466456` / `0.52177` / `0.502471` / `0.499837` / `0.508163`
...
- Final-frame `u` field remains 2.09x over threshold at `recovery/interior` cell 7,19.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.466456` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.52177`/`0.499837`).
- Cross-stream shear/momentum mismatch is still active in `recovery/interior` (max v/hv delta `0.37979`/`0.508163`).

## Next Levers

- Start with `recovery/interior` cell 7,19; `u` delta is `-0.52177` with reference h `1.3358` m and C++ h `1.57587` m.
