...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_recovery_middle_interior_to_lower_shelf_depth_balance_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_recovery_middle_interior_to_lower_shelf_depth_balance_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.265283`
- Max h/u/v/hu/hv delta: `0.263487` / `0.265283` / `0.263705` / `0.250292` / `0.258701`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `recovery` | `interior` | 46 | 0.0733994 / 0.261945 | -0.0582122 / 0.265283 | -0.00960214 / 0.189581 | 0.0578403 / 0.250292 | -0.00670219 / 0.226973 | 3.37637 | 0 | 1.06113 |
...
- Final-frame `u` field remains 1.06x over threshold at `recovery/interior` cell 5,21.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.263487` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.265283`/`0.250292`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/upper_shelf` (max v/hv delta `0.263705`/`0.20115`).

## Next Levers

- Start with `recovery/interior` cell 5,21; `u` delta is `-0.265283` with reference h `1.36231` m and C++ h `1.46453` m.
