...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_middle_interior_streamwise_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_middle_interior_streamwise_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.456117`
- Max h/u/v/hu/hv delta: `0.355242` / `0.439552` / `0.402237` / `0.456117` / `0.440334`
...
- Final-frame `hu` field remains 1.82x over threshold at `recovery/lower_edge` cell 2,18.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `recovery/lower_edge` (max u/hu delta `0.387147`/`0.456117`).
- Cross-stream shear/momentum mismatch is still active in `constriction_throat/lower_edge` (max v/hv delta `0.286263`/`0.440334`).

## Next Levers

- Start with `recovery/lower_edge` cell 2,18; `hu` delta is `0.456117` with reference h `1.1836` m and C++ h `1.15756` m.
