...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_boundary_upper_interior_cross_stream_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_boundary_upper_interior_cross_stream_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.399788`
- Max h/u/v/hu/hv delta: `0.355242` / `0.389623` / `0.33579` / `0.399788` / `0.386887`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `upstream_approach` | `interior` | 54 | -0.0472677 / 0.263487 | 0.030751 / 0.254771 | -0.02942 / 0.220581 | 0.0333405 / 0.399788 | -0.0583092 / 0.386887 | -2.55245 | 0 | 1.59915 |
...
- Final-frame `hu` field remains 1.60x over threshold at `upstream_approach/interior` cell 5,2.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/interior` (max u/hu delta `0.254771`/`0.399788`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/interior` (max v/hv delta `0.220581`/`0.386887`).

## Next Levers

- Start with `upstream_approach/interior` cell 5,2; `hu` delta is `0.399788` with reference h `1.79616` m and C++ h `1.74488` m.
