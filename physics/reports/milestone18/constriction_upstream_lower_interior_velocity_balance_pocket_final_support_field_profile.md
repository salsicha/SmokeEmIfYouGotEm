...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_upstream_lower_interior_velocity_balance_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_upstream_lower_interior_velocity_balance_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.313287`
- Max h/u/v/hu/hv delta: `0.309993` / `0.309551` / `0.300528` / `0.313287` / `0.312878`
...
- Final-frame `hu` field remains 1.25x over threshold at `recovery/interior` cell 5,19.
- Depth/profile mismatch is still active in `upstream_approach/lower_edge` (max h delta `0.309993` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.309551`/`0.313287`).
- Cross-stream shear/momentum mismatch is still active in `constriction_throat/lower_edge` (max v/hv delta `0.196049`/`0.312878`).

## Next Levers

- Start with `recovery/interior` cell 5,19; `hu` delta is `0.313287` with reference h `1.22837` m and C++ h `1.475` m.
