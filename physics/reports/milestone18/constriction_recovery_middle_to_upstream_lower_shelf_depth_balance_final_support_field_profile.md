...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_recovery_middle_to_upstream_lower_shelf_depth_balance_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_recovery_middle_to_upstream_lower_shelf_depth_balance_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.294638`
- Max h/u/v/hu/hv delta: `0.277436` / `0.291581` / `0.294316` / `0.291333` / `0.294638`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `upstream_approach` | `lower_shelf` | 12 | -0.0236296 / 0.0951501 | 0.000743586 / 0.291581 | -0.00501698 / 0.185912 | -0.00829467 / 0.23179 | -0.0161932 / 0.294638 | -0.283556 | 0 | 1.17855 |
...
- Final-frame `hv` field remains 1.18x over threshold at `upstream_approach/lower_shelf` cell 1,2.
- Depth/profile mismatch is still active in `recovery/interior` (max h delta `0.277436` m).
- Streamwise shear/momentum mismatch is still active in `upstream_approach/lower_shelf` (max u/hu delta `0.291581`/`0.23179`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/lower_shelf` (max v/hv delta `0.185912`/`0.294638`).

## Next Levers

- Start with `upstream_approach/lower_shelf` cell 1,2; `hv` delta is `0.294638` with reference h `0.299491` m and C++ h `0.389605` m.
