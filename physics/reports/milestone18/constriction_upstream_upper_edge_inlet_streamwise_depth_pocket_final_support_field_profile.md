...
- Max field delta: `0.550763`
- Max h/u/v/hu/hv delta: `0.466456` / `0.550763` / `0.502471` / `0.544494` / `0.549948`
...
- Final-frame `u` field remains 2.20x over threshold at `recovery/interior` cell 6,19.
- Depth/profile mismatch is still active in `upstream_approach/interior` (max h delta `0.466456` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.550763`/`0.532979`).
- Cross-stream shear/momentum mismatch is still active in `upstream_approach/interior` (max v/hv delta `0.502471`/`0.549948`).

## Next Levers

- Start with `recovery/interior` cell 6,19; `u` delta is `-0.550763` with reference h `1.28613` m and C++ h `1.57884` m.
