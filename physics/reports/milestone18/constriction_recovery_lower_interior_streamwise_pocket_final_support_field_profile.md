...
Dual solver manifest: `physics/outputs/m18cmp/c_constrict_recovery_lower_interior_streamwise_pocket_final_support/finite_volume_roe/dual_solver_manifest.json`
Scenario package: `physics/outputs/m18cmp/c_constrict_recovery_lower_interior_streamwise_pocket_final_support/finite_volume_roe/scenario/constriction_seed_16`
Wet-depth threshold: `0.15` m
Velocity depth floor: `0.15` m

## Summary

- Max field delta: `0.436356`
- Max h/u/v/hu/hv delta: `0.355242` / `0.410862` / `0.402237` / `0.436356` / `0.412687`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: |
| `recovery` | `interior` | 46 | 0.09473 / 0.340402 | -0.0619365 / 0.309551 | -0.0107176 / 0.278258 | 0.101699 / 0.436356 | -0.00546702 / 0.349635 | 4.35758 | 0 | 1.74542 |
...
- Final-frame `hu` field remains 1.75x over threshold at `recovery/interior` cell 8,20.
- Depth/profile mismatch is still active in `recovery/upper_edge` (max h delta `0.355242` m).
- Streamwise shear/momentum mismatch is still active in `recovery/interior` (max u/hu delta `0.309551`/`0.436356`).
- Cross-stream shear/momentum mismatch is still active in `recovery/lower_edge` (max v/hv delta `0.306653`/`0.412687`).

## Next Levers

- Start with `recovery/interior` cell 8,20; `hu` delta is `0.436356` with reference h `1.40312` m and C++ h `1.74352` m.
