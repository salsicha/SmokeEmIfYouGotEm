...
CHECKPOINT_ID = "production_detail_material_checkpoint_2026_07_09"
...
def _checkpoint(base_review: dict, flow_review: dict) -> dict:
    return {
        "status": "production_detail_materials_rendered_legacy_overlays_retired_gate_blocked_not_lifelike",
        ...
        "next_visual_art_blocker": (
            "Replace smooth preview terrain/water geometry and proxy rocks/foliage with production Landscape or "
            "Nanite geometry, biome-specific foliage, production water shading, and reviewed atmospheric/VFX layers "
            "until edge density, broad smooth area, color entropy, and Colorado luminance blockers clear without "
            "synthetic grain or overlay cards."
        ),
    }
...
def sync_photoreal_environment_review_rollups(repo_root: Path) -> list[Path]:
    ...
    asset_plan["current_decision"] = (
        ...
        "detail without legacy overlay geometry or synthetic film grain; the current base and flow reviews still "
        "record automated smoothness, edge-density, entropy, and Colorado luminance blockers. Human review and "
        "measured desktop/VR performance remain open after those visual blockers clear."
    )
    ...
    gap_register["next_checkpoint_order"][0] = (
        "Replace smooth preview terrain/water geometry and proxy rocks/foliage with production geometry, "
        "materials, ecology, and atmosphere until the current edge-density, low-gradient, entropy, and Colorado "
        "luminance blockers clear without synthetic grain or overlay cards."
    )
