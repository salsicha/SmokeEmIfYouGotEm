"""Validate a filled Futaleufu A4 DGA flow-source result payload."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Filled Futaleufu A4 DGA flow-source result JSON.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    if args.result is None:
        packet_path = write_futaleufu_a4_flow_source_review_packet(args.repo_root)
        template_path = write_futaleufu_a4_flow_source_result_template(args.repo_root)
        report_path = write_futaleufu_a4_flow_source_validation_report(args.repo_root)
    ...
    print(f"passing_flow_band_count={report['passing_flow_band_count']}")
    print(
        "can_update_flow_presets_source_class_labels="
        f"{report['promotion_permissions']['can_update_flow_presets_source_class_labels']}"
    )
    print(
