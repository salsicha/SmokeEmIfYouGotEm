"""Cook the Terminator C3 window steady flow fields for in-engine live water.

This is the authored-scenario-window counterpart to
``generate_south_fork_cooked_flow_fields`` (and mirrors
``generate_troublemaker_cooked_flow_fields``): instead of synthesizing a pilot
scenario, it runs the genuine C++ FV solver (order 2, HLL, calibrations disabled)
directly on the committed Terminator per-band scenario packages until steady state,
then exports the per-band ``h/u/v/bed/wet_mask`` grids and a
``raftsim.cooked_flow_fields.v1`` manifest that
...
from ..futaleufu_terminator_c3_window import RAPID_NAME, RIVER_ID, WINDOW_ID
...
def _terminator_bands(repo_root: Path) -> tuple[AuthoredWindowBand, ...]:
    ...
    for band_id in ("low_runnable", "median_runnable", "high_runnable"):
        ...
        bands.append(
            AuthoredWindowBand(
                ...
                discharge_target_m3s=float(entry["target_discharge_m3s"]),
            )
        )
    return tuple(bands)
...
def _window_extra_top_level(repo_root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "window_provenance": {
            "route_order_station_m": window_manifest.get("route_order_station_m"),
            "corridor_river_id": window_manifest.get("corridor_river_id"),
...
def _notes() -> list[str]:
    return [
        ...
        "Terminator C3 window: order 2, HLL, calibrations disabled, feature_strength_scale=0 (the "
        "interpreted Terminator features are authored bed geometry, not dynamic feature forcing), "
        "matching the settings the window was behaviorally validated with.",
        "The terrain source is Copernicus DEM GLO-30 (~30 m), corridor-conditioned; it cannot "
        "resolve the channel, so the whole cross-section and every in-channel feature are interpreted "
        "bed geometry. The window is pending_human_review and not production_promoted; these cooked "
        "fields inherit that provenance and make no claim of numeric parity with a reference solution.",
        ...
        "Convergence is recorded honestly per band against the pilot thresholds; the strong authored "
        "hydraulics (the Terminator hole/jump, the offset hole, the flip-wave train) keep feature-scale "
        "cells unsteady, and the thin low band does not fully convey to the window exit at big-water "
        "low technical flow, so a band that does not meet the thresholds is recorded as "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Committed cooked-fields package dir (default: scenario_terminator/cooked_flow_fields).",
    )
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("outputs/futaleufu_terminator_cooked_flow_fields"),
        help="Scratch directory for raw solver runs (gitignored).",
    )
    parser.add_argument("--steps", type=int, default=8000, help="Solver steps per band (fixed_dt is 0.05 s).")
