"""Generate the Meat Grinder reach-local C3 water-window scenario packages.
...
(order 2, HLL, calibrations disabled) at all three bands to produce the A-2
...
from raftsim.meat_grinder_c3_window import (
    MeatGrinderWindowParameters,
    SCENARIO_ROOT_RELATIVE,
    run_meat_grinder_behavioral_validation,
    write_meat_grinder_scenario_packages,
)
...
def main() -> None:
    ...
    manifest = write_meat_grinder_scenario_packages(
        repo_root, MeatGrinderWindowParameters(cell_size_m=args.cell_size_m)
    )
    ...
    work_dir = args.work_dir or (repo_root / "physics/outputs/meat_grinder_c3_window")
    report = run_meat_grinder_behavioral_validation(
        repo_root,
