"""Validate a filled Pacuare A3 rapid-stationing digitizing result."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Filled Pacuare A3 rapid-stationing digitizing result JSON.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    if args.result is None:
        packet_path = write_pacuare_a3_digitizing_action_packet(args.repo_root)
        template_path = write_pacuare_a3_digitizing_result_template(args.repo_root)
        report_path = write_pacuare_a3_digitizing_validation_report(args.repo_root)
