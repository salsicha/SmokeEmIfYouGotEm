"""Compare V22 and V23 cordilleran-cypress crown topology."""
...
def main() -> None:
    ...
    triangles_before = v22_report["local_impostor_source"]["triangle_count_lod0"]
    triangles_after = v23_report["local_impostor_source"]["triangle_count_lod0"]
    instances_before = v22_report["generated_collection"]["foliage_instance_count"]
    instances_after = v23_report["generated_collection"]["foliage_instance_count"]
    ...
    findings = {
        ...
        "v23_hlod_to_source_foreground_ratio": hlod_after / source_after,
    ...
    topology_gate_passed = (
        findings["triangle_cost_change_fraction"] <= 0.02
        and findings["foliage_instance_count_change_fraction"] <= 0.02
        and findings["closeup_foreground_change_fraction"] > -0.12
        and findings["exact_60m_source_foreground_change_fraction"] > -0.18
        and 0.95 < findings["v23_hlod_to_source_foreground_ratio"] < 1.20
        and findings["mean_turntable_open_band_fraction_change"] < -0.005
        and largest_open_band_after <= largest_open_band_before
    )

    output = {
        "schema": "raftsim.unreal.futaleufu_cypress_async_secondary_crown_review.v23",
        "river_id": "futaleufu_terminator",
        "candidate": (
            "v23_open_grown_conical_async_secondary_compound_branchlet_atlas"
        ),
        "status": (
            "async_secondary_crown_baseline_retained_visual_promotion_rejected"
            if topology_gate_passed
            else "async_secondary_crown_gate_failed_visual_promotion_rejected"
        ),
        "comparison_contract": {
            "fixed": [
                "V21 compound branchlet textures, fourteen-card authored twig, and material response",
                "V22 de-tiered main grower, golden-angle graft schedule, and attachment density",
                "post-usage shader-resource gate and corrected source-lit HLOD path",
                "open-grown form envelope and exact review cameras",
            ],
            "v23_changed": [
                "expand the graft palette from one to three branch-growth templates",
                "phase branch-template seeds by 104729 while preserving six growth cycles",
                "use distinct bounded secondary-shoot priority ramps, phyllotaxy angles, offsets, and staggers",
                "select one template uniformly per existing graft attachment through Unreal's deterministic picker",
            ],
        ...
        "human_review": {
            "accepted": [
                "The three deterministic branch-growth templates visibly break repeated shoot alignment and fill portions of the central crown in both exact turntable views.",
                "Mean low-occupancy crown rows and the largest open run both improve while exact closeup mass remains effectively unchanged.",
                "Uniform template selection lowers source triangles and foliage instances by about nine percent without a material exact-60-meter silhouette loss.",
            ],
            "rejected": [
                "The open-grown tree still reads as a sparse ladder of broad primary shelves rather than a photoreal irregular cordilleran-cypress crown.",
                "The source-lit HLOD remains darker and more saturated than the exact source even though its foreground footprint is bounded.",
                "Only one form is evaluated; no handoff, temporal, mixed-ecology, corridor, packaged desktop, target-VR, or native-art promotion gate is closed.",
            ],
            "decision": (
                "Retain V23 as the open-grown asynchronous-secondary topology baseline because "
                "the automated and human topology gates pass at lower source cost. Keep visual, "
                "corridor, all-form, and production promotion closed; next replace the remaining "
                "broad primary-shelf rhythm with irregular crown-level mass under the same cost "
                "and exact-camera guardrails, then calibrate HLOD photometry and handoffs."
            ),
        },
        "production_promoted": False,
        "corridor_substitution_allowed": False,
        "regenerate_other_forms_allowed": False,
        "v23_async_secondary_crown_baseline_retained": topology_gate_passed,
    }
