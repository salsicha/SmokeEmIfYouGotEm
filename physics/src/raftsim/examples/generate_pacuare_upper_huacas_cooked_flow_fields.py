"""Cook the Upper Huacas (Pacuare) C3 window steady flow fields for live water.
...
(order 2, HLL, calibrations disabled) directly on the committed Upper Huacas
...
from ..pacuare_upper_huacas_c3_window import RAPID_NAME, RIVER_ID, WINDOW_ID
...
def _upper_huacas_bands(repo_root: Path) -> tuple[AuthoredWindowBand, ...]:
    ...
    for band_id in BAND_IDS:
        ...
        # No gauge / cfs authority for this corridor: discharge is an interpreted
        # planning value in m3/s only.
        bands.append(
            AuthoredWindowBand(
                band_id=band_id,
                scenario_dir=repo_root / entry["package"],
                discharge_target_m3s=target_m3s,
                discharge_target_cfs=None,
            )
        )
    return tuple(bands)
...
def _window_extra_top_level(repo_root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "window_provenance": {
            "anchor_preview_station_m": window_manifest.get("anchor_preview_station_m"),
            ...
            "dem_source": "copernicus_glo30_~30m",
...
def _notes() -> list[str]:
    return [
        ...
        "Upper Huacas (Pacuare) C3 window: order 2, HLL, calibrations disabled, "
        "feature_strength_scale=0 (the Class IV features are authored bed geometry, not dynamic "
        ...
        "Data maturity: the Pacuare corridor is far less constrained than the South Fork. The DEM is "
        "30 m Copernicus GLO-30, named-rapid stationing is order-only, and the three band discharges "
        "are interpreted planning values (no gauge). The bed is interpreted geometry (GLO-30 valley "
        "frame + authored in-channel features from the catalog feature_tags), the window is "
        "pending_human_review and not production_promoted; these cooked fields inherit that "
        "provenance and make no claim of numeric parity with a reference solution.",
        "Convergence is recorded honestly per band against the pilot thresholds; the strong authored "
        "hydraulics (river-wide drop, secondary pourover, gorge lateral) can keep the near-feature "
        "cross-stream velocity unsteady, so a band that does not meet the thresholds is recorded as "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Committed cooked-fields package dir (default: scenario_upper_huacas/cooked_flow_fields).",
    )
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("outputs/upper_huacas_cooked_flow_fields"),
