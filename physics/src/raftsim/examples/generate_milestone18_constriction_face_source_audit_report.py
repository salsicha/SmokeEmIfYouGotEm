"""Generate the Milestone 18 constriction face/source audit diagnostic."""
...
from ..milestone18 import build_milestone18_constriction_face_source_audit_report
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--balance-delta-threshold-m3ps2", type=float, default=0.75)
    ...
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("reports/milestone18/constriction_face_source_audit_diagnostic.md"),
    )
    args = parser.parse_args(argv)

    report = build_milestone18_constriction_face_source_audit_report(
        ...
        balance_delta_threshold_m3ps2=args.balance_delta_threshold_m3ps2,
        top_n=args.top_n,
    )
    json_path = report.write_json(args.output_json)
    markdown_path = report.write_markdown(args.output_md)
    print(f"constriction_face_source_audit_report={json_path}")
