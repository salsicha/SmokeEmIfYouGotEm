"""Validate and merge compliant-reference D6 measured results."""
...
from raftsim.flexible_raft_d6 import (
    ...
    write_flexible_raft_d6_compliant_measured_results_merge_payload,
    write_flexible_raft_d6_compliant_measured_results_merge_report,
    write_flexible_raft_d6_compliant_measured_results_sidecar_template,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Project Chrono or reviewed-compliant measured-results sidecar JSON.",
    )
    parser.add_argument(
    ...
    if args.compliant_sidecar is None:
        template_path = (
            write_flexible_raft_d6_compliant_measured_results_sidecar_template(
                args.repo_root
            )
        )
        report_path = write_flexible_raft_d6_compliant_measured_results_merge_report(
            args.repo_root
        )
    ...
    sidecar_payload = _load_json_object(args.compliant_sidecar)
    report = build_flexible_raft_d6_compliant_measured_results_merge_report(
        sidecar_payload
    )
    report_path = args.report or args.compliant_sidecar.with_suffix(
        ".merge_report.json"
    )
    write_flexible_raft_d6_compliant_measured_results_merge_payload(report_path, report)
    ...
    merged = merge_flexible_raft_d6_compliant_measured_results_sidecar(
        sidecar_payload,
        base_measured_results_payload=base_payload,
    )
    output_path = args.output or args.compliant_sidecar.with_suffix(
