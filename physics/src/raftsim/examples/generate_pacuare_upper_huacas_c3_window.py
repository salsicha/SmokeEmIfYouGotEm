"""Generate the Upper Huacas (Pacuare) reach-local C3 water-window scenario packages.
...
def main() -> None:
    ...
    for band_id, entry in manifest["flow_band_packages"].items():
        print(
            ...
            f"Q={entry['target_discharge_m3s']:.2f} m3/s (interpreted planning) "
    ...
    print(f"raw_dem_drop_m={manifest['conditioning']['raw_dem_floor_drop_m']:.2f} "
          f"conditioned_drop_m={manifest['conditioning']['conditioned_profile_drop_m']:.2f}")

    if args.solver_executable is None:
        print("solver=skipped (no --solver-executable)")
        return

    work_dir = args.work_dir or (repo_root / "physics/outputs/upper_huacas_c3_window")
    report = run_upper_huacas_behavioral_validation(
        repo_root,
