"""Generate the Milestone 18 constriction final-frame field-profile diagnostic."""
...
from ..milestone18 import build_milestone18_constriction_field_profile_report
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("reports/milestone18/constriction_field_profile_diagnostic.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("reports/milestone18/constriction_field_profile_diagnostic.md"),
    )
    args = parser.parse_args(argv)

    report = build_milestone18_constriction_field_profile_report(
    ...
    print(f"constriction_field_profile_report={json_path}")
