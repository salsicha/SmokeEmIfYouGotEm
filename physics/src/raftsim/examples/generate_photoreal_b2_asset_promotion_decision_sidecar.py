"""Generate the B2 asset-promotion decision sidecar template and merge report."""
...
def main() -> None:
    ...
    paths = [
        write_b2_asset_promotion_decision_sidecar_template(repo_root),
        write_b2_asset_promotion_decision_sidecar_merge_report(repo_root),
    ]
    report = build_b2_asset_promotion_decision_sidecar_merge_report()
    for path in paths:
        print(f"wrote={path.relative_to(repo_root)}")
    print(f"status={report['status']}")
    print(f"sidecar_decision_count={report['sidecar_decision_count']}")
