"""Generate the Hance reach-local C3 water-window scenario packages.
...
from raftsim.colorado_hance_c3_window import (
    HanceWindowParameters,
    SCENARIO_ROOT_RELATIVE,
    run_hance_behavioral_validation,
    write_hance_scenario_packages,
)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Uncommitted solver run directory (default: physics/outputs/hance_c3_window).",
    )
    ...
    manifest = write_hance_scenario_packages(
        repo_root, HanceWindowParameters(cell_size_m=args.cell_size_m)
    )
    ...
    print(f"raw_dem_floor_drop_m={manifest['conditioning']['raw_dem_floor_drop_m']:.2f}")
    ...
    work_dir = args.work_dir or (repo_root / "physics/outputs/hance_c3_window")
    report = run_hance_behavioral_validation(
        repo_root,
    ...
    print(f"headline_gate={ {k: v for k, v in report['headline_gate'].items() if k != 'parameterization_iterations'} }")
