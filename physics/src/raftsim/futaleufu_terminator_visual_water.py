"""Build a provenance-locked Terminator capture-water derivative.

The packed field drives only non-colliding authored ribbon and foam used by
deterministic editor captures. Gameplay continues to load the cooked arrays and
uses the custom C++ shallow-water solver as the physical authority.
"""
...
SCHEMA = "raftsim.unreal.futaleufu_terminator_solver_visualization.v1"
...
SOURCE_ROOT_RELATIVE = Path(
    "physics/data/real_world/futaleufu_river_chile/"
...
MANIFEST_RELATIVE = (
    OUTPUT_ROOT_RELATIVE / "futaleufu_terminator_median_visualization_manifest.json"
)
NORMALIZATION_CAPS = {
    "depth_m": 6.0,
    "speed_mps": 10.0,
    "froude": 6.0,
    "surface_relief_m": 2.5,
}
RENDER_HEIGHT_SCALE = 0.22
...
def _load_source(
    ...
    if manifest.get("schema") != "raftsim.cooked_flow_fields.v1":
        raise ValueError("Terminator cooked-field manifest has an unexpected schema")
    if manifest.get("solver", {}).get("solver") != "raftsim_water_cpp_v1":
        raise ValueError("Terminator visualization must come from the C++ solver")
    if manifest.get("solver", {}).get("feature_strength_scale") != 0.0:
        raise ValueError("Terminator source must keep feature forcing disabled")
    ...
    if not np.any(wet) or np.any(arrays["h"][wet] <= 0.0):
        raise ValueError("Terminator cooked source must contain wet positive depths")
...
def build_futaleufu_terminator_visual_water(repo_root: Path) -> dict[str, object]:
    """Generate the committed Terminator capture derivative and manifest."""
    ...
    manifest: dict[str, object] = {
        ...
        "field_ranges": {
            ...
            "wet_speed_mps": _range(speed[wet]),
            "wet_froude": _range(froude[wet]),
        ...
        "texture": {
            ...
            "channels": (
                "R=depth_m/6 G=speed_mps/10 B=froude/6 "
                "A=detrended_surface_relief_m/5+0.5"
            ),
            "source_orientation": (
        ...
        "render_binding": {
            "map": "/Game/RaftSim/Maps/L_Terminator",
            "surface_actor": (
                "RaftSim_PhysicalCorridorRiverRibbon_futaleufu_terminator"
            ),
            "foam_actor": "RaftSim_SolverFieldFoam_futaleufu_terminator",
            "capture_only": True,
            "hidden_in_game": True,
        ...
        "authority_policy": {
            ...
            "reason": (
                "source convergence is false and Terminator stationing, geometry, "
                "flow bands, hazards, and guide lines remain review-gated"
            ),
        },
    }
