...
PACKED_TEXTURE_RELATIVE = (
    OUTPUT_ROOT_RELATIVE / "colorado_hance_moderate_depth_speed_froude_surface_v1.png"
)
MANIFEST_RELATIVE = (
    OUTPUT_ROOT_RELATIVE / "colorado_hance_moderate_visualization_manifest.json"
)
...
def _load_source(
    ...
    if manifest.get("schema") != "raftsim.cooked_flow_fields.v1":
        raise ValueError("Hance cooked-field manifest has an unexpected schema")
    if manifest.get("solver", {}).get("solver") != "raftsim_water_cpp_v1":
        raise ValueError("Hance visualization must come from the C++ water solver")
    if manifest.get("solver", {}).get("feature_strength_scale") != 0.0:
        raise ValueError(
            "Hance visualization source must keep feature forcing disabled"
        )
    ...
    if not np.any(wet) or np.any(arrays["h"][wet] <= 0.0):
        raise ValueError("Hance cooked source must contain positive-depth wet cells")
...
def build_colorado_hance_visual_water(repo_root: Path) -> dict[str, object]:
    """Generate the committed Hance capture derivative and return its manifest."""
    ...
    manifest: dict[str, object] = {
        ...
        "solver_evidence": {
            ...
            "discharge_authority": (
                "planning_placeholders_pending_release_history_review"
                if source_manifest["window_provenance"].get(
                    "flow_bands_are_planning_placeholders", False
                )
        ...
        "surface_relief_derivation": {
            ...
            "render_height_cap_cm": (
                NORMALIZATION_CAPS["surface_relief_m"] * 100.0 * RENDER_HEIGHT_SCALE
            ),
        },
        "hydraulic_visualization_evidence": {
            ...
            "camera_progress": {
                ...
                "guide_target": float(np.clip(strongest_progress - 0.01, 0.15, 0.92)),
                "river_eye": float(np.clip(strongest_progress - 0.075, 0.08, 0.82)),
                "river_eye_target": float(
                    np.clip(strongest_progress + 0.025, 0.18, 0.95)
                ),
                "solver_rapid": float(np.clip(strongest_progress - 0.055, 0.10, 0.84)),
        ...
        "render_binding": {
            "map": "/Game/RaftSim/Maps/L_Hance",
            "surface_actor": "RaftSim_PhysicalCorridorRiverRibbon_colorado_river",
            "foam_actor": "RaftSim_SolverFieldFoam_colorado_river",
            "capture_only": True,
            "hidden_in_game": True,
        ...
        "authority_policy": {
            ...
            "reason": (
                "source convergence is false and Hance geometry, release bands, "
