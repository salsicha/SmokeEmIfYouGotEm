"""Build a provenance-locked Pacuare water-visualization derivative.

The generated texture is consumed only by the non-colliding authored ribbon
used for deterministic editor captures.  The cooked arrays remain the live
solver seed and the sole source of gameplay water state and raft forces.
"""
...
SCHEMA = "raftsim.unreal.pacuare_upper_huacas_solver_visualization.v1"
GENERATED_ON = "2026-08-01"
RIVER_ID = "pacuare"
FLOW_BAND = "rainfed_runnable_planning"
...
NORMALIZATION_CAPS = {
    ...
    "speed_mps": 4.5,
    "froude": 2.5,
    "surface_relief_m": 1.0,
}
RENDER_HEIGHT_SCALE = 0.18
...
def _load_source(repo_root: Path) -> tuple[dict, dict, dict[str, np.ndarray]]:
    manifest = json.loads(
        (repo_root / SOURCE_MANIFEST_RELATIVE).read_text(encoding="utf-8")
    )
    if manifest.get("schema") != "raftsim.cooked_flow_fields.v1":
        raise ValueError("Pacuare cooked-field manifest has an unexpected schema")
    if manifest.get("solver", {}).get("solver") != "raftsim_water_cpp_v1":
        raise ValueError("Pacuare visualization must come from the C++ water solver")
    if manifest.get("solver", {}).get("feature_strength_scale") != 0.0:
        raise ValueError("Pacuare visualization source must keep feature forcing disabled")

    matches = [band for band in manifest.get("bands", []) if band.get("band_id") == FLOW_BAND]
    if len(matches) != 1:
    ...
    if not np.any(wet) or np.any(arrays["h"][wet] <= 0.0):
        raise ValueError("Pacuare cooked source must contain positive-depth wet cells")
...
def build_pacuare_upper_huacas_visual_water(
    ...
    surface = bed + depth
    column_surface = np.array(
        [np.median(surface[:, column][wet[:, column]]) for column in range(surface.shape[1])],
        dtype=np.float64,
    )
    ...
    wet_speed = speed[wet]
    eligible_foam = wet & (froude >= 0.72) & (speed >= 0.8)
    supercritical = wet & (froude >= 1.0)
    column_mean_froude = np.array(
        [np.mean(froude[:, column][wet[:, column]]) for column in range(froude.shape[1])]
    )
    ...
    manifest = {
        ...
        "solver_evidence": {
            ...
            "feature_strength_scale": source_manifest["solver"]["feature_strength_scale"],
            "converged": band["convergence"]["converged"],
            "final_convergence_window": band["convergence"]["final_window"],
            "bed_geometry_authority": source_manifest["window_provenance"][
                "bed_geometry_authority"
            ],
            "discharge_authority": source_manifest["window_provenance"][
                "discharge_authority"
            ],
            "pending_human_review": source_manifest["window_provenance"][
                "pending_human_review"
            ],
            "production_promoted": source_manifest["window_provenance"][
                "production_promoted"
            ],
        },
        "field_ranges": {
        ...
        "surface_relief_derivation": {
            ...
            "detrending": "subtract_least_squares_fit_of_per_column_wet_cell_median_surface",
        ...
        "hydraulic_visualization_evidence": {
            ...
            "foam_mask": "wet_and_froude_at_least_0.72_and_speed_at_least_0.8_mps",
            "strongest_column_index": strongest_column,
            "strongest_column_station_m": float(station_m[strongest_column]),
            "strongest_column_mean_froude": float(column_mean_froude[strongest_column]),
            "camera_progress": {
                "guide": 0.405,
                "guide_target": 0.490,
                "river_eye": 0.445,
                "river_eye_target": 0.505,
                "solver_rapid": 0.430,
                "solver_rapid_target": 0.515,
            },
        },
        "texture": {
            ...
            "channels": (
                "R=depth_m/3.5 G=speed_mps/4.5 B=froude/2.5 "
                "A=detrended_surface_relief_m/2+0.5"
            ),
            "source_orientation": (
        ...
        "render_binding": {
            "map": "/Game/RaftSim/Maps/L_UpperHuacas",
            "surface_actor": "RaftSim_PhysicalCorridorRiverRibbon_pacuare",
            "foam_actor": "RaftSim_SolverFieldFoam_pacuare",
            "capture_only": True,
            "hidden_in_game": True,
            ...
            "foam_persistence_policy": (
                "bounded_decay_from_current_and_two_upstream_cooked_field_samples"
            ),
        },
        "authority_policy": {
            ...
            "reason": (
                "source convergence is false and Upper Huacas geometry, discharge, "
                "stationing, and guide lines remain review-gated"
            ),
        },
    }
    manifest_path = (
