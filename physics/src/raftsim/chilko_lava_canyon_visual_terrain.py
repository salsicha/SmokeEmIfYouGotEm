"""Build a reach-local Lava Canyon Landscape from official terrain and C3 bed data.

The NRCan MRDEM-30 corridor remains the authority outside the rapid.  The full
interpreted 2 m C3 solver strip replaces the DEM only inside its committed
600 x 80 m window.  A smooth bank-edge correction and deterministic sub-30 m
microrelief are visual/collision infill, never survey or solver authority.
"""
...
SCHEMA = "raftsim.chilko.lava_canyon_visual_terrain.v1"
DEFAULT_OUTPUT_RELATIVE = Path(
    "physics/data/real_world/chilko_river_lava_canyon/terrain/lava_canyon_visual"
)
PRODUCTION_ROOT_RELATIVE = Path(
    "physics/data/real_world/chilko_river_bc/production_corridor/"
    "chilko_river_lodge_to_taseko_junction"
)
...
def build_chilko_lava_canyon_visual_terrain(
    repo_root: Path,
    *,
    output_dir: Path | None = None,
    output_size_px: int = 1009,
) -> dict[str, object]:
    """Build the deterministic Lava Canyon Landscape and alignment manifests."""

    if output_size_px < 127:
        raise ValueError("Lava Canyon visual terrain requires at least 127 samples")
    ...
    if bed.shape != (ny, nx) or depth.shape != bed.shape or wet.shape != bed.shape:
        raise ValueError("Lava Canyon cooked arrays do not match their manifest grid")
    ...
    # Follow the official Freshwater Atlas centerline through the raster.  Its
    # local X points east and Y points south, so (tangent_y, -tangent_x) points
    # river left in the same raster coordinate system.
    ...
    terrain_record = source_manifest["terrain"]
    source_min_m = float(terrain_record["conditioned_minimum_elevation_m"])
    source_relief_m = float(terrain_record["relief_m"])
    ...
    official_dem = _sample_image_bilinear(source_elevation, source_px_x, source_px_y)
    ...
    left_delta = left_edge - official_dem[left_row]
    right_delta = right_edge - official_dem[right_row]
    edge_delta = np.where(yy >= 0.0, left_delta[None, :], right_delta[None, :])
    bank_distance = np.maximum(np.abs(yy) - source_half_width_m, 0.0)
    alignment_weight = 1.0 - _smoothstep(0.0, 120.0, bank_distance)
    aligned_dem = official_dem + edge_delta * alignment_weight

    # Restore plausible sub-30 m surface breakup without modifying the DEM's
    # broad canyon form.  The envelope is zero at the solver boundary and the
    # amplitude is explicitly bounded below 1.4 m.
    micro_fade = _smoothstep(0.0, 18.0, bank_distance)
    microrelief = micro_fade * (
        0.54 * np.sin(xx / 13.7 + yy / 8.9 + 0.35 * np.sin(xx / 51.0))
        + 0.31 * np.sin(xx / 6.1 - yy / 11.3 + 1.2)
        + 0.18 * np.sin(xx / 3.7 + yy / 5.4 + 2.1)
    )
    microrelief = np.clip(microrelief, -1.35, 1.35)
    ...
    if terrain_span_m <= 0.0:
        raise ValueError("Lava Canyon conditioned terrain has no elevation range")
    heightfield_u16 = np.round(
        np.clip((conditioned - terrain_min_m) / terrain_span_m, 0.0, 1.0) * 65535.0
    ).astype(np.uint16)

    heightfield_path = (
        output_root / f"lava_canyon_conditioned_heightfield_{output_size_px}.png"
    )
    centerline_path = output_root / "lava_canyon_local_centerline.json"
    coordinate_map_path = output_root / "lava_canyon_runtime_coordinate_map.json"
    manifest_path = output_root / "lava_canyon_visual_terrain_manifest.json"
    ...
    centerline = {
        ...
        "section_id": "lava_canyon_station_9030m_c3_window",
        "local_metric_policy": (
            "channel-following C3 coordinates; Unreal X is downstream station and "
            "Unreal Y shifts solver lateral 0 to the center of a 600 m visual canyon"
        ),
    ...
    source_change = conditioned - official_dem
    manifest: dict[str, object] = {
        "schema": SCHEMA,
        "status": (
            "generated_review_gated_reach_local_official_dem_plus_interpreted_c3_"
            "solver_strip_not_survey_or_production_terrain_authority"
        ),
        "river_id": "chilko_river_lava_canyon",
        "section_id": "lava_canyon_station_9030m_c3_window",
        ...
        "inputs": {
            ...
            "official_conditioned_heightfield": _relative(
                source_heightfield_path, repo_root
            ),
            "official_conditioned_heightfield_sha256": _sha256(
                source_heightfield_path
            ),
        ...
        "source_registration": {
            ...
            "official_hydrography": "BC Freshwater Atlas centerline",
            "official_terrain": "NRCan CanElevation MRDEM-30 DTM CGVD2013",
            "sampling_policy": "bilinear_frenet_cross_section_along_official_route",
        },
        "procedural_infill": {
            "algorithm": "official_dem_edge_alignment_and_sub_30m_microrelief_v1",
            ...
            "maximum_official_dem_edge_correction_m": float(
                max(np.max(np.abs(left_delta)), np.max(np.abs(right_delta)))
            ),
            "edge_correction_feather_distance_m": 120.0,
            "maximum_procedural_microrelief_m": float(
                np.max(np.abs(microrelief))
            ),
            "maximum_change_from_official_dem_outside_solver_strip_m": float(
                np.max(np.abs(source_change[~protected_mask]))
            ),
            "authority": (
                "visual and Landscape collision infill only; official DEM broad form, "
                "complete C3 bed, cooked water state, and raft forces remain unchanged"
            ),
        },
        "alignment": {
        ...
        "honesty": {
            ...
            "source_terrain_resolution_m": float(terrain_record["source_resolution_m"]),
            ...
            "missing": [
                "surveyed Lava Canyon bathymetry, bank breaklines, and rapid rocks",
                "reviewed rapid station and guide lines",
                "discharge calibration and solver convergence",
