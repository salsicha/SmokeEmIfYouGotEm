"""Generate review-only Futaleufu A4 rapid digitizing work windows."""
...
from .futaleufu_a4_digitizing import (
...
FUTALEUFU_A4_DIGITIZING_WORK_WINDOWS_RELATIVE_PATH = (
    ...
    "a4_stationing_digitizing_work_windows.geojson"
)
FUTALEUFU_A4_DIGITIZING_WORK_WINDOW_MANIFEST_RELATIVE_PATH = (
    "physics/data/real_world/futaleufu_river_chile/review/"
    "a4_stationing_digitizing_work_window_manifest.json"
)
FUTALEUFU_A4_DIGITIZING_WORK_WINDOWS_SCHEMA = (
    "raftsim.futaleufu.a4_stationing_digitizing_work_windows.geojson.v1"
)
FUTALEUFU_A4_DIGITIZING_WORK_WINDOW_MANIFEST_SCHEMA = (
    "raftsim.futaleufu.a4_stationing_digitizing_work_window_manifest.v1"
)

_WINDOW_BY_PRIORITY = {
    "critical": {"half_length_m": 800.0, "lateral_review_buffer_m": 180.0},
    "high": {"half_length_m": 600.0, "lateral_review_buffer_m": 140.0},
    "medium": {"half_length_m": 450.0, "lateral_review_buffer_m": 100.0},
}


def build_futaleufu_a4_digitizing_work_windows_geojson(
    repo_root: Path,
) -> dict[str, Any]:
    """Build review-only Futaleufu rapid work windows on the current route trace."""
    ...
    features = [
        _work_window_feature(record, samples, route_length_m)
        for record in rapid_records
    ]
    return {
        "type": "FeatureCollection",
        "schema": FUTALEUFU_A4_DIGITIZING_WORK_WINDOWS_SCHEMA,
        ...
        "source_stationing": FUTALEUFU_ROUTE_STATIONING_RELATIVE_PATH,
        "review_limits": [
            "Window center stations are distributed by catalog rapid order over the current route trace.",
            "The current route trace is OSM-derived technical corridor evidence, not surveyed guide-approved stationing.",
            "The route length still conflicts with the approximate catalog run length.",
...
def write_futaleufu_a4_digitizing_work_windows_geojson(repo_root: Path) -> Path:
    payload = build_futaleufu_a4_digitizing_work_windows_geojson(repo_root)
    path = repo_root / FUTALEUFU_A4_DIGITIZING_WORK_WINDOWS_RELATIVE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path


def build_futaleufu_a4_digitizing_work_window_manifest(repo_root: Path) -> dict[str, Any]:
    """Summarize the Futaleufu A4 digitizing work windows and promotion gates."""

    status = build_futaleufu_a4_stationing_flow_status(repo_root)
    geojson = build_futaleufu_a4_digitizing_work_windows_geojson(repo_root)
    ...
    return {
        "schema": FUTALEUFU_A4_DIGITIZING_WORK_WINDOW_MANIFEST_SCHEMA,
        "generated_on": "2026-07-16",
        "task_id": "A4",
        "river_id": FUTALEUFU_RIVER_ID,
        "status": "work_windows_ready_for_manual_digitizing_review_no_stationing_promotion",
        "production_promoted": False,
        "work_windows_geojson": FUTALEUFU_A4_DIGITIZING_WORK_WINDOWS_RELATIVE_PATH,
        "source_action_packet": FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_RELATIVE_PATH,
        "source_status_artifact": FUTALEUFU_A4_STATUS_RELATIVE_PATH,
        "source_stationing": FUTALEUFU_ROUTE_STATIONING_RELATIVE_PATH,
        ...
        "route_stationing_length_m": scaffold["route_stationing_length_m"],
        "catalog_run_length_m": scaffold["catalog_run_length_m"],
        "route_to_catalog_length_ratio": scaffold["route_to_catalog_length_ratio"],
        "window_policy": {
            ...
            "not_authoritative_reason": (
                "The current route trace is OSM-derived technical corridor evidence, "
                "not a surveyed guide-approved station axis, and rapid stations remain "
                "order-only until reviewed source geometry exists."
            ),
        },
        "windows": windows,
        "promotion_gate": {
            ...
            "complete_when": [
                "reviewed route station axis replaces or approves the current OSM scaffold",
                "accepted aerial or orthomosaic source is selected",
                "each rapid is digitized as exact point or span geometry",
                "guide/outfitter, geospatial, rights/publication, and DGA/guide flow-context reviews approve",
                "the A4 digitizing result validator passes on reviewed records",
            ],
        },
    }


def write_futaleufu_a4_digitizing_work_window_manifest(repo_root: Path) -> Path:
    payload = build_futaleufu_a4_digitizing_work_window_manifest(repo_root)
    path = repo_root / FUTALEUFU_A4_DIGITIZING_WORK_WINDOW_MANIFEST_RELATIVE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path
...
def _work_window_feature(
    ...
    route_station_m = float(record["route_order_station_m_for_review_focus"])
    ...
    return {
        ...
        "properties": {
            "window_id": f"futaleufu_a4_window_{int(record['order']):02d}_{_slug(record['name'])}",
            "river_id": FUTALEUFU_RIVER_ID,
            ...
            "catalog_order_station_m": record[
            ...
            "promotion_gate": (
                ...
                "station axis, GPS/aerial rapid point/span geometry, guide confirmation, "
                "rights review, and DGA/guide flow-context review before catalog, editor, "
                "water-window, or solver binding."
            ),
        },
    }
