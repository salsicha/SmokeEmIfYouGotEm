"""Build Futaleufu A4 rapid-stationing digitizing packets and validators."""
...
FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_RELATIVE_PATH = (
    ...
    "a4_stationing_digitizing_action_packet.json"
)
FUTALEUFU_A4_DIGITIZING_RESULT_TEMPLATE_RELATIVE_PATH = (
    "physics/data/real_world/futaleufu_river_chile/review/"
    "a4_stationing_digitizing_result_template.json"
)
FUTALEUFU_A4_DIGITIZING_VALIDATION_REPORT_RELATIVE_PATH = (
    "physics/data/real_world/futaleufu_river_chile/review/"
    "a4_stationing_digitizing_validation_report.json"
)
FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_SCHEMA = (
    "raftsim.futaleufu.a4_stationing_digitizing_action_packet.v1"
)
FUTALEUFU_A4_DIGITIZING_RESULT_TEMPLATE_SCHEMA = (
    "raftsim.futaleufu.a4_stationing_digitizing_result_template.v1"
)
FUTALEUFU_A4_DIGITIZING_VALIDATION_REPORT_SCHEMA = (
    "raftsim.futaleufu.a4_stationing_digitizing_validation_report.v1"
)
...
_ALLOWED_FLOW_CONTEXT_CLASSES = (
    "dga_downstream_station_translation_pending",
    "dga_time_series_reviewed",
    "guide_reviewed_flow_band",
    "snowmelt_rainfall_context",
    "hypothesis_pending_guide_review",
)


def build_futaleufu_a4_digitizing_action_packet(repo_root: Path) -> dict[str, Any]:
    """Build the fail-closed Futaleufu A4 digitizing action packet."""
    ...
    return {
        "schema": FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_SCHEMA,
        ...
        "source_status_artifact": FUTALEUFU_A4_STATUS_RELATIVE_PATH,
        ...
        "current_blockers": {
            ...
            "catalog_run_length_m": status["catalog_stationing_scaffold"][
                "catalog_run_length_m"
            ],
            "route_stationing_length_m": status["catalog_stationing_scaffold"][
                "route_stationing_length_m"
            ],
            "route_to_catalog_length_ratio": status["catalog_stationing_scaffold"][
                "route_to_catalog_length_ratio"
            ],
            "corridor_status": corridor["corridor_status"],
            "geometry_authority": corridor["geometry_authority"],
            "flow_station_code": flow["station_code"],
            "numeric_flow_bands_promoted": flow["numeric_flow_bands_promoted"],
        },
        "source_inputs": {
            "status_artifact": FUTALEUFU_A4_STATUS_RELATIVE_PATH,
            "route_stationing": corridor["route_stationing"],
            "seasonal_flow_context": flow["seasonal_flow_context"],
        },
        "digitizing_actions": [_digitizing_action(record) for record in rapid_records],
        "required_workflow": [
            {
                "step_id": "select_reviewed_route_station_axis",
                "required_before": "stationing_digitizing",
                "complete_when": (
                    "OSM route trace is replaced or approved by guide/geospatial review "
                    "for rapid-stationing use"
                ),
            },
            {
                ...
                "complete_when": (
                    "all five rapids have Point or LineString WGS84 geometry, route "
                    "station, confidence, source evidence, and guide review"
                ),
            },
            {
                "step_id": "attach_flow_context_to_stationing_review",
                "required_before": "rapid_water_window_generation",
                "complete_when": (
                    "DGA station translation, guide-reviewed low/normal/high/unsafe "
                    "context, and hydrology reviewer signoff are recorded"
                ),
            },
            {
                "step_id": "regenerate_catalog_editor_and_water_window_inputs",
                "required_before": "a4_promotion",
                "complete_when": (
                    "named rapid catalog, editor geometry, flow context labels, and "
                    "water-window inputs are regenerated from the reviewed result"
                ),
            },
        ],
        "promotion_gate": {
...
def write_futaleufu_a4_digitizing_action_packet(repo_root: Path) -> Path:
    payload = build_futaleufu_a4_digitizing_action_packet(repo_root)
    path = repo_root / FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_RELATIVE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path


def build_futaleufu_a4_digitizing_result_template(repo_root: Path) -> dict[str, Any]:
    """Build an empty result template for Futaleufu A4 stationing."""

    packet = build_futaleufu_a4_digitizing_action_packet(repo_root)
    return {
        "schema": FUTALEUFU_A4_DIGITIZING_RESULT_TEMPLATE_SCHEMA,
        "generated_on": "2026-07-16",
        "task_id": "A4",
        ...
        "production_promoted": False,
        "source_action_packet": FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_RELATIVE_PATH,
        "source_status_artifact": FUTALEUFU_A4_STATUS_RELATIVE_PATH,
...
def write_futaleufu_a4_digitizing_result_template(repo_root: Path) -> Path:
    payload = build_futaleufu_a4_digitizing_result_template(repo_root)
    path = repo_root / FUTALEUFU_A4_DIGITIZING_RESULT_TEMPLATE_RELATIVE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path


def build_futaleufu_a4_digitizing_validation_report(
    repo_root: Path,
    result_payload: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Validate a Futaleufu digitizing result without promoting geometry."""

    packet = build_futaleufu_a4_digitizing_action_packet(repo_root)
    payload = (
        result_payload
        if result_payload is not None
        else build_futaleufu_a4_digitizing_result_template(repo_root)
    )
    ...
    return {
        "schema": FUTALEUFU_A4_DIGITIZING_VALIDATION_REPORT_SCHEMA,
        "generated_on": "2026-07-16",
        "task_id": "A4",
        "river_id": packet["river_id"],
        "display_name": packet["display_name"],
        ...
        "production_promoted": False,
        "source_action_packet": FUTALEUFU_A4_DIGITIZING_ACTION_PACKET_RELATIVE_PATH,
        "source_result_template": FUTALEUFU_A4_DIGITIZING_RESULT_TEMPLATE_RELATIVE_PATH,
        "source_status_artifact": FUTALEUFU_A4_STATUS_RELATIVE_PATH,
        ...
        "promotion_gate": {
            "can_promote_current_osm_route": False,
            ...
            "next_after_valid_result": [
                ...
                "generate_futaleufu_rapid_water_window_inputs",
...
def write_futaleufu_a4_digitizing_validation_report(repo_root: Path) -> Path:
    payload = build_futaleufu_a4_digitizing_validation_report(repo_root)
    path = repo_root / FUTALEUFU_A4_DIGITIZING_VALIDATION_REPORT_RELATIVE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path
...
def _digitizing_action(record: dict[str, Any]) -> dict[str, Any]:
    return {
        "action_id": f"futaleufu_a4_digitize_{int(record['order']):02d}_{_slug(record['name'])}",
        ...
        "catalog_station_m_from_order_interpolation": record[
            "catalog_station_m_from_order_interpolation"
        ],
        "route_order_station_m_for_review_focus": record[
            "route_order_station_m_for_review_focus"
        ],
        ...
        "required_source_classes": [
            "reviewed_route_station_axis",
            "gps_or_aerial_rapid_geometry",
            "guide_or_outfitter_confirmation",
            "rights_and_publication_review",
            "dga_or_guide_flow_context",
        ],
...
def _record_errors(record: dict[str, Any]) -> list[dict[str, str]]:
    ...
    elif not _geometry_coordinates_valid(record["geometry_type"], record["geometry_coordinates_wgs84"]):
