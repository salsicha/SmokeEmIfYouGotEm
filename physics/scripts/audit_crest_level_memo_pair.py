...
def summarize(report):
    ...
    for i, row in enumerate(rows):
        if row['pair'] != i or type(row['level_first']) is not bool or row['level_first'] != bool(i % 2):
        ...
        for key in ('shared_retained_bytes', 'level_retained_bytes'):
            value = row[key]
            if type(value) not in (int, float) or not math.isfinite(value) or value != int(value) or value < 0:
                raise ValueError('Finite nonnegative integer retained allocation required')
    if rows[-1]['frame'] == rows[0]['frame']:
        raise ValueError('Actual changing-input history, not one repeated frame, required')
    groups = {}
    for name, selected in [('all', rows), ('shared_first', rows[::2]), ('level_first', rows[1::2])]:
        groups[name] = dict(pairs=len(selected), shared_mean_ms=statistics.mean(r['shared_ms'] for r in selected),
            level_mean_ms=statistics.mean(r['level_ms'] for r in selected),
            level_faster_pairs=sum(r['level_ms'] < r['shared_ms'] for r in selected))
    return dict(exact_topology_and_coordinates=report['exact'] and all(r['exact'] for r in rows),
        measured_both_orders_faster=all(g['level_mean_ms'] < g['shared_mean_ms'] for g in groups.values()),
        compared_vertices=sum(r['vertices'] for r in rows), compared_triangles=sum(r['triangles'] for r in rows),
        shared_peak_retained_bytes=max(r['shared_retained_bytes'] for r in rows),
        level_peak_retained_bytes=max(r['level_retained_bytes'] for r in rows),
        first_frame=rows[0]['frame'], last_frame=rows[-1]['frame'], groups=groups,
        scope='Whole adaptive-build level-local memo CPU comparison on actual changed inputs; not ordinary frame performance, visual or physics acceptance.',
