...
def parse_capture(stream, require_water_scopes=False):
    ...
    columns = {name: header.index(name) for name in METRICS + WATER_SCOPES + PUBLISH_SCOPES + SMOOTHING_SCOPES if name in header}
    samples, metadata = [], {}
    footer = False
    ...
    for row in reader:
        ...
        if row[0] == "[HasHeaderRowAtEnd]":
            ...
            footer = True
            continue
        if footer or len(row) != len(header):
...
def summarize(samples, first, last):
    ...
    result = {}
    for name in METRICS + WATER_SCOPES + PUBLISH_SCOPES + SMOOTHING_SCOPES:
    ...
    result["frame_p95_within_60fps_budget"] = result["FrameTime"]["p95_ms_nearest_rank"] <= 1000. / 60.
    return result
...
def main():
    ...
    for path in args.captures:
        ...
        runs.append({"csv": str(path.resolve()), "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
                     ...
                     "metrics": summarize(samples, args.first_sample, args.last_sample)})
    report = {"schema": "raftsim.unreal_frame_csv_audit.v1", "release_accepted": False,
              ...
              "Refresh contains source samples/handover/optical filter; CartesianPublish contains packing/SetMesh; "
              "SetMesh contains topology/crest input/crest update, which contains selection). "
