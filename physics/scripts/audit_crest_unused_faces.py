"""Check 64 exact actual-input normal-pass pairs in both call orders, not FPS."""
import argparse
import hashlib
import json
import math
...
PATTERN = re.compile(
    r'CrestUnusedFaceAudit frame=(\d+) exact=([01]) candidate_first=([01]) '
    r'vertices=(\d+) triangles=(\d+) reference_ms=([\d.eE+-]+) candidate_ms=([\d.eE+-]+)'
)
...
def summarize(text):
    ...
    for line in text.splitlines():
        if 'CrestUnusedFaceAudit frame=' not in line:
            continue
        match = PATTERN.search(line)
        if not match:
            raise ValueError('Malformed native normal audit row')
        frame, exact, first, vertices, triangles = map(int, match.groups()[:5])
        reference, candidate = map(float, match.groups()[5:])
        if vertices <= 0 or triangles <= 0:
            raise ValueError('Positive original mesh counts required')
        ...
        rows.append(dict(frame=frame, exact=bool(exact), candidate_first=bool(first),
                         vertices=vertices, triangles=triangles, reference_ms=reference, candidate_ms=candidate))
    ...
    return dict(exact=all(r['exact'] for r in rows),
                ...
                release_accepted=False, scope='Normal-pass CPU work only; not FPS, physics or visual acceptance.')
