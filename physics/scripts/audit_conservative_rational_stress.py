...
def run(seed,n,metric_source='dense',flux_scheme='donor-stress'):
    ...
    if metric_source=='primal':
        r=stress_stage_physical(g,source[...,1:],flux_scheme=flux_scheme)
    elif metric_source=='dense':
        k,_=metric(g,rational=True);root=np.sqrt(h)[...,None]
        v=(k@(root*u).ravel()).reshape(u.shape)/root;r=stress_stage(g,v,flux_scheme=flux_scheme)
    else:raise ValueError('Unknown physical state preparation')
    momentum=np.sum(r['physical_momentum_rate'],axis=(0,1))*dx**2
    return dict(seed=seed,resolution=n,flux_scheme=flux_scheme,source_state_sha256=hashlib.sha256(source.tobytes()).hexdigest(),
        ...
        mass_scheme_incremental_energy_rate=r['mass_scheme_incremental_energy_rate'],
        incremental_work_is_donor=r['incremental_work_is_donor'],
...
def main():
    ...
    for n in (64,128):
        for seed in (2200,2202,2204,2206):
            r=run(seed,n,args.metric_source,args.flux_scheme);records.append(r);print(json.dumps(r),flush=True)
    if hashes()!=original:raise RuntimeError('Implementation changed during audit')
    with args.report.open('x') as stream:
        json.dump(dict(scope=__doc__,metric_source=args.metric_source,flux_scheme=args.flux_scheme,records=records,
