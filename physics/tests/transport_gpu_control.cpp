...
// Not a pressure solve, RK step, engine integration, or scene-acceptance test.
...
int main(int argc,char** argv)
{
    try
    {
        ...
        if(argc!=5 || std::string(argv[1])!="run" || (std::string(argv[4])!="hardware" && std::string(argv[4])!="warp"))return 2;
        ...
        ComPtr<ID3D11ComputeShader> shaders[6];ComPtr<ID3D11ShaderReflection> reflection[6];
        for(U phase=0;phase<6;++phase)
        {
            auto code=read(fs::path(argv[2])/(std::to_string(phase)+".cso"));
            check(device->CreateComputeShader(code.data(),code.size(),nullptr,&shaders[phase]));
            check(D3DReflect(code.data(),code.size(),IID_ID3D11ShaderReflection,&reflection[phase]));
        }
        bool passed=true;
        for(U index=0;index<cases;++index)
        {
            ...
            auto pressure=array<float>(fixture,2*n),fraction=array<float>(fixture,n); // Consume; never claim pressure qualification.
            ...
            for(U phase=0;phase<6;++phase)
            {
                ...
                D3D11_SHADER_DESC sd{};check(reflection[phase]->GetDesc(&sd));
                for(U binding=0;binding<sd.BoundResources;++binding)
                {
                    D3D11_SHADER_INPUT_BIND_DESC d{};check(reflection[phase]->GetResourceBindingDesc(binding,&d));
                    if(d.Type==D3D_SIT_CBUFFER)
                    {
                        auto* cb=reflection[phase]->GetConstantBufferByName(d.Name);D3D11_SHADER_BUFFER_DESC bd{};check(cb->GetDesc(&bd));
                ...
                context->CSSetShader(shaders[phase].Get(),nullptr,0);context->CSSetConstantBuffers(0,14,constants);
                context->CSSetShaderResources(0,128,srvs);context->CSSetUnorderedAccessViews(0,8,uavs,nullptr);
                context->Dispatch(phase==5?1:groups,1,1);
        ...
        std::cout<<"backend="<<argv[4]<<" cases="<<cases<<" transport_pass="<<passed<<" pressure_or_step_accepted=0\n";
        return passed?0:1;
    }
    catch(const std::exception& e){std::cerr<<e.what()<<'\n';return 2;}
}
