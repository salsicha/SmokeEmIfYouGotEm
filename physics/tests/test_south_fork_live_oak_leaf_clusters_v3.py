...
def test_dense_live_oak_leaf_clusters_v3_are_deterministic_and_isolated():
    first = generate_south_fork_live_oak_leaf_clusters_v3(REPO_ROOT)
    first_hashes = {
        name: record["sha256"] for name, record in first["maps"].items()
    }
    second = generate_south_fork_live_oak_leaf_clusters_v3(REPO_ROOT)
