"""Behavioral validation of the committed Hance C3 cooked flow fields package.

This is the Colorado / Hance counterpart to ``test_troublemaker_cooked_flow_fields.py``.
It checks the same ``raftsim.cooked_flow_fields.v1`` schema the UE loader reads,
plus the additions the authored windows require: the manifest records the Manning n
each band was solved with, and convergence is recorded honestly (the converged flag
is only asserted to be self-consistent with the recorded windows, never forced true).
"""
...
from raftsim.colorado_hance_c3_window import FLOW_BAND_IDS, WINDOW_ID
...
def manifest() -> dict:
    if not (PACKAGE_DIR / "manifest.json").exists():
        pytest.skip("Hance cooked flow fields package is not committed yet.")
...
def test_manifest_declares_v1_schema_and_all_three_bands(manifest):
    ...
    assert manifest["river_id"] == "colorado_river_grand_canyon_rowing"
    assert manifest["section_id"] == WINDOW_ID
    assert manifest["window_id"] == WINDOW_ID
    assert manifest["rapid_name"] == "Hance"
    provenance = manifest["window_provenance"]
    assert provenance["bed_geometry_authority"] == "interpreted_bed_geometry"
    assert provenance["hance_outside_committed_corridor"] is True
    assert provenance["imposed_longitudinal_gradient"] is True
    assert provenance["flow_bands_are_planning_placeholders"] is True
...
def test_manifest_grid_matches_committed_scenario_packages(manifest):
    scenario = json.loads((SCENARIO_ROOT / FLOW_BAND_IDS[1] / "scenario.json").read_text(encoding="utf-8"))
    grid = manifest["grid"]
...
def test_package_validates_clean_and_detects_tampering(manifest, tmp_path):
    ...
    victim = tampered / FLOW_BAND_IDS[1] / "u.npy"
    array = np.load(victim)
    array[0, 0] += 1.0
    np.save(victim, array)
    problems = validate_cooked_flow_fields_package(tampered)
    assert any(f"{FLOW_BAND_IDS[1]}/u" in problem and "sha256" in problem for problem in problems)
