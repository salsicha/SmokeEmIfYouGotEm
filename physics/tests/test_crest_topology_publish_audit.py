...
from audit_crest_topology_publish import summarize


def capture():
    return '\n'.join(f'CrestTopologyPublishAudit frame={i} exact=1 candidate_first={i%2} indices=300000 cells=50001 reference_ms=0.5 candidate_ms=0.2' for i in range(120,184))


def test_complete_exact_both_orders():
    report = summarize(capture())
    assert report['exact'] and report['measured_both_orders_faster']
    assert report['compared_indices'] == 64*300000
...
@pytest.mark.parametrize('change', [
    ...
    lambda s: s.replace('indices=300000', 'indices=300001', 1),
    lambda s: s.replace('cells=50001', 'cells=0', 1),
