...
from raftsim.real_world import (
    ...
    PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_16PHR_IMAGE_FILE,
    PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_17PKM_IMAGE_FILE,
    PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_PROBE_REVIEW_FILE,
...
def test_pacuare_sentinel_cleaner_scene_search_review_prioritizes_next_scl_probe():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    ...
    review = json.loads((pacuare_dir / PACUARE_SENTINEL_CLEANER_SCENE_SEARCH_REVIEW_FILE).read_text())
    rivers = {river["river_id"]: river for river in readiness["rivers"]}
    pacuare_sources = next(river for river in photoreal_sources["rivers"] if river["river_id"] == "pacuare")

    assert PACUARE_SENTINEL_CLEANER_SCENE_SEARCH_REVIEW_FILE in source_manifest["artifacts"]["imagery"]
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_cleaner_scene_search_review"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/" + PACUARE_SENTINEL_CLEANER_SCENE_SEARCH_REVIEW_FILE
        in rivers["pacuare"]["attached_sources_by_class"]["aerial_or_satellite_imagery"]["artifacts"]
    )
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_cleaner_scene_search_review"
    ...
    assert review["schema"] == "raftsim.pacuare_sentinel_cleaner_scene_search_review.v1"
    assert review["status"] == "cleaner_sentinel_candidates_identified_scl_probe_pending"
    assert review["query_result"]["feature_count"] == 19
    assert review["query_result"]["raw_response_committed"] is False
    assert review["recommended_next_probe"]["date"] == "2025-03-20"
    assert review["recommended_next_probe"]["tile_ids"] == ["16PHR", "17PKM"]
    assert review["recommended_next_probe"]["decision"] == "download_scl_first_then_tci_only_if_qa_passes"
    assert review["same_date_pair_candidates"][0]["date"] == "2025-03-20"
    assert review["same_date_pair_candidates"][0]["max_catalog_cloud_cover_percent"] == 11.723126
    assert review["blocked_scene_context"]["blocked_date"] == "2025-01-19"
    assert review["blocked_scene_context"]["reason"] == "corridor_bbox_scl_qa_cloud_or_cirrus_fraction_too_high"
    assert "production source-drape replacement" in review["forbidden_use"]


def test_pacuare_sentinel_20250320_scl_qa_review_passes_for_tci_probe_only():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    ...
    review = json.loads((pacuare_dir / PACUARE_SENTINEL_20250320_SCL_QA_REVIEW_FILE).read_text())
    rivers = {river["river_id"]: river for river in readiness["rivers"]}
    pacuare_sources = next(river for river in photoreal_sources["rivers"] if river["river_id"] == "pacuare")

    assert PACUARE_SENTINEL_20250320_SCL_QA_REVIEW_FILE in source_manifest["artifacts"]["imagery"]
    assert PACUARE_SENTINEL_20250320_SCL_QA_17PKM_IMAGE_FILE in source_manifest["artifacts"]["imagery"]
    assert PACUARE_SENTINEL_20250320_SCL_QA_16PHR_IMAGE_FILE in source_manifest["artifacts"]["imagery"]
    assert (pacuare_dir / PACUARE_SENTINEL_20250320_SCL_QA_17PKM_IMAGE_FILE).is_file()
    assert (pacuare_dir / PACUARE_SENTINEL_20250320_SCL_QA_16PHR_IMAGE_FILE).is_file()
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20250320_scl_qa_review"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/" + PACUARE_SENTINEL_20250320_SCL_QA_REVIEW_FILE
        in rivers["pacuare"]["attached_sources_by_class"]["aerial_or_satellite_imagery"]["artifacts"]
    )
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20250320_scl_qa_review"
    ...
    assert review["schema"] == "raftsim.pacuare_sentinel_20250320_scl_qa_review.v1"
    assert review["status"] == "cleaner_scene_scl_qa_passed_tci_review_pending"
    assert review["windowed_read_decision"]["scl_qa_passed_for_tci_review"] is True
    assert review["windowed_read_decision"]["tci_review_clips_downloaded"] is False
    assert review["windowed_read_decision"]["source_drape_replaced"] is False
    assert review["recommended_next_step"] == "download_tci_bbox_clips_for_march_20_2025"

    outputs = {output["tile_id"]: output for output in review["outputs"]}
    assert outputs["17PKM"]["dimensions_px"] == [1024, 875]
    assert outputs["17PKM"]["sha256"] == "6549c3832d8f7fdd71945b7c8e308b82dac5d82549f1a6b3857b91361e9db35b"
    assert outputs["17PKM"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.029006
    assert outputs["17PKM"]["class_summary"]["cloud_shadow_fraction"] == 0.029049
    assert outputs["16PHR"]["dimensions_px"] == [1024, 666]
    assert outputs["16PHR"]["sha256"] == "2d4a8750fc7abac6da78e4b32eee1cd04421f7ebb8616a5d1797eec757c1b3d2"
    assert outputs["16PHR"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.020025
    assert outputs["16PHR"]["class_summary"]["cloud_shadow_fraction"] == 0.018101
    assert "production source-drape replacement" in review["forbidden_use"]


def test_pacuare_sentinel_20250320_tci_bbox_clip_review_is_attached_but_not_promoted():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    ...
    review = json.loads((pacuare_dir / PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_REVIEW_FILE).read_text())
    rivers = {river["river_id"]: river for river in readiness["rivers"]}
    pacuare_sources = next(river for river in photoreal_sources["rivers"] if river["river_id"] == "pacuare")

    assert PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_REVIEW_FILE in source_manifest["artifacts"]["imagery"]
    assert PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_17PKM_IMAGE_FILE in source_manifest["artifacts"]["imagery"]
    assert PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_16PHR_IMAGE_FILE in source_manifest["artifacts"]["imagery"]
    assert (pacuare_dir / PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_17PKM_IMAGE_FILE).is_file()
    assert (pacuare_dir / PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_16PHR_IMAGE_FILE).is_file()
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20250320_tci_bbox_clip_review"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/" + PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_REVIEW_FILE
        in rivers["pacuare"]["attached_sources_by_class"]["aerial_or_satellite_imagery"]["artifacts"]
    )
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20250320_tci_bbox_clip_review"
    ...
    assert review["schema"] == "raftsim.pacuare_sentinel_20250320_tci_bbox_clip_review.v1"
    assert review["status"] == "sentinel_20250320_tci_bbox_window_clips_attached_review_only"
    assert review["windowed_read_decision"]["corridor_bbox_clip_previews_generated"] is True
    assert review["windowed_read_decision"]["exact_route_or_bank_clip_generated"] is False
    assert review["windowed_read_decision"]["stitched_corridor_mosaic_generated"] is False
    assert review["windowed_read_decision"]["source_drape_replaced"] is False
    assert review["windowed_read_decision"]["full_cogs_committed"] is False
    assert review["windowed_read_decision"]["scl_qa_review_attached"] is True

    source_cogs = {cog["tile_id"]: cog for cog in review["source_cogs"]}
    assert source_cogs["17PKM"]["full_cog_sha256"] == "5f3fbec512b86456dbb5c5e78aca777b40cad388f5434bb1897578fa534cdf22"
    assert source_cogs["16PHR"]["full_cog_sha256"] == "a1009ad7cecb14a9a6381abc535775906201fdf45788bc533f65e730b542b06c"
    outputs = {output["tile_id"]: output for output in review["outputs"]}
    assert outputs["17PKM"]["dimensions_px"] == [1024, 875]
    assert outputs["17PKM"]["sha256"] == "28fbf0ae030bd74c5570cd349bb444c9e6df6ea3fb668f39ce37490b34350d73"
    assert outputs["16PHR"]["dimensions_px"] == [1024, 666]
    assert outputs["16PHR"]["sha256"] == "a42a0931b739ca2c7571c57068e9ab3aa9c6318c411313a914f60ac209ead6eb"
    assert "production source-drape replacement" in review["forbidden_use"]


def test_pacuare_sentinel_augmented_preview_drape_is_unreal_preview_only():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    derivatives_manifest = json.loads((pacuare_dir / "production_import_pilot_derivatives_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    gap_register = build_production_environment_gap_register()
    photoreal_sources = json.loads(
        (
            Path(__file__).resolve().parents[2]
            / "unreal/Content/RaftSim/Rendering/photoreal_river_environment_sources.json"
    ...
    for artifact in [
        ...
        assert artifact in gap_rivers["pacuare"]["attached_preview_inputs"]

    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_augmented_source_drape_preview"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        derivatives_manifest["outputs"]["sentinel_augmented_preview_drape"]
        == "physics/data/real_world/pacuare_river_costa_rica/"
        + PACUARE_SENTINEL_AUGMENTED_SOURCE_DRAPE_PREVIEW_FILE
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/" + PACUARE_SENTINEL_AUGMENTED_SOURCE_DRAPE_PREVIEW_FILE
        in rivers["pacuare"]["attached_sources_by_class"]["aerial_or_satellite_imagery"]["artifacts"]
    )
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_augmented_source_drape_preview"
    ...
    assert manifest["schema"] == "raftsim.pacuare_sentinel_augmented_source_drape_preview.v1"
    assert manifest["status"] == "generated_review_gated_sentinel_calibrated_preview_not_production_source_drape"
    assert manifest["output"]["dimensions_px"] == [4096, 4096]
    assert manifest["output"]["sha256"] == "7f37001de40c17abad25564c978f542624fef241ba04ebed13bd80bf31c413cf"
    assert manifest["processing"]["production_source_drape_replaced"] is False
    assert manifest["processing"]["production_masks_replaced"] is False
    assert manifest["processing"]["unreal_preview_calibration_drape_enabled"] is True
    assert "production source-drape replacement" in manifest["forbidden_use"]
    assert "claiming lifelike Pacuare screenshot readiness" in manifest["forbidden_use"]
    assert PACUARE_SENTINEL_AUGMENTED_SOURCE_DRAPE_PREVIEW_FILE in editor_module


def test_pacuare_sentinel_20250320_draft_bank_window_review_blocks_promotion():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    ...
    review = json.loads((pacuare_dir / PACUARE_SENTINEL_20250320_DRAFT_BANK_WINDOW_REVIEW_FILE).read_text())
    rivers = {river["river_id"]: river for river in readiness["rivers"]}
    pacuare_sources = next(river for river in photoreal_sources["rivers"] if river["river_id"] == "pacuare")
    ...
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20250320_draft_bank_window_review"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/"
    ...
    assert review["schema"] == "raftsim.pacuare_sentinel_20250320_draft_bank_window_review.v1"
    assert review["status"] == "draft_bank_window_review_attached_cloud_shadow_blocked"
    assert review["windowed_read_decision"]["draft_bank_window_previews_generated"] is True
    assert review["windowed_read_decision"]["official_route_or_bank_clip_generated"] is False
    assert review["windowed_read_decision"]["scl_mask_applied_to_tci"] is False
    assert review["windowed_read_decision"]["source_drape_replaced"] is False
    assert review["windowed_read_decision"]["cloud_screening_blocks_promotion"] is True
    assert review["recommended_next_step"] == "probe_alternate_scene_or_exact_route_scl_mask_before_source_drape"

    outputs = {(output["tile_id"], output["kind"]): output for output in review["outputs"]}
    assert outputs["17PKM", "tci"]["dimensions_px"] == [1024, 223]
    assert outputs["17PKM", "tci"]["sha256"] == "4b6f86136d5f33e2b4b3bfedfcb6e6c0acf4fedd3b5aed76c42d1ccf991aa444"
    assert outputs["16PHR", "tci"]["dimensions_px"] == [1024, 146]
    assert outputs["16PHR", "tci"]["sha256"] == "b01b8f9b473084f8ac1e2f9a7ff1ecfa17c9b31fed6e29a36796e748b3ccf4db"
    assert outputs["17PKM", "scl"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.037358
    assert outputs["17PKM", "scl"]["class_summary"]["cloud_shadow_fraction"] == 0.039898
    assert outputs["16PHR", "scl"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.050347
    assert outputs["16PHR", "scl"]["class_summary"]["cloud_shadow_fraction"] == 0.038287
    assert "production source-drape replacement" in review["forbidden_use"]


def test_pacuare_sentinel_20250305_draft_bank_scl_probe_blocks_tci_download():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    ...
    review = json.loads((pacuare_dir / PACUARE_SENTINEL_20250305_DRAFT_BANK_SCL_PROBE_REVIEW_FILE).read_text())
    rivers = {river["river_id"]: river for river in readiness["rivers"]}
    pacuare_sources = next(river for river in photoreal_sources["rivers"] if river["river_id"] == "pacuare")
    ...
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20250305_draft_bank_scl_probe"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/"
    ...
    assert review["schema"] == "raftsim.pacuare_sentinel_20250305_draft_bank_scl_probe.v1"
    assert review["status"] == "alternate_scene_scl_probe_failed_route_local_cloud_shadow"
    assert review["windowed_read_decision"]["scl_qa_preview_generated"] is True
    assert review["windowed_read_decision"]["tci_review_clips_downloaded"] is False
    assert review["windowed_read_decision"]["source_drape_replaced"] is False
    assert review["windowed_read_decision"]["cloud_screening_blocks_promotion"] is True
    assert review["recommended_next_step"] == "skip_march_5_tci_and_probe_next_cleaner_route_local_scene"

    outputs = {output["tile_id"]: output for output in review["outputs"]}
    assert outputs["17PKM"]["sha256"] == "a691ddc9a1cc86ca952b61fa86bf7ab3880b2ca083354d6170eb35ad6296ae35"
    assert outputs["17PKM"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.099172
    assert outputs["17PKM"]["class_summary"]["cloud_shadow_fraction"] == 0.088355
    assert outputs["16PHR"]["sha256"] == "515ab212d7d76949294fa3e5a52c0bf7697374d1a5d9b3317bfbce2866b6c43f"
    assert outputs["16PHR"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.116973
    assert outputs["16PHR"]["class_summary"]["cloud_shadow_fraction"] == 0.064959
    assert "production source-drape replacement" in review["forbidden_use"]
...
def test_pacuare_sentinel_20240204_draft_bank_scl_probe_and_tci_review_block_promotion():
    ...
    assert outputs["17PKM"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.03646
    assert outputs["17PKM"]["class_summary"]["cloud_shadow_fraction"] == 0.047691
    assert outputs["16PHR"]["sha256"] == "53d48bf8b29e7ddb9e142b25693898f4162f7b057711454910c42f1cb74dd470"
...
def test_pacuare_sentinel_20240224_draft_bank_scl_probe_blocks_tci_download():
    pacuare_dir = REAL_WORLD_DATA_DIR / "pacuare_river_costa_rica"
    source_manifest = json.loads((pacuare_dir / "source_manifest.json").read_text())
    pull_manifest = json.loads((pacuare_dir / "production_source_pull_manifest.json").read_text())
    readiness = json.loads((REAL_WORLD_DATA_DIR / "production_geospatial_source_readiness.json").read_text())
    ...
    review = json.loads((pacuare_dir / PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_PROBE_REVIEW_FILE).read_text())
    rivers = {river["river_id"]: river for river in readiness["rivers"]}
    pacuare_sources = next(river for river in photoreal_sources["rivers"] if river["river_id"] == "pacuare")
    ...
    assert any(
        artifact["artifact_id"] == "pacuare_sentinel_20240224_draft_bank_scl_probe"
        for artifact in pull_manifest["pulled_artifacts"]
    )
    assert (
        "physics/data/real_world/pacuare_river_costa_rica/"
    ...
    assert review["schema"] == "raftsim.pacuare_sentinel_20240224_draft_bank_scl_probe.v1"
    assert review["status"] == "alternate_scene_scl_probe_failed_route_local_cloud_shadow"
    assert review["windowed_read_decision"]["scl_qa_preview_generated"] is True
    assert review["windowed_read_decision"]["tci_review_clips_downloaded"] is False
    assert review["windowed_read_decision"]["source_drape_replaced"] is False
    assert review["recommended_next_step"] == "skip_february_24_tci_and_continue_cleaner_scene_search"

    outputs = {output["tile_id"]: output for output in review["outputs"]}
    assert outputs["17PKM"]["sha256"] == "4484ae9a54495c9306727b9dfcd1b421f4f6c1ce12a6cdb2786beec8d1229285"
    assert outputs["17PKM"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.445073
    assert outputs["17PKM"]["class_summary"]["cloud_shadow_fraction"] == 0.139582
    assert outputs["16PHR"]["sha256"] == "857fbb8594cf875a4eef508853afd324b391abbb3b8a6b1cb5393b80a36efc69"
    assert outputs["16PHR"]["class_summary"]["cloud_or_cirrus_fraction"] == 0.442627
    assert outputs["16PHR"]["class_summary"]["cloud_shadow_fraction"] == 0.108325
    assert "production source-drape replacement" in review["forbidden_use"]
...
def test_production_geospatial_attachment_audit_matches_ledger_and_keeps_preview_gate():
    ...
    assert rivers["pacuare"]["artifact_count"] == 32
...
def test_production_environment_gap_register_tracks_lifelike_blockers_for_all_rivers():
    ...
    assert PACUARE_SENTINEL_CLEANER_SCENE_SEARCH_REVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_SCL_QA_REVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_SCL_QA_17PKM_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_SCL_QA_16PHR_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_REVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_17PKM_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_TCI_BBOX_CLIP_16PHR_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_AUGMENTED_SOURCE_DRAPE_PREVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_AUGMENTED_SOURCE_DRAPE_PREVIEW_MANIFEST_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_DRAFT_BANK_WINDOW_REVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_DRAFT_BANK_WINDOW_TCI_17PKM_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_DRAFT_BANK_WINDOW_TCI_16PHR_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_DRAFT_BANK_WINDOW_SCL_17PKM_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250320_DRAFT_BANK_WINDOW_SCL_16PHR_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250305_DRAFT_BANK_SCL_PROBE_REVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250305_DRAFT_BANK_SCL_17PKM_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20250305_DRAFT_BANK_SCL_16PHR_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    ...
    assert PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_PROBE_REVIEW_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_17PKM_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
    assert PACUARE_SENTINEL_20240224_DRAFT_BANK_SCL_16PHR_IMAGE_FILE in rivers["pacuare"]["attached_preview_inputs"]
