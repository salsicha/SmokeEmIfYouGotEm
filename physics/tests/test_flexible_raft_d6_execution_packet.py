...
def test_flexible_raft_d6_execution_packet_is_reproducible_and_not_promoted():
    chaos_sidecar = json.loads(
        (REPO_ROOT / D6_CHAOS_MEASURED_RESULTS_RELATIVE_PATH).read_text(
            encoding="utf-8"
        )
    )
    compliant_sidecar = json.loads(
        (REPO_ROOT / D6_PROJECT_CHRONO_SIDECAR_RELATIVE_PATH).read_text(
            encoding="utf-8"
        )
    )
    measured_results = json.loads(
    ...
    assert committed["summary"]["completed_chaos_job_count"] == len(
        REQUIRED_D6_FIXTURE_IDS
    )
    assert committed["summary"]["completed_compliant_job_count"] == len(
        REQUIRED_D6_FIXTURE_IDS
    )
    assert committed["summary"]["pending_compliant_job_count"] == 0
    assert committed["summary"]["pending_external_job_count"] == 0
    assert committed["summary"]["all_measurements_present"] is True
    assert committed["summary"]["comparison_passed"] is True
    assert committed["summary"]["ready_for_manual_promotion_review"] is True
    assert committed["summary"]["can_promote_d6_now"] is False


def test_flexible_raft_d6_execution_packet_consumes_valid_chaos_measurements():
    chaos_sidecar = json.loads(
        (REPO_ROOT / D6_CHAOS_MEASURED_RESULTS_RELATIVE_PATH).read_text(
            encoding="utf-8"
        )
    ...
    chaos_jobs = [
        job
        for job in packet["execution_jobs"]
        if job["target_id"] == "unreal_chaos_rigid_baseline"
    ]
    compliant_jobs = [
        job
        for job in packet["execution_jobs"]
    ...
    assert all(job["measurement_recorded"] for job in chaos_jobs)
    assert all(
        job["status"] == "measured_engine_output_manual_review_pending"
        for job in chaos_jobs
    )
    assert all(not job["measurement_recorded"] for job in compliant_jobs)
    groups = {group["target_id"]: group for group in packet["adapter_groups"]}
    assert groups["unreal_chaos_rigid_baseline"]["pending_job_count"] == 0
    assert groups["project_chrono_or_reviewed_compliant_model"][
        "pending_job_count"
    ] == len(REQUIRED_D6_FIXTURE_IDS)
...
def test_flexible_raft_d6_execution_packet_tracks_all_pending_external_jobs():
    ...
    assert (
        summary["committed_report_missing_target_count"]
        == len(REQUIRED_D6_FIXTURE_IDS) * 2
    )
...
def test_flexible_raft_d6_execution_packet_links_source_artifacts():
    ...
    assert (
        artifacts["measured_results_template"]
        == D6_MEASURED_RESULTS_TEMPLATE_RELATIVE_PATH
    )
    ...
    assert (
        packet["local_preparation_checks"][
            "can_generate_empty_target_sidecar_templates"
        ]
        is True
    )
    assert (
        packet["local_preparation_checks"]["can_validate_target_sidecars_before_merge"]
        is True
    )
    assert (
        packet["local_preparation_checks"][
            "can_run_external_measurement_gate_with_python_only"
        ]
        is False
    )
...
def test_flexible_raft_d6_execution_jobs_cover_manifest_tasks_with_evidence():
    ...
    for job in jobs:
        ...
        assert (
            job["required_evidence"]["metrics"]["metric_paths"]
            == job["required_metric_paths"]
        )
...
def test_flexible_raft_d6_execution_packet_separates_compliant_and_chaos_policies():
    ...
    assert (
        ...
        in compliant["sidecar_merge_command"]
    )
    ...
    assert (
        ...
        in chaos["sidecar_merge_command"]
    )
...
def test_flexible_raft_d6_execution_packet_blocks_runtime_until_reviewed():
    ...
    assert (
        packet["post_measurement_workflow"][0]["required_job_count"]
        == len(REQUIRED_D6_FIXTURE_IDS) * 2
    )
    ...
    assert (
        packet["post_measurement_workflow"][-1]["may_mark_d6_complete_before_review"]
        is False
    )
