"""Behavioral validation of the committed Terminator C3 cooked flow fields package.

This is the authored-scenario-window counterpart to ``test_cooked_flow_fields.py``
(and mirrors ``test_troublemaker_cooked_flow_fields.py``). It checks the same v1
schema the UE loader reads, plus the additions the Terminator window requires: the
manifest records the Manning n each band was solved with, and convergence is
recorded honestly (the strong authored hydraulics -- and the thin, only-partly-
conveying low big-water band -- keep feature-scale cells unsteady, so the converged
flag is only asserted to be self-consistent with the recorded windows, never forced
true).
"""
...
from raftsim.futaleufu_terminator_c3_window import FLOW_BAND_IDS
...
def manifest() -> dict:
    if not (PACKAGE_DIR / "manifest.json").exists():
        pytest.skip("Terminator cooked flow fields package is not committed yet.")
...
def test_manifest_declares_v1_schema_and_all_three_bands(manifest):
    ...
    assert manifest["river_id"] == "futaleufu_river_chile"
    assert manifest["section_id"] == "terminator_station_5312m_c3_window"
    assert manifest["window_id"] == "terminator_station_5312m_c3_window"
    assert manifest["rapid_name"] == "Terminator"
...
def test_manifest_grid_matches_committed_scenario_packages(manifest):
    ...
    assert grid["ny"] == scenario["grid"]["ny"] == 43
...
def test_band_fields_are_physically_sane(manifest, band_id):
    ...
    # west and mid cross-sections, consistent with the manifest record.
    assert band["discharge_steady_m3s"]["west"] > 0.0
    assert band["discharge_steady_m3s"]["mid"] > 0.0
    mid = h.shape[1] // 2
