...
```bash
cd physics
PYTHONPATH=src python -m raftsim.examples.compare_dual_solver_probes \
  outputs/dual_solver/flat_pool
```

This writes `probe_comparison.json` with matched point-probe and cross-section error summaries. Samples are aligned by nearest exported time so PyClaw can write sparse reference frames while the C++ solver writes denser runtime frames.

Compare higher-level diagnostics with:

```bash
cd physics
PYTHONPATH=src python -m raftsim.examples.compare_dual_solver_diagnostics \
  outputs/dual_solver/flat_pool
```
...
Run a first C++ parameter sweep against PyClaw with:
...
PYTHONPATH=src python -m raftsim.examples.tune_cpp_solver \
  --cpp-solver /tmp/raftsim-water-build/raftsim_water_solver \
  --fixture flat_pool \
  --output-dir outputs/tuning/flat_pool
```
...
`python -m raftsim.examples.fit_parameters --cpp-solver build/raftsim_water_solver --fixture flat_pool --scale-values 0.85,1.0,1.15` runs combined fitting: each candidate is scored against PyClaw water-field thresholds and raft-force envelope agreement sampled from PyClaw and C++ water outputs.
