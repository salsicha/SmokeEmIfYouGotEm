...
- [x] Decide the constriction y-face hydrostatic/source-splitting scope from the native audit: `physics/reports/milestone18/constriction_hydrostatic_source_decision.*` records `TEST_REQUIRED`, targets `upper_edge_face` column 6 rows 8-9 first, keeps feature/gameplay forcing disabled, requires manifest-recorded source-split parameters and conservation deltas, and blocks promotion unless the face/source report, throat/shape/timing diagnostics, Milestone 17 guardrail, and threshold report all support the change. The refreshed remaining-geometry queue now carries this decision as focused constriction evidence.
