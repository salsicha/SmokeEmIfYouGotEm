...
def finish(error=None):
    ...
    for path,digest in protected.items():
        if sha(Path(path))!=digest:error=(error or '')+'\nProtected package changed: '+path
...
def tick(delta):
    try:
        ...
        if not state.get('verified_play_world'):
            ...
            unreal.log('Verified actual PIE uses the paired revised terrain and source-matched water: '+json.dumps(state))
...
try:
    ...
    descriptor_path=(ROOT/raw['descriptor']).resolve();report=(ROOT/raw['report']).resolve();label=raw['label']
