param(
    [ValidatePattern('^[a-z0-9_]+$')][string]$Label = 'hance_verified',
    [int[]]$Stations = @(100, 300, 410),
    [int]$FrameCount = 8
)
...
foreach ($Station in $Stations) {
    if ($Station -lt 0 -or $Station -gt 600) { throw 'Station is outside the Hance window.' }
    $CaptureLabel = "${Label}_${Station}"
    $LogPath = Join-Path $ProjectRoot "Saved/Logs/${CaptureLabel}.log"
    Write-Output "Reviewing Hance station $Station m"
    & $EditorExe $Project /Game/RaftSim/Maps/L_Hance `
        -game -Unattended -NoSplash -RenderOffscreen -ResX=1280 -ResY=720 -windowed `
        "-ExecCmds=RaftSim.CaptureRaftSeries 8 $FrameCount 0.3 $CaptureLabel 3.2 2 1.7 8 station=$Station" `
        "-abslog=$LogPath"
    if ($LASTEXITCODE -ne 0) { throw "Hance capture failed: $Station" }
