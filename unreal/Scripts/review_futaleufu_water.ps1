param(
    [ValidatePattern('^[a-z0-9_]+$')][string]$Label = 'futaleufu_verified',
    [int[]]$Stations = @(24, 150, 300, 480),
    [int]$FrameCount = 8,
...
foreach ($Station in $Stations) {
    if ($Station -lt 0 -or $Station -gt 600) { throw 'Station is outside Terminator.' }
    $CaptureLabel = "${Label}_${Station}"
    $LogPath = Join-Path $ProjectRoot "Saved/Logs/${CaptureLabel}.log"
    Write-Output "Reviewing Futaleufu station $Station m"
    & $EditorExe $Project /Game/RaftSim/Maps/L_Terminator `
        ...
        "-abslog=$LogPath"
    if ($LASTEXITCODE -ne 0) { throw "Futaleufu capture failed: $Station" }
