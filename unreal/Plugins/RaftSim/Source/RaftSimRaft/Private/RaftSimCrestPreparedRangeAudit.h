...
namespace RaftSimCrestPreparedRangeAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestPreparedRangeAudit="),P);return P;}();
    ...
    double Times[2]={};bool Passed=bool(Input.HeightRangeWidthAtWorldXYCm) && bool(Input.PreparedHeightRangeWidthAtWorldXYCm);
    for(int32 Offset=0;Offset<2;++Offset)
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];
        W.HeightRangeWidthCm=Kind ? Input.PreparedHeightRangeWidthAtWorldXYCm : Input.HeightRangeWidthAtWorldXYCm;
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("prepared_first"),Calls%2==1);
        Row->SetNumberField(TEXT("reference_ms"),Times[0]);Row->SetNumberField(TEXT("prepared_ms"),Times[1]);
        Row->SetNumberField(TEXT("preparation_ms"),Input.PreparedRangeConstructionMs);
    ...
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating actual changed-input builds after two warm builds; reference vs immutable prepared range constants. Separate preparation_ms must be charged to each candidate build for conservative qualification. Exact parents, triangles, ownership, expanded coordinates and production topology. Original quarter-point tolerance and detail-window refinement unchanged. Diagnostic output is never published; not FPS or release acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestPreparedRangeAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
