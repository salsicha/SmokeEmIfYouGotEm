...
namespace RaftSimCrestBoundMemoAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestBoundMemoAudit="),P);return P;}();
    ...
    for(int32 Offset=0;Offset<2;++Offset)
    {
        ...
        W.bBoundCoordinateMemo=Kind==1;
        W.HeightRangeWidthCm=Input.HeightRangeWidthAtWorldXYCm;
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("bound_first"),Calls%2==1);
        Row->SetNumberField(TEXT("reference_ms"),Times[0]);Row->SetNumberField(TEXT("bound_ms"),Times[1]);
        Row->SetNumberField(TEXT("reference_retained_bytes"),Work[0].GetRetainedMemoAllocatedBytes());
        Row->SetNumberField(TEXT("bound_retained_bytes"),Work[1].GetRetainedMemoAllocatedBytes());
    ...
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating actual changed-input builds after two warmups. Exact ordered parents, triangles, ownership, expanded coordinates and production topology. Current heights are recomputed per profile epoch; only coordinate lookup bindings are reused. Diagnostic arrays are never published. Not ordinary FPS or release acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestBoundMemoAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
