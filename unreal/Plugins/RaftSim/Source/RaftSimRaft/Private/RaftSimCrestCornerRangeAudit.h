...
namespace RaftSimCrestCornerRangeAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestCornerRangeAudit="),P);return P;}();
    ...
    double Times[2]={};bool Passed=bool(Input.TightHeightRangeWidthAtWorldXYCm);
    for(int32 Offset=0;Offset<2;++Offset)
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];
        W.bIndexedEdges=true;
        W.HeightRangeWidthCm=Input.TightHeightRangeWidthAtWorldXYCm;
        const double Start=FPlatformTime::Seconds();
        if(Kind)Passed&=W.BuildAdaptive<true>(XY,Triangles,Input.HeightAtWorldXYCm,3,.5f,Input.NonzeroRegionsCm,
                nullptr,true,true,Input.DetailSpanCm>0 ? &Input.DetailWindowCm : nullptr,Input.DetailSpanCm,true);
        else Passed&=W.BuildAdaptive(XY,Triangles,Input.HeightAtWorldXYCm,3,.5f,Input.NonzeroRegionsCm,
                nullptr,true,true,Input.DetailSpanCm>0 ? &Input.DetailWindowCm : nullptr,Input.DetailSpanCm,true);
    ...
    Report->SetStringField(TEXT("schema"),TEXT("raftsim.crest_corner_range_pair.v1"));
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating actual changed-input builds after two warmups. Candidate samples corners before the range query, skipping that query only when their spread exceeds the unchanged tolerance. Same current profile, indexed edges, quarter-point selection, 0.5cm tolerance and detail window. Exact ordered parents, indices, ownership, expanded coordinates and production topology. Diagnostic output is never published. Not FPS or visual acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestCornerRangeAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
