...
namespace RaftSimCrestIntervalAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestIntervalAudit="),P);return P;}();
    ...
    double Times[2]={};bool Passed=bool(Input.PreparedHeightRangeWidthAtWorldXYCm) && bool(Input.TightHeightRangeWidthAtWorldXYCm);
    for(int32 Offset=0;Offset<2;++Offset)
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];
        W.bIndexedEdges=true;
        W.HeightRangeWidthCm=Kind ? Input.TightHeightRangeWidthAtWorldXYCm : Input.PreparedHeightRangeWidthAtWorldXYCm;
    ...
    Report->SetStringField(TEXT("schema"),TEXT("raftsim.crest_interval_pair.v1"));
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating actual changed-input builds after two warmups, same immutable prepared sites and indexed edges. Exact ordered parents, triangles, ownership, expanded coordinates and production topology. Original 0.5cm quarter-point and detail-window selection unchanged. No diagnostic output published. Not FPS or visual acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestIntervalAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
