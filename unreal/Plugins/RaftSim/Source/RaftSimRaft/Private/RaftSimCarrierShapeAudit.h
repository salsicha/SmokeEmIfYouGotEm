    ...
    TConstArrayView<FVector> TargetPositions,TConstArrayView<float> TargetCrestCm)
{
    ...
    const FString NormalPath=Path+TEXT(".normals.csv");
    if (FPaths::FileExists(Path) || FPaths::FileExists(VertexPath) ||
        FPaths::FileExists(TrianglePath) || FPaths::FileExists(SourcePath) ||
        FPaths::FileExists(NormalPath)) return false;
    ...
    Rows=TEXT("id,normal_x,normal_y,normal_z\n");
    for (int32 I=0;I<ActiveCount;++I)
    {
        const FVector& Normal=Vertices[I].Normal;
        if (Normal.ContainsNaN()) return false;
        Rows.Appendf(TEXT("%d,%.17g,%.17g,%.17g\n"),I,Normal.X,Normal.Y,Normal.Z);
    }
    if (!FFileHelper::SaveStringToFile(Rows,*NormalPath)) return false;
    ...
    Report->SetStringField(TEXT("normal_scope"),TEXT("Current submitted CPU vertex normals, before tangent packing, material normal maps or GPU detail. Not a measured optical normal."));
