    ...
    TConstArrayView<float> BedM,float WorldYSign)
{
    ...
    if (ActiveCount<=0 || ActiveCount>Vertices.Num() || Coarse.Num()!=ActiveCount ||
        ...
        DepthM.Num()!=Nx*Ny || BedM.Num()!=Nx*Ny) return false;
    ...
    Rows=TEXT("id,field_x_m,field_y_m,clipping_wet,source_bed_plus_depth_m,source_depth_m\n");
    for (int32 I=0;I<DepthM.Num();++I)
    {
        const auto& P=FieldCoordinates[I];
        Rows.Appendf(TEXT("%d,%.17g,%.17g,%d,%.17g,%.17g\n"),I,P.X,P.Y,
    ...
    Report->SetStringField(TEXT("schema"),TEXT("raftsim.submitted_carrier_shape.v1"));
