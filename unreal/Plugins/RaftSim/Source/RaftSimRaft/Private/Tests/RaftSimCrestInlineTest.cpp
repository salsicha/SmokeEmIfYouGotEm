...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRaftSimCrestInlineTest,
    "RaftSim.M4.CrestInlineSelection",EAutomationTestFlags::EditorContext|EAutomationTestFlags::EngineFilter)
bool FRaftSimCrestInlineTest::RunTest(const FString&)
{
    ...
    Candidate.bInlineSelection=true;
    int64 Compared=0;
    for(int32 Frame=0;Frame<24;++Frame)
    {
        ...
        TestTrue(TEXT("typed predicate preserves original and serial ordered topology, ownership and coordinates"),
    ...
    AddInfo(FString::Printf(TEXT("Inline/erased/serial exact over %lld vertices and 24 moving profiles, crops, winding, regions, detail windows and cache epochs"),Compared));
    return !HasAnyErrors();
}
#endif
