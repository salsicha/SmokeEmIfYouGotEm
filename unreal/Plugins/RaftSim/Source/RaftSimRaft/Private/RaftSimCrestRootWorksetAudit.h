...
namespace RaftSimCrestRootWorksetAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestRootWorksetAudit="),P);return P;}();
    ...
    for(int32 Offset=0;Offset<2;++Offset)
    {
        ...
        W.bSparseRoots=Kind==1;W.bIndexedEdges=true;
        W.HeightRangeWidthCm=Input.PreparedHeightRangeWidthAtWorldXYCm
    ...
    TArray<FVector2D> Points[2];for(int32 I=0;I<2;++I)Work[I].Expand(XY,Points[I]);
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("sparse_first"),Calls%2==1);
        Row->SetNumberField(TEXT("reference_ms"),Times[0]);Row->SetNumberField(TEXT("sparse_ms"),Times[1]);
        Row->SetNumberField(TEXT("roots"),Work[1].RootTrianglesConsidered);
        Row->SetNumberField(TEXT("workset_roots"),Work[1].RootTrianglesRetained);
        Row->SetNumberField(TEXT("vertices"),Points[0].Num());Row->SetNumberField(TEXT("triangles"),Work[0].Triangles.Num()/3);
        Rows.Add(MakeShared<FJsonValueObject>(Row));
    }
    ++Calls;if(Passed && Rows.Num()<16)return;Done=true;
    auto Report=MakeShared<FJsonObject>();Report->SetBoolField(TEXT("exact"),Passed);
    Report->SetStringField(TEXT("scope"),TEXT("16 alternating actual changed-input builds after two warmups. Exact ordered parents, full triangles, ownership, expanded coordinates and production topology. Timing includes current-support pruning and full-output merge. Diagnostic outputs are never published. Not FPS or visual acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestRootWorksetAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
