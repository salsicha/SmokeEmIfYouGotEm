...
namespace RaftSimCrestLevelMemoAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestLevelMemoAudit="),P);return P;}();
    ...
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];W.bLevelLocalMemos=Kind==1;
    ...
    // The first two actual changed-input builds warm independent memo/topology
    // histories. Each retained pair uses a fresh actual input/profile epoch.
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("level_first"),Calls%2==1);
        Row->SetNumberField(TEXT("shared_ms"),Times[0]);Row->SetNumberField(TEXT("level_ms"),Times[1]);
        Row->SetNumberField(TEXT("shared_retained_bytes"),Work[0].GetRetainedMemoAllocatedBytes());
        Row->SetNumberField(TEXT("level_retained_bytes"),Work[1].GetRetainedMemoAllocatedBytes());
    ...
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating-order actual changed-input builds after two warm builds; exact ordered parents, triangles, ownership, expanded coordinates and production topology. No diagnostic arrays are published. Not ordinary FPS or release acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestLevelMemoAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
