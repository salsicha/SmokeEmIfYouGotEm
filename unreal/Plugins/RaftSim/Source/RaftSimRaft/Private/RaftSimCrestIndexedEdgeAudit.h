...
namespace RaftSimCrestIndexedEdgeAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestIndexedEdgeAudit="),P);return P;}();
    ...
    for(int32 Offset=0;Offset<2;++Offset)
    {
        ...
        W.bIndexedEdges=Kind==1;W.bMeasureStages=true;
        // Both sides use the currently installed exact range optimization.
        ...
        W.HeightRangeWidthCm={}; // Do not retain this frame's captured profile.
    }
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetNumberField(TEXT("legacy_ms"),Times[0]);Row->SetNumberField(TEXT("indexed_ms"),Times[1]);
        Row->SetNumberField(TEXT("legacy_assembly_ms"),Work[0].AssemblySeconds*1000.);
        Row->SetNumberField(TEXT("indexed_assembly_ms"),Work[1].AssemblySeconds*1000.);
    ...
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating-order actual changed-input whole builds after two warm builds; exact ordered parents, triangles, owners, expanded coordinates and production topology. Includes map construction. Not ordinary FPS, physics or release acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestIndexedEdgeAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
