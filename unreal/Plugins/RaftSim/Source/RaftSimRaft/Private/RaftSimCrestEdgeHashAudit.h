...
namespace RaftSimCrestEdgeHashAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestEdgeHashAudit="),P);return P;}();
    ...
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];W.bStrongEdgeHash=Kind==1;W.bMeasureStages=true;
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("strong_first"),Calls%2==1);
        Row->SetNumberField(TEXT("legacy_ms"),Times[0]);Row->SetNumberField(TEXT("strong_ms"),Times[1]);
        Row->SetNumberField(TEXT("legacy_assembly_ms"),Work[0].AssemblySeconds*1000.);
        Row->SetNumberField(TEXT("strong_assembly_ms"),Work[1].AssemblySeconds*1000.);
    ...
    UE_LOG(LogTemp,Display,TEXT("CrestEdgeHashAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
