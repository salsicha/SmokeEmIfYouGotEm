...
namespace RaftSimCrestAdjacentRangeAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestAdjacentRangeAudit="),P);return P;}();
    ...
    double Times[2]={};bool Passed=bool(Production.HeightRangeWidthCm);
    for(int32 Offset=0;Offset<2;++Offset)
    {
        ...
        W.bIndexedEdges=Production.bIndexedEdges;
        W.bInlineSelection=Production.bInlineSelection;
        const double Start=FPlatformTime::Seconds();
        Passed&=W.BuildAdaptive(XY,Triangles,Input.HeightAtWorldXYCm,3,.5f,Input.NonzeroRegionsCm,
            nullptr,true,true,Input.DetailSpanCm>0 ? &Input.DetailWindowCm : nullptr,Input.DetailSpanCm,
            true,true,true,false,Kind==1);
    ...
    Report->SetStringField(TEXT("schema"),TEXT("raftsim.crest_adjacent_range_pairs.v1"));
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating actual changed-input full adaptive builds after two warm builds; exact adjacent-box range memo vs uncached range. Includes cache allocation/destruction. Exact ordered parents, triangles, ownership, expanded coordinates and production topology. No geometry, sampling, refresh or tolerance changes. Diagnostic outputs are not published; not FPS or physical acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestAdjacentRangeAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
