...
namespace RaftSimCrestEmptyTileAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    static const FString Path=[]{FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestEmptyTileAudit="),P);return P;}();
    ...
    if(!Input.EmptyTileComparisonHeight[0] || !Input.EmptyTileComparisonHeight[1])return;
    double Times[2]={};bool Passed=true;
    for(int32 Offset=0;Offset<2;++Offset)
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];
        W.bIndexedEdges=true;W.bMeasureStages=true;
        W.HeightRangeWidthCm=Production.HeightRangeWidthCm;
        const double Start=FPlatformTime::Seconds();
        Passed&=W.BuildAdaptive(XY,Triangles,Input.EmptyTileComparisonHeight[Kind],3,.5f,Input.NonzeroRegionsCm,
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("skip_first"),Calls%2==1);
        Row->SetNumberField(TEXT("legacy_ms"),Times[0]);Row->SetNumberField(TEXT("skip_ms"),Times[1]);
        Row->SetNumberField(TEXT("legacy_assembly_ms"),Work[0].AssemblySeconds*1000.);
        Row->SetNumberField(TEXT("skip_assembly_ms"),Work[1].AssemblySeconds*1000.);
    ...
    Report->SetStringField(TEXT("scope"),TEXT("64 alternating-order actual-input builds after two warm builds; empty validated physical tile shortcut only. Exact ordered parents, triangles, owners, expanded XY, cache decisions and production topology. Not ordinary FPS, physics or visual acceptance."));
    Report->SetArrayField(TEXT("pairs"),Rows);FString Json;
    FJsonSerializer::Serialize(Report,TJsonWriterFactory<>::Create(&Json));
    const bool Saved=FFileHelper::SaveStringToFile(Json,*Path);
    UE_LOG(LogTemp,Display,TEXT("CrestEmptyTileAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
