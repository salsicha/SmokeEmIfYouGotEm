...
namespace RaftSimCrestInlineAudit
{
    ...
    const FRaftSimShorelineCrestInput& Input,const FRaftSimSurfaceRefinement& Production)
{
    ...
    {FString P;FParse::Value(FCommandLine::Get(),TEXT("RaftSimCrestInlineAudit="),P);return P;}();
    ...
    {
        const int32 Kind=(Calls+Offset)%2;auto& W=Work[Kind];W.bInlineSelection=Kind==1;
    ...
    if(Calls>=2 || !Passed)
    {
        ...
        Row->SetBoolField(TEXT("inline_first"),Calls%2==1);
        Row->SetNumberField(TEXT("erased_ms"),Times[0]);Row->SetNumberField(TEXT("inline_ms"),Times[1]);
    ...
    UE_LOG(LogTemp,Display,TEXT("CrestInlineAudit completed exact=%d saved=%d pairs=%d path=%s"),Passed,Saved,Rows.Num(),*Path);
#endif
}
}
