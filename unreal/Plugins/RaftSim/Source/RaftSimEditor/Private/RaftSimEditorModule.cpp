    ...
    const FRaftSimPreviewImage* VegetationMask)
{
    ...
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    const int32 CrownCount = Spec.bDesertCanyon ? 28 : (bRainforest ? 96 : 58);
    const int32 ShadowQuadsPerCrown = Spec.bDesertCanyon ? 1 : (bRainforest ? 2 : 2);
    const int32 LeafletsPerCrown = Spec.bDesertCanyon ? 4 : (bRainforest ? 12 : 8);
    ...
    const float FoliageCardSilhouetteDemotion = Spec.bDesertCanyon ? 0.86f : (bRainforest ? 0.58f : 0.66f);
    const float FirstPartyProceduralCanopyBlobDemotion = Spec.bDesertCanyon ? 0.86f : (bRainforest ? 0.68f : 0.74f);
    const FVector Up(0.0f, 0.0f, 1.0f);
        ...
        for (int32 YIndex = 0; YIndex <= YSteps; ++YIndex)
        {
            ...
            Vertices.Add(FVector(
                ...
                TerrainZ + BaseTerrainMicroReliefCm + SourceAwareTerrainPhotoMottleReliefCm +
                    ...
                    SourceAwareMacroTerrainRidgeReliefCm));
...
void AddPreviewRiverRibbonMesh(UWorld* World, const FRaftSimEnvironmentPreviewSpec& Spec)
{
    ...
    const FLinearColor FlowCuedWaterFoamMottleColor = Spec.bDesertCanyon
        ...
        : (Spec.bHasWaterfalls ? FLinearColor(0.52f, 0.76f, 0.66f) : FLinearColor(0.50f, 0.73f, 0.72f));
    ...
    const FLinearColor BaseWaterCrossChannelHighlight = Spec.bDesertCanyon
        ...
        : (Spec.bHasWaterfalls ? FMath::Lerp(Spec.WaterColor, FLinearColor(0.090f, 0.45f, 0.36f), 0.22f)
                                : FMath::Lerp(Spec.WaterColor, FLinearColor(0.090f, 0.49f, 0.50f), 0.20f));
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    ...
    const float LongDarkWaterStreakDemotion = 0.12f;
    const float BaseWaterCenterGuideStripeDemotion = 0.0f;
    const float WaterLineArtifactColorReblend = Spec.bDesertCanyon ? 0.22f : 0.30f;
    ...
    for (int32 XIndex = 0; XIndex <= XSteps; ++XIndex)
    {
        ...
        for (int32 CrossIndex = 0; CrossIndex <= CrossSteps; ++CrossIndex)
        {
            ...
            WaterColor = FMath::Lerp(
                ...
                FMath::Clamp(
                    CenterT * WaterLineArtifactColorReblend +
                        CenterGuideStripeT * (0.42f + 0.38f * NearFieldCenterStripeDemotionT),
                    0.0f,
                    0.88f));
            const FLinearColor CenterLaneEraseColor = FMath::Lerp(Spec.WaterColor, SurfaceGlint, Spec.bDesertCanyon ? 0.28f : 0.18f);
            const float CenterLaneBroadEraseT = SmoothPreviewStep(0.12f, 0.68f, CenterT);
            WaterColor = FMath::Lerp(
                WaterColor,
                CenterLaneEraseColor,
                FMath::Clamp(CenterLaneBroadEraseT * (0.36f + 0.38f * NearFieldCenterStripeDemotionT), 0.0f, 0.72f));
            WaterColor = FMath::Lerp(
                WaterColor,
            ...
            const FLinearColor BaseWaterCenterSeamDiffusionColor =
                FMath::Lerp(Spec.WaterColor, SurfaceGlint, Spec.bDesertCanyon ? 0.16f : 0.12f);
            WaterColor = FMath::Lerp(
                WaterColor,
                BaseWaterCenterSeamDiffusionColor,
                FMath::Clamp(BaseWaterCenterSeamDiffusionT * (Spec.bDesertCanyon ? 0.16f : 0.20f), 0.0f, 0.22f));
            ...
            Vertices.Add(FVector(
                X,
                CenterY + Lateral,
                WaterBaseZ + Wave + FineRippleWave + FlowCuedWaterMottleRippleCm +
                    BaseWaterCrossChannelReliefCm));
    ...
    UStaticMesh* PlaneMesh)
{
    ...
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    const float SquareFoliageSourceCardArtifactDemotion = 0.34f;
    const int32 CardsPerSide = Spec.bDesertCanyon ? 34 : (Spec.bHasWaterfalls ? 54 : 42);
    ...
    const float FoliageCardVisibilityBreakupT = Spec.bHasWaterfalls ? 0.20f : 0.26f;
        ...
        for (int32 CardIndex = 0; CardIndex < CardsPerSide; ++CardIndex)
        {
            ...
            if (bCanopyCard)
            {
                ...
                AddPreviewTranslucentMeshActor(
                    ...
                    FVector(
                        (0.46f + 0.10f * FMath::Abs(FMath::Sin(Phase))) * SquareFoliageSourceCardArtifactDemotion,
                        0.06f,
                        1.0f),
                    CanopyCardColor,
                    0.10f);
            ...
            else
            {
                AddPreviewTranslucentMeshActor(
                    ...
                    FVector(
                        ...
                        (Spec.bDesertCanyon ? 0.28f : (Spec.bHasWaterfalls ? 0.32f : 0.26f)) * 0.42f,
                        1.0f),
                    GroundColor,
                    Spec.bDesertCanyon ? 0.12f : (Spec.bHasWaterfalls ? 0.13f : 0.12f));
            }
        }
    }
}
    ...
    const FLinearColor& ShadowColor)
{
    ...
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    const float CenterGuideRibbonDemotion = FMath::Lerp(
        0.08f,
        1.0f,
        SmoothPreviewStep(0.16f, 0.42f, FMath::Abs(LateralOffset) / FMath::Max(1.0f, ActiveRiverHalfWidth)));
    if (CenterGuideRibbonDemotion <= 0.10f && StartX < 18000.0f)
    {
        return;
    }
    TArray<FVector> Vertices;
...
void AddPreviewWaterShaderDepthReflectionScaffoldDetail(UWorld* World, const FRaftSimEnvironmentPreviewSpec& Spec)
{
    ...
    const float NearCameraWaterScaffoldClearanceMinX = 20000.0f;
    const float CentralWaterScaffoldPlateDemotion = 0.32f;
    ...
    for (int32 XIndex = 0; XIndex <= XSteps; ++XIndex)
    {
        ...
        for (int32 CrossIndex = 0; CrossIndex <= CrossSteps; ++CrossIndex)
        {
            ...
            WaterColor = FMath::Lerp(
                WaterColor,
                Spec.WaterColor,
	                FMath::Clamp(0.72f + 0.12f * (1.0f - CenterFeather) + 0.06f * FMath::Sin(X * 0.0023f + Lateral * 0.0051f), 0.64f, 0.90f));
    ...
    const int32 RefractionSeamCount = Spec.bDesertCanyon ? 2 : (Spec.bHasWaterfalls ? 3 : 3);
    ...
    for (int32 HydraulicIndex = 0; HydraulicIndex < HydraulicCount; ++HydraulicIndex)
    {
        ...
        for (int32 CandidateIndex = 0; CandidateIndex < 5; ++CandidateIndex)
        {
            ...
            const float LaneBias = (HydraulicIndex % 5 == 0)
                ? 0.12f
                : (0.26f + 0.50f * FMath::Abs(FMath::Sin(CandidatePhase * 0.91f)));
    ...
    UStaticMesh* PlaneMesh)
{
    ...
    const int32 GradientBandCount = Spec.bDesertCanyon ? 5 : (Spec.bHasWaterfalls ? 7 : 6);
    for (int32 BandIndex = 0; BandIndex < GradientBandCount; ++BandIndex)
    {
        ...
        const FLinearColor BandColor = FMath::Lerp(HorizonWarmthColor, UpperSkyColor, FMath::Clamp(T * 0.82f + 0.12f, 0.0f, 1.0f));
        AddPreviewTranslucentMeshActor(
            World,
            PlaneMesh,
            ...
            FVector(Spec.bDesertCanyon ? 22.0f : 18.0f, Spec.bHasWaterfalls ? 4.4f : 3.6f, 1.0f),
    ...
    AddPreviewTranslucentMeshActor(
        ...
        FVector(Spec.bDesertCanyon ? 18.0f : 14.0f, Spec.bHasWaterfalls ? 3.2f : 2.8f, 1.0f),
    ...
    const int32 DepthSheetCount = Spec.bDesertCanyon ? 5 : (Spec.bHasWaterfalls ? 7 : 5);
    for (int32 SheetIndex = 0; SheetIndex < DepthSheetCount; ++SheetIndex)
    {
        ...
        AddPreviewTranslucentMeshActor(
            ...
            FVector(Spec.bDesertCanyon ? 16.0f : 12.5f, Spec.bHasWaterfalls ? 3.8f : 3.0f, 1.0f),
            ScalePreviewColor(DepthSheetColor, 0.84f + 0.04f * static_cast<float>(SheetIndex % 4)),
    ...
    UStaticMesh* PlaneMesh)
{
    ...
    const float SquareFoliageCardArtifactDemotion = 0.36f;
    const int32 GrassCardCount = Spec.bDesertCanyon ? 28 : (Spec.bHasWaterfalls ? 58 : 42);
    for (int32 CardIndex = 0; CardIndex < GrassCardCount; ++CardIndex)
    {
        ...
        AddPreviewTranslucentMeshActor(
            ...
            FVector(
                ...
                (Spec.bHasWaterfalls ? 0.18f : 0.13f) * 0.48f,
                1.0f),
            CardColor,
            Spec.bHasWaterfalls ? 0.14f : 0.12f);
    }
    ...
    UStaticMesh* PlaneMesh)
{
    ...
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    const int32 SilhouetteCount = Spec.bDesertCanyon ? 16 : (Spec.bHasWaterfalls ? 36 : 24);
    ...
    const float SquareFoliageCardArtifactDemotion = 0.36f;
    const float FoliageCardVisibilityBreakupT = Spec.bHasWaterfalls ? 0.22f : 0.30f;
    ...
    for (int32 FoliageIndex = 0; FoliageIndex < SilhouetteCount; ++FoliageIndex)
    {
        ...
        if (Spec.bHasWaterfalls && FoliageIndex % 8 == 0)
        {
            ...
            AddPreviewTranslucentMeshActor(
                ...
                FVector(
                    (0.36f + 0.06f * FMath::Abs(FMath::Sin(Phase))) * SquareFoliageCardArtifactDemotion,
                    (1.18f + 0.16f * static_cast<float>(FoliageIndex % 3)) * 0.42f,
                    1.0f),
                VineColor,
                0.13f);
        }
        else if (Spec.bDesertCanyon)
        {
            ...
            AddPreviewTranslucentMeshActor(
                ...
                FVector(
                    (0.38f + 0.05f * static_cast<float>(FoliageIndex % 4)) * SquareFoliageCardArtifactDemotion,
                    0.20f,
                    1.0f),
                ScrubColor,
                0.12f);
    ...
    UStaticMesh* PlaneMesh)
{
    ...
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    const bool bRainforest = Spec.bHasWaterfalls;
    const int32 ClusterCount = Spec.bDesertCanyon ? 44 : (bRainforest ? 116 : 76);
    ...
    const float FirstPartyProceduralCanopyBlobDemotion = Spec.bDesertCanyon ? 0.88f : (bRainforest ? 0.72f : 0.78f);
    const float FoliageCardSilhouetteDemotion = Spec.bDesertCanyon ? 0.90f : (bRainforest ? 0.68f : 0.74f);
    ...
    UStaticMesh* CylinderMesh)
{
    ...
    const bool bRainforest = Spec.bHasWaterfalls;
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    const int32 ClusterCount = Spec.bDesertCanyon ? 34 : (bRainforest ? 84 : 58);
    ...
    const float FirstPartyProceduralCanopyBlobDemotion = Spec.bDesertCanyon ? 0.88f : (bRainforest ? 0.68f : 0.74f);
...
bool BuildPreviewMapForSpec(const FRaftSimEnvironmentPreviewSpec& Spec, FString& OutSummary)
{
    ...
    for (int32 FoliageIndex = 0; FoliageIndex < Spec.FoliageCount; ++FoliageIndex)
    {
        ...
        const float FirstPartyProceduralCanopyBlobDemotion = Spec.bDesertCanyon ? 0.90f : (Spec.bHasWaterfalls ? 0.70f : 0.76f);
        const float FoliageCardSilhouetteDemotion = Spec.bDesertCanyon ? 0.88f : (Spec.bHasWaterfalls ? 0.64f : 0.70f);
