    ...
    for (int32 XIndex = 0; XIndex <= XSteps; ++XIndex)
    {
        ...
        for (int32 YIndex = 0; YIndex <= YSteps; ++YIndex)
        {
            ...
            TerrainColor = ApplyPreviewSourceAwareTerrainPhotoMottle(
                ...
                SourceWaterT,
                SourceVegetationT);
            Vertices.Add(FVector(X, Y, TerrainZ + BaseTerrainMicroReliefCm + SourceAwareTerrainPhotoMottleReliefCm));
        ...
        if (!Spec.bDesertCanyon)
        {
            ...
            AddPreviewFineTwigCanopyLaceActor(
                ...
                FVector(
                    ...
                    TerrainZ + 106.0f * Height + (Spec.bHasWaterfalls ? 32.0f : 18.0f)),
                static_cast<float>((FoliageIndex * 73 + 11) % 360),
                FVector(
                    CanopyWidth * (Spec.bHasWaterfalls ? 0.92f : 0.68f),
                    CanopyWidth * (Spec.bHasWaterfalls ? 0.74f : 0.54f),
                    Height * (Spec.bHasWaterfalls ? 0.42f : 0.28f)),
