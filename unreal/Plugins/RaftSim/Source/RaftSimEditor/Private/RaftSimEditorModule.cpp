...
UMaterialInterface* LoadOrCreatePreviewWaterVertexColorMaterial()
{
    ...
    const float FirstPartyLitWaterNormalResponseEmissiveFill = 0.22f;
    const float FirstPartyLitWaterNormalResponseRoughness = 0.62f;
    const float FirstPartyLitWaterNormalResponseSpecular = 0.32f;
...
struct FRaftSimFirstPartyMaterialTextureAssetSpec
{
    ...
    FString GetTextureAssetPath() const
    {
        ...
        return FString::Printf(
            TEXT("/Game/RaftSim/Rendering/ProceduralTextureAtlases/Textures/%s"),
            *TextureAssetName);
    }
};
...
bool LoadPreviewPngBgraPixels(const FString& RelativePath, int32& OutWidth, int32& OutHeight, TArray<FColor>& OutPixels)
{
    ...
    {
        UE_LOG(LogRaftSimEditor, Warning, TEXT("Failed to load first-party material atlas PNG: %s"), *AbsolutePath);
        return false;
    }
    ...
    {
        UE_LOG(LogRaftSimEditor, Warning, TEXT("Failed to decode first-party material atlas PNG header: %s"), *AbsolutePath);
        return false;
    }
    ...
    {
        UE_LOG(LogRaftSimEditor, Warning, TEXT("Failed to decode first-party material atlas PNG pixels: %s"), *AbsolutePath);
        return false;
    }
    ...
    {
        UE_LOG(LogRaftSimEditor, Warning, TEXT("First-party material atlas PNG dimensions are invalid: %s"), *AbsolutePath);
        OutWidth = 0;
        OutHeight = 0;
        return false;
    }
    ...
    bool& bOutSaved)
{
    ...
    if (!LoadPreviewPngBgraPixels(Spec.SourceRelativePath, Width, Height, Pixels))
    {
        OutSummary += FString::Printf(
            TEXT("Failed to load first-party material texture atlas source %s for %s/%s\n"),
    ...
    {
        OutSummary += FString::Printf(TEXT("Existing atlas asset is not a Texture2D: %s\n"), *ObjectPath);
        return nullptr;
    }
    if (!Texture)
    {
    ...
    const FRaftSimPreviewImage* MaterialAtlasPacked)
{
    ...
    const FLinearColor BaseWaterFlowThreadHighlight = Spec.bDesertCanyon
        ...
        : (Spec.bHasWaterfalls ? FMath::Lerp(Spec.WaterColor, FLinearColor(0.130f, 0.55f, 0.430f), 0.28f)
                                : FMath::Lerp(Spec.WaterColor, FLinearColor(0.140f, 0.58f, 0.580f), 0.25f));
    const FLinearColor BaseWaterFlowThreadFoamGlint = Spec.bDesertCanyon
        ...
        : (Spec.bHasWaterfalls ? FMath::Lerp(Spec.WaterColor, FLinearColor(0.42f, 0.78f, 0.66f), 0.18f)
                                : FMath::Lerp(Spec.WaterColor, FLinearColor(0.42f, 0.80f, 0.79f), 0.16f));
    ...
    const FLinearColor NearCameraWaterMacroRippleHighlight = Spec.bDesertCanyon
        ...
        : (Spec.bHasWaterfalls ? FMath::Lerp(Spec.WaterColor, FLinearColor(0.135f, 0.52f, 0.405f), 0.28f)
                                : FMath::Lerp(Spec.WaterColor, FLinearColor(0.140f, 0.55f, 0.555f), 0.24f));
    const float ActiveRiverHalfWidth = GetPreviewActiveRiverHalfWidthCm(Spec);
    ...
    for (int32 XIndex = 0; XIndex <= XSteps; ++XIndex)
    {
        ...
        for (int32 CrossIndex = 0; CrossIndex <= CrossSteps; ++CrossIndex)
        {
            ...
            WaterColor = FMath::Lerp(
                ...
                FMath::Clamp(BaseWaterResidualCenterSeamEraseT * (Spec.bDesertCanyon ? 0.62f : 0.72f), 0.0f, 0.78f));
            WaterColor = FMath::Lerp(
                WaterColor,
            ...
            const FLinearColor BaseWaterResidualDarkStreakLumaFloor = Spec.bDesertCanyon
                ...
                : (Spec.bHasWaterfalls ? FLinearColor(0.0f, 0.225f, 0.190f)
                                        : FLinearColor(0.0f, 0.275f, 0.290f));
    ...
    UStaticMesh* PlaneMesh)
{
    ...
    const float RemainingAtmosphericCardCull = 0.14f;
    const int32 GradientBandCount = Spec.bDesertCanyon ? 3 : (Spec.bHasWaterfalls ? 3 : 3);
    ...
    const int32 DepthSheetCount = Spec.bDesertCanyon ? 2 : (Spec.bHasWaterfalls ? 3 : 2);
...
AActor* AddPreviewRiverEyeCenterArtifactCover(UWorld* World, const FRaftSimEnvironmentPreviewSpec& Spec)
{
    ...
    const FLinearColor RiverEyeCenterCoverDeepCurrentColor = Spec.bDesertCanyon
        ...
        : FMath::Lerp(
              Spec.WaterColor,
              Spec.bHasWaterfalls ? FLinearColor(0.04f, 0.25f, 0.20f, 1.0f)
                                  : FLinearColor(0.04f, 0.31f, 0.33f, 1.0f),
    ...
    const FLinearColor RiverEyeCenterCoverSkyHighlightColor = Spec.bDesertCanyon
        ...
        : FMath::Lerp(
              Spec.WaterColor,
              Spec.bHasWaterfalls ? FLinearColor(0.30f, 0.50f, 0.44f, 1.0f)
                                  : FLinearColor(0.26f, 0.48f, 0.50f, 1.0f),
              0.11f);
    ...
    const FLinearColor RiverEyeCenterCoverMacroShadowColor = Spec.bDesertCanyon
        ...
        : FMath::Lerp(
              Spec.WaterColor,
              Spec.bHasWaterfalls ? FLinearColor(0.010f, 0.145f, 0.115f, 1.0f)
                                  : FLinearColor(0.014f, 0.190f, 0.205f, 1.0f),
    ...
    const FLinearColor RiverEyeCenterCoverMacroHighlightColor = Spec.bDesertCanyon
        ...
        : FMath::Lerp(
              Spec.WaterColor,
              Spec.bHasWaterfalls ? FLinearColor(0.16f, 0.54f, 0.42f, 1.0f)
                                  : FLinearColor(0.17f, 0.57f, 0.57f, 1.0f),
              0.28f);
    ...
    for (int32 XIndex = 0; XIndex <= XSteps; ++XIndex)
    {
        ...
        for (int32 CrossIndex = 0; CrossIndex <= CrossSteps; ++CrossIndex)
        {
            ...
            CoverColor = ScalePreviewColor(
                CoverColor,
                0.985f + 0.018f * FMath::Sin(X * 0.0180f + Lateral * 0.0100f) +
    ...
    FString& OutSummary)
{
    ...
    AActor* RiverEyeCenterArtifactCoverActor = bHideForegroundRaftProxies
