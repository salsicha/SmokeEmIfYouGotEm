...
FAutoConsoleCommandWithWorldAndArgs StageProbe(TEXT("RaftSim.LiquidRegionalStageProbe"),
    ...
    FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args,UWorld* World)
    {
        if ((Args.Num()==1 || (Args.Num()==2 && Args[1]==TEXT("all"))) && Args[0]==TEXT("start") && !Probe)
        {
            ...
            const auto Boundary=ReadProbeJson(Base/TEXT("south-fork-whole-rapid-liquid-float-seeds-20260910/grid_boundary_profile.json"));
            ...
            if (!Source) return;
            auto State=MakeShared<FStageProbe,ESPMode::ThreadSafe>();State->World=World;
            ...
            // Dense compact histories retain only 304 bytes per commit. Bound
            // the longer replay by the existing 8192 complete stage records;
            // never permit this duration with full dense payload histories.
            const int32 MaxCommits=State->DenseRequested?118:(State->RetirementRequested?10:8);
            ...
            if (State->PacketStep<1 || State->PacketStep>(State->DenseRequested?120:12) || (!State->PacketRequested && State->PacketStep!=1))
            { UE_LOG(LogTemp,Error,TEXT("Packet step exceeds the bounded native replay (120 dense, 12 small)"));return; }
            ...
            if (State->ParentExterior || State->RetirementRequested)
            {
                ...
                if (!RaftSimLiquidParentExterior::FProfile::Build(Parent,All,
                    ReadProbeJson(Base/TEXT("south-fork-whole-rapid-liquid-float-seeds-20260910/grid_vector_boundary_profile.json")),Exterior,Error))
                { UE_LOG(LogTemp,Error,TEXT("Exterior table: %s"),*Error);return; }
                if(State->DenseRequested)
                {
                    const auto Geometry=ReadProbeJson(Base/TEXT("south-fork-liquid-regional-geometry-v4-20260910/manifest.json"));
                    FString MeshHash;FRaftSimLiquidFaceBed Bed;
                    if(!Geometry || !Geometry->TryGetStringField(TEXT("original_mesh_sha256"),MeshHash) ||
                        !RaftSimLiquidFaceBedProfile::Decode(ReadProbeJson(Base/TEXT("south-fork-liquid-face-bed-20260910/physical_face_bed.json")),Parent,
                            Base/TEXT("south-fork-rock-return-xy-candidate-v2-20260907/registered_mesh_source.npz"),
                            Base/TEXT("south-fork-whole-rapid-liquid-float-seeds-20260910/grid_vector_boundary_profile.json"),MeshHash,Bed,Error))
            ...
            for (int32 Id=Args.Num()==2?0:10;Id<12;++Id)
            {
                ...
                if (State->ContactInstalled && !RaftSimInstallRegionalLiquidContact(System,R,
                    ReadProbeJson(Base/TEXT("south-fork-liquid-regional-geometry-v4-20260910")/
                ...
                if (State->ProjectionInstalled && !RaftSimInstallRegionalLiquidProjection(System,Parent,R,Error))
            ...
            ENQUEUE_RENDER_COMMAND(RaftSimStartRegionalStageProbe)([State](FRHICommandListImmediate&)
            {
                if (State->HandoffRequested)
                ...
                State->Handle=URaftSimLiquidStageInterface::PostGroupEvent().AddLambda(
                    [State](FRDGBuilder& Graph,uint32 Ordinal,TConstArrayView<FRaftSimLiquidCompletedStage> Stages)
                    {
                        ...
                        if (State->Groups.Num()>=8192) return;
                        auto J=MakeShared<FJsonObject>();J->SetNumberField(TEXT("render_frame"),GFrameNumberRenderThread);
                        ...
                        J->SetNumberField(TEXT("graph_group"),Ordinal);J->SetBoolField(TEXT("complete"),Full);J->SetBoolField(TEXT("aligned"),Aligned);
                        ...
                        J->SetArrayField(TEXT("entries"),Entries);State->Groups.Add(MakeShared<FJsonValueObject>(J));
                    });
            });
        ...
        else if (Args.Num()==2 && Args[0]==TEXT("stop") && Probe)
        {
            ...
            J->SetBoolField(TEXT("records_truncated"),State->Complete+State->Incomplete+State->Misaligned>uint32(State->Groups.Num()));
