...
The July 8 preview passes remain a historical pixel-gate experiment, not current completion evidence. The material checkpoint fixed the UE 5.8 Metal UV failure, rendered first-party close-range terrain detail, and disabled legacy overlays plus synthetic film grain. The active source-terrain checkpoint now uses bilinear, seam-feathered review heightfields for meter-scale macro terrain and bounded local erosion relief, with every per-river amplitude and normal value recorded in base and flow capture manifests. All 6 base captures still fail edge density and color entropy, 5 fail broad low-gradient coverage, and 2 Colorado views fail luminance variation. All 20 flow captures fail edge density and entropy, 18 fail broad low-gradient coverage, and all 6 Colorado flow captures fail luminance. The synchronized human-review and performance packets remain open, but blocked captures must not be submitted for lifelike approval.
