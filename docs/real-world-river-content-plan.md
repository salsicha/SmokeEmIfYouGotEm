...
The current photoreal environment source plan is `unreal/Content/RaftSim/Rendering/photoreal_river_environment_sources.json`; the production workflow is described in `docs/photoreal-river-environment-production-plan.md`. That plan promotes river maps, seasonal/release/rainfall flow levels, aerial/satellite imagery, rights-reviewed guide and media references, and procedural generation into a single Unreal environment track for South Fork American, Colorado River, and Pacuare. Pacuare now has a retained 1024px NASA GIBS plus Copernicus DEM source-preview seed, a review-gated 4096px Sentinel-augmented active Unreal calibration drape, a 1024px DEM relief preview, production-import placeholder derivatives, a generated Unreal-preview centerline/stationing scaffold, an official service access plan for SNIT/IDECORI plus Direccion de Agua/SINIGIRH source review, an archived SNIT node layer-list summary, selected SNIT per-layer metadata wired into the review manifests, a consolidated source metadata review at `review/production_import_pilot/source_metadata_review.json`, official Landsat/Sentinel high-resolution scene metadata at `imagery/production_import_pilot/high_resolution_scene_metadata_review.json`, a Landsat product access gate at `imagery/production_import_pilot/landsat_product_access_gate_review.json`, and Sentinel COG thumbnail/tile metadata triage at `imagery/production_import_pilot/sentinel_cog_thumbnail_review.json`. The Pacuare reviews record local hashes and compact counts for the DEM, MODIS/GIBS cloud-screen, Landsat/Sentinel candidates and thumbnails, SNIT/Direccion, hydrology, access/protected-area, and link-only media-rights artifacts, but the Sentinel-augmented drape is preview calibration only and the Direccion de Agua/SINIGIRH WMS capabilities metadata, SNIT layer lists, selected per-layer SNIT metadata, and scene metadata remain metadata-only: no official geometry, station records, map tiles, WFS features, station time series, production masks, full-resolution imagery products, or media are imported yet. The environment track now also has importable first-party procedural material texture atlases at `unreal/Content/RaftSim/Rendering/ProceduralTextureAtlases/first_party_material_texture_atlas_manifest.json` for all three rivers; the active Unreal preview samples their albedo tiles into terrain, water, boulder, foliage, raft, and oar vertex colors as bounded review tints, and samples their normal plus packed AO/roughness/height tiles into bounded surface response and tiny terrain/water microrelief. Editor automation now creates nine review-only atlas `Texture2D` assets under `unreal/Content/RaftSim/Rendering/ProceduralTextureAtlases/Textures/` and records their import settings, while `unreal/Content/RaftSim/Rendering/first_party_material_instance_candidates.json` records the 18 review-only `MI_RaftSim_*_AtlasCandidate` Material Instance Constant assets with bound `AlbedoAtlas`, `NormalAtlas`, and `AORoughnessHeightAtlas` texture parameters parented to the review-only `M_RaftSim_AtlasSampleReview` shader graph and assigned to saved preview-map terrain, water, boulder, foliage, and raft proxy surfaces for review. Guide/geospatial review, hazard readability, performance evidence, and production material promotion remain unpromoted gates rather than source imagery or lifelike evidence; `docs/environment-captures/photoreal_river_previews/photoreal_capture_quality_review.json` now records all six current captures as preview-only with `low_color_texture_entropy` blockers, not approved production screenshots. The access gate records that anonymous LandsatLook product asset URLs returned EROS login HTML instead of JPEG/JSON/COG content, and the Sentinel reviews are visual triage/calibration only, so Pacuare remains preview-only until higher-resolution imagery is downloaded only through an approved access path and visual QA, official hydrography, hydrology/rainfall, protected-area/access review, and guide/media review are attached. Social media and outfitter media are reference-only links until explicit reuse rights are recorded. `physics/data/real_world/reference_media_review_queue.json` is the station-aware handoff from link leads to review work: it records visual questions, candidate source IDs, flow context, annotation outputs, and rights status without downloading or promoting third-party media. Each runnable river now also has link-only `review/production_import_pilot/reference_annotations.geojson` and `field_media/production_import_pilot/rights_manifest.json` scaffolds with review-only stationing-derived points so editors can place review targets and track rights before any photo, footage, caption, thumbnail, comment, or audio becomes an asset source.

The July 8 preview passes remain a historical pixel-gate experiment, not current completion evidence. The material checkpoint fixed the UE 5.8 Metal UV failure, rendered first-party close-range terrain detail, and disabled legacy overlays plus synthetic film grain. The source-terrain checkpoint uses bilinear, seam-feathered review heightfields for meter-scale macro terrain and bounded local erosion relief. The active water-light-response checkpoint isolates a dedicated DefaultLit water parent, preserves river-ribbon wave normals, samples tile-safe first-party water normal detail, records bounded per-river response values, and reduces legacy near-camera flattening. All 6 base captures still fail edge density and color entropy, 5 fail broad low-gradient coverage, and 1 Colorado view fails luminance variation. All 20 flow captures fail edge density and entropy, 17 fail broad low-gradient coverage, and 3 Colorado flow captures fail luminance. The synchronized human-review and performance packets remain open, but blocked captures must not be submitted for lifelike approval.

Loaded atlas terrain instances rendered black during the water-parent calibration, so the active maps keep source-baked terrain on the proven lit vertex-color material and retain `M_RaftSim_AtlasSampleReview` as an unpromoted candidate. Water material instances now use `M_RaftSim_VertexColorWaterPreview`; production Landscape/Nanite terrain and WaterBody/custom water parents still require loaded-map visual, guide, geospatial, hazard-readability, and desktop/VR review.
...
The source-conditioned material-map pass now derives 2048px macro albedo, material-zone, packed AO/roughness/height, and DEM/mask/source-luma-derived normal-detail candidates for all three rivers from the active review-gated aerial drapes, DEM relief, water masks, and vegetation masks. The manifest is `unreal/Content/RaftSim/Rendering/SourceConditionedMaterialMaps/first_party_source_conditioned_material_map_manifest.json`, and now records `source_luma_wet_vegetation_microdetail_v2` as the bounded source-detail model: aerial-luma variation is baked into banks, rocks, vegetation, wet edges, roughness, height, and normal-detail candidates while visible-water normal strength stays low so the material pass cannot hide hazards or solver geometry. Editor automation imports those 12 maps as review-only Unreal `Texture2D` assets, binds them into the atlas candidate material instances, exposes low-gain zone, macro-albedo, surface-response, and normal-detail weights so terrain, water-edge, vegetation, wet-bank, and relief-driven response can be tuned per river, and refreshes the capture-quality/human-review handoff after regenerated guide-seat and river-eye frames. They remain unpromoted until external review accepts geospatial alignment, guide fidelity, rights/provenance, hazard readability, art direction, and performance.

The close-range companion layer is `unreal/Content/RaftSim/Rendering/ProductionDetailTextures/first_party_production_detail_texture_manifest.json`. It records prompt/hash provenance and deterministic seamless albedo, tangent normal, and packed AO/roughness/height maps for Sierra granite bank material, Grand Canyon sandstone/limestone/silt, and Pacuare volcanic gravel/humid rainforest soil. Unreal imports nine Texture2D assets and applies bounded terrain-only weights on separate tiled UVs. This layer solves material sampling and scale detail; it does not replace source-aligned Landscape/Nanite geometry, bank/bed transitions, rocks, foliage, water shading, foam/mist, lighting, or human review.

Run `python -m raftsim.examples.sync_photoreal_environment_review_rollups` after regenerating quality, human-review, and performance artifacts. The synchronizer updates the source plan, procedural asset plan, art research, and production gap register from authoritative review JSON so a renderer change cannot leave stale blocker counts in the roadmap.
...
- Current South Fork source slice: `physics/data/real_world/south_fork_american_chili_bar/production_source_pull_manifest.json` records historical USGS `11445500` daily discharge, CDEC CBR/A25 flow context, a review-gated flow-band comparison at `hydrology/production_import_pilot/flow_band_review.json`, access/publication source leads at `review/production_import_pilot/access_publication_sensitivity_review.json`, TNM NHD product metadata, a review-gated NHD HU8 `18020129` bbox source extract, a derived South Fork mainstem candidate with preview metric stationing and cross-section seed lines, generated production-import draft hydrography overlays at `hydrography/production_import_pilot/centerline.geojson`, `banks.geojson`, and `cross_sections.geojson`, a first NAIP/DEM preview-mask alignment diagnostic, TNM DEM/NAIP zero-hit diagnostics, retained smoke-test USGS/USDA samples, and larger active official USGS 3DEP/USDA NAIP corridor samples. The flow-band review compares the current low/median/high planning presets to the attached 30-day CBR window, recording 25 local days peaking at or above 900 cfs, 9 days peaking at or above 1600 cfs, and 0 days reaching 3000 cfs; the multi-year CBR review now extends seasonal context, but A25 X-flag/routing review, high-flow interpretation, legal signoff, and guide validation still block preset promotion. The access/publication review records California State Parks Marshall Gold Discovery evidence, an El Dorado County GIS lead, three station review zones, required access/sensitive-location/evacuation annotations, and a rejected BLM legacy URL; `review/production_import_pilot/access_points.geojson`, `no_publish_sensitive_polygons.geojson`, and `evacuation_and_rescue_routes.geojson` now provide stationing-derived editor seeds, but exact access geometry, parcel/public-private land status, current restrictions, sensitive-location screening, emergency-route authority, and guide/local approval still block detailed map or screenshot publication. The Unreal preview automation now samples the larger NAIP export into a terrain-conforming source-drape mosaic, uses the larger derived 3DEP relief preview for preview terrain, and records `terrain/usgs_3dep_chili_bar_corridor_heightfield_1009.png` plus `unreal/Content/RaftSim/River/south_fork_heightfield_import_test.json` for review-gated Landscape import testing. The NHD extract supplies official flowline/waterbody context, the mainstem candidate supplies a single review overlay, and stationing/cross-section seed files now supply production-import draft edit scaffolding, but the diagnostic confirms current preview masks are not acceptance evidence; production CRS review, direction confirmation, bank/cross-section work, stronger NAIP/DEM alignment, and guide validation remain open before replacing seed hydrography.
- Current South Fork source-gate decision: `physics/data/real_world/south_fork_american_chili_bar/production_source_gate_review.json` summarizes DEM/lidar, imagery/masks, hydrography, seasonal flow, access/protected context, and guide/media annotations as attached review-gated preview evidence. It permits another renderer iteration from the current sources, but blocks production terrain, production water, and lifelike status until geospatial, guide, rights, A25 routing, high-flow interpretation, and seasonal-flow review pass.
- Current South Fork source metadata review: `review/production_import_pilot/source_metadata_review.json` records compact official metadata for the pilot 3DEP/NAIP source envelope without committing raw ArcGIS payloads. The review attaches hashes for USGS 3DEP and USDA/APFO NAIP service/query responses, confirms 12 NAIP primary items acquired July 20-21, 2022, records 12 primary 3DEP item records spanning 2012-03-04 through 2023-10-13 with NAVD 88 vertical-datum evidence, and keeps production promotion blocked on full-corridor tiling, source selection, terms/attribution, CRS/vertical-datum acceptance, conditioning, channel burning, masks, and guide/geospatial approval.
- Current South Fork seasonal-flow expansion: `hydrology/production_import_pilot/cdec_seasonal_window_review_wy2026_to_2026-07-06.json` summarizes CDEC CBR flow/stage and A25 daily powerhouse context from October 1, 2025 through July 6, 2026, with raw-response hashes and compact daily/monthly summaries rather than raw API payloads. It records 47 nonphysical negative CBR flow rows normalized to null, 245 days peaking at or above 900 cfs, 182 at or above 1600 cfs, and 48 at or above 3000 cfs, while keeping presets, visual forcing, high-flow behavior, and lifelike water blocked on A25 routing/flag review, legal signoff, station-to-rapid relation, high-flow interpretation, and guide validation.
- Current South Fork multi-year CBR flow context: `hydrology/production_import_pilot/cdec_multiyear_flow_review_2021-10-01_2026-07-06.json` summarizes CDEC CBR event-flow data from October 1, 2021 through July 6, 2026, covering four complete water years plus partial water year 2026. It records 164,992 valid samples, 144 nonphysical negative flow rows normalized to null, 1,439 local days peaking at or above 900 cfs, 1,076 at or above 1600 cfs, and 364 at or above 3000 cfs. The artifact preserves 92 official samples at or above 20,000 cfs as high-flow/flood context requiring hydrology and guide review; presets, visual forcing, hole stickiness/washout behavior, unsafe-flow variants, and lifelike water remain blocked on A25 routing, high-flow interpretation, legal signoff, station-to-rapid relation, and guide/outfitter validation.
...
- Current Colorado source metadata review: `review/production_import_pilot/source_metadata_review.json` records compact official metadata for the Lees Ferry pilot 3DEP/NAIP source envelope without committing raw ArcGIS payloads. The review attaches hashes for USGS 3DEP and USDA/APFO NAIP service/query responses, confirms 9 NAIP primary items acquired October 15, 2021, records 9 primary 3DEP item records spanning November 10, 2022 through September 16, 2023 with NAVD 88 vertical-datum evidence, and keeps production promotion blocked on full-canyon tiling, river-mile stationing, source selection, terms/attribution, CRS/vertical-datum acceptance, conditioning, channel burning, release-aware masks, and guide/oarsman/geospatial approval.
- Current Pacuare source metadata review: `review/production_import_pilot/source_metadata_review.json` consolidates the local review-gated Pacuare source stack without promoting any raw media, station data, feature geometry, or production imagery. It records Copernicus DEM heightfield candidate sizes and elevation range, the five-date NASA GIBS/MODIS cloud-screened scene index with April 2, 2025 retained as the active preview date, 17 SNIT candidate layers, 11 selected SNIT metadata layers, 52 Direccion de Agua/SINIGIRH WMS named layers, rainfall/discharge/flash-response candidate counts, protected-area/access policy status, and zero media items cleared for asset use. Terrain, imagery, masks, official hydrography, numeric flow bands, route/publication details, and media-derived art remain blocked.
- Current Pacuare high-resolution imagery scene review: `imagery/production_import_pilot/high_resolution_scene_metadata_review.json` records official USGS LandsatLook STAC and Copernicus Data Space OData request URLs and hashes for the Pacuare planning bounds. The USGS cloud-filtered query found three 2025 Landsat Collection 2 Level-2 surface-reflectance candidates below 20% catalog cloud cover, and the Copernicus expanded page found low-cloud Sentinel-2 candidates on January 19, 2025, including an L2A tile at 4.785432% catalog cloud cover. No imagery products are downloaded or promoted; full-year paging, download approval, CRS/band/scale review, manual cloud/shadow QA, mask generation, and guide/geospatial approval remain open.
- Current Pacuare Landsat access gate: `imagery/production_import_pilot/landsat_product_access_gate_review.json` records anonymous requests for the best Landsat candidate's reduced-resolution browse JPEG and MTL JSON URL. Both returned HTTP 200 `text/html` EROS login pages rather than expected JPEG/JSON data, so the bad download was removed and no Landsat product is attached. Use an approved USGS/EROS/API path, reviewed public COG mirror, or another approved source before source-drape or mask replacement.
- Current Pacuare Sentinel COG thumbnail triage: `imagery/production_import_pilot/sentinel_cog_thumbnail_review.json` records public January 19, 2025 Sentinel-2 L2A thumbnail and tile metadata for MGRS tiles `17PKM` and `16PHR`, including thumbnail hashes, 343px dimensions, tile CRS, data coverage, and catalog cloud metrics. These thumbnails can inform source selection and editor discussion, but full-resolution COG reads, SCL/QA masking, corridor clipping, terms/attribution review, source-drape replacement, mask authority, and photoreal material promotion remain blocked.
- Current Colorado overlay addition: `hydrography/production_import_pilot/river_mile_markers.geojson` and `hydrography/production_import_pilot/sandbars.geojson` now seed preview river-mile markers and release-sensitive sandbar polygons from the NHD stationing candidate. These close the production-import overlay placeholder while remaining non-authoritative until official Grand Canyon river-mile stationing, release-aware water-edge/sandbar review, camp/beach publication sensitivity, and oarsman/geospatial approval pass.
- Current Colorado access/publication addition: `review/production_import_pilot/access_points.geojson`, `no_publish_sensitive_polygons.geojson`, `camps_and_beaches_review.geojson`, and `oarsman_route_publication_notes.json` now seed launch/context, no-publish, camp/beach, and oarsman note review overlays from the NPS access review and Lees Ferry stationing. Diamond Creek remains a null-geometry placeholder outside the pilot slice, and all outputs stay non-authoritative until official access/camp/beach/sensitive-resource review and oarsman approval pass.
- Current Pacuare source slice: `physics/data/real_world/pacuare_river_costa_rica/production_source_pull_manifest.json` records Copernicus DEM GLO-30 tiles, NASA GIBS/Copernicus preview drapes and masks, a retained MODIS/GIBS cloud-screened scene index at `imagery/production_import_pilot/cloud_screened_scene_index.json`, a cloud/shadow diagnostic at `imagery/production_import_pilot/cloud_shadow_review.json`, a high-resolution Landsat/Sentinel scene metadata review at `imagery/production_import_pilot/high_resolution_scene_metadata_review.json`, a Landsat product access-gate review at `imagery/production_import_pilot/landsat_product_access_gate_review.json`, a Sentinel COG thumbnail/tile triage review at `imagery/production_import_pilot/sentinel_cog_thumbnail_review.json`, the review-only Sentinel-augmented Unreal calibration drape at `imagery/production_import_pilot/sentinel_augmented_source_drape_preview_4096.png`, production-import placeholder derivatives, Costa Rica hydrology/rainfall/protected-area source leads, archived Direccion de Agua/SINIGIRH WMS capabilities metadata, archived SNIT node layer-list metadata, selected SNIT per-layer metadata, rainfall/station review at `hydrology/production_import_pilot/rainfall_station_review.json`, discharge/stage station review at `hydrology/production_import_pilot/discharge_or_stage_station_review.json`, flash-response review at `hydrology/production_import_pilot/flash_response_review.json`, the consolidated source metadata review at `review/production_import_pilot/source_metadata_review.json`, and now `hydrography/production_import_pilot/preview_centerline_scaffold.geojson` plus `preview_stationing_scaffold.json`. The active Unreal Pacuare preview uses the Sentinel-augmented drape for color/detail calibration only; it is not a production source-drape replacement, mask source, material source, route imagery claim, or lifelike evidence. The scene index keeps April 2, 2025 as the base preview seed and records March 8, 2025 as the lowest simple cloud-like metric, but imagery promotion remains blocked until an approved Landsat/Sentinel/local/first-party source is exact-route clipped, SCL/QA-masked, stitched, terms-reviewed, and guide/geospatial reviewed. The SNIT summaries identify metadata-only candidate layers for Direccion de Agua aforos/hydrologic units, IMN rainfall climatology, SENARA rain stations/recharge, SINAC protected-area/forest/wetland context, and selected CRS/terms review; no features, map tiles, station time series, masks, or geometry have been imported. The rainfall/station review summarizes three IMN precipitation layers, two SENARA station layers, and one SENARA recharge context layer; the discharge/stage review records `DA_AFOROS`, `JASEC_ESTACIONES_HIDROMETRICAS`, and `DA_UNIDADES_HIDROLOGICAS` candidates; and the flash-response review links those leads to basin/recharge context while keeping rainfall-to-stage lag, unsafe-flow criteria, rescue readability, evacuation constraints, and guide/outfitter validation blocked. `review/production_import_pilot/protected_area_publication_sensitivity.json` and `review/production_import_pilot/access_and_conservation_policy.json` now convert SINAC/SNIT/SENARA metadata and access constraints into no-publish annotation, flow-dependent access review, and guide/conservation signoff gates; they still import no protected-area, access, evacuation, rescue, or sensitive-location geometry. The scaffold is generated from the deterministic Unreal Pacuare preview curve with 129 vertices and 184 station samples for annotation placement and procedural rainforest dressing only; it is not official hydrography, banks, wetted width, access stationing, or solver geometry.
- Current Pacuare hydrography overlay addition: `hydrography/production_import_pilot/centerline.geojson`, `banks.geojson`, and `rapid_and_access_stationing.geojson` now package the preview scaffold into production-import review layers for editor and guide/outfitter annotation. They close the current file placeholders but remain non-authoritative until official Costa Rica hydrography, working CRS, bank/wetted-width geometry, rainfall/flow context, protected-area review, and guide validation pass.
...
The detailed implementation and validation sequence is now `docs/named-rapid-realism-validation-plan.md`. The source catalog records 85 review markers across six runnable river environments. Generated Unreal manifests expose those markers, 50 non-authoritative candidate map points, and 453 low/reference/high-flow simulator run definitions in the Rapid/River Editor. Published South Fork miles are retained, Colorado miles remain cross-check gated, and Pacuare/Zambezi/Futaleufu/Chilko markers are visibly provisional until GPS/aerial/guide stationing replaces them. Chilko contributes Bidwell Rapids, Lava Canyon, White Mile, Green Mile, and Miracle Canyon leads. Zambezi's complete physical corridor and procedural water seed make it a reference Free Run, but do not validate its rapid-specific geometry, hydraulic outcomes, or guide lines. No rapid review run is considered executed or production-validated until exact geometry, an accepted C++ water window, and a guide-reviewed line are bound and telemetry/video evidence passes.
