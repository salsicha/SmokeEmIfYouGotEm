...
The July 8 preview capture passes removed the automated `large_flat_blue_field`, `dark_foreground_proxy`, `excess_low_gradient_area`, and `low_luma_variation` blocker classes from all six current South Fork, Colorado, and Pacuare guide-seat/river-eye captures by retuning generated water color, fog/sky contribution, capture composition, raft review materials, near-field water texture, foam-lace, wet-rock, and bank-drape dressing. The same quality review still blocks lifelike status for every capture because low-color-texture-entropy proxy fields remain, with all three river-eye frames also failing edge-density checks.
