...
The latest preview capture passes removed the dark foreground obstruction, `large_flat_blue_field`, `dark_foreground_proxy`, `excess_low_gradient_area`, `low_luma_variation`, and `low_edge_density` blocker classes from the South Fork, Colorado, and Pacuare guide-seat/river-eye captures by retuning generated water color, fog/sky contribution, capture composition, raft review materials, near-field water texture, foam-lace, wet-rock, bank-drape dressing, source-mask-biased near-field riverbed pebble/debris dressing, source-mask-biased shoreline lip/overhang edge breakup, first-party source-aware terrain surface granularity, first-party photographic color-grade/palette compression with bounded film-grain dither, bank/water texture flecks, smooth current-thread color diversity, Pacuare rainforest reflection bands, river-eye current contrast, and scene-capture post-processing. The current quality review still records `low_color_texture_entropy` blockers, and the source-aware check also blocks visibly faceted base-water mesh undulation unless the renderer records the smoothed near-field water-surface faceting demotion, so source-conditioned captures cannot pass on pixel metrics while still reading as simplified preview water. `docs/environment-captures/photoreal_river_previews/photoreal_human_lifelike_review_handoff.json` records the per-river capture hashes, source inputs, flow context, link-only reference-media review targets, current automated blockers, and seven open review domains; `docs/environment-captures/photoreal_river_previews/photoreal_human_lifelike_review_packet.md` is the human-readable review packet with embedded captures, source/flow context, reviewer prompts, signoff checkboxes, and the promotion rule; `docs/environment-captures/photoreal_river_previews/photoreal_human_lifelike_review_results_template.json` is the machine-readable form where external reviewers record identity, evidence, blockers, follow-up actions, and final decisions; and `docs/environment-captures/photoreal_river_previews/photoreal_environment_performance_review.json` records static desktop/VR performance evidence inventory while keeping measured profile approvals open. Source/geospatial, guide, rights, hazard-readability, production material/asset, art-direction, and performance gates still block any lifelike claim.
