...
Out_OutPosition = In_Position;
	...
	if (abs(SDFValue) <  In_FoamThickness)
	{
	...
	if (length(Out_OutVelocity) < 1e-5 || 
...
void ParticleState_Emitter_Func(inout FSimulationContext Context)
{
	...
	float Result47 = max(Context.MapUpdate.ParticleState.Lifetime, Constant107);
	float Constant108 = 0.0001;
	float Result48 = Result47 - Constant108;
...
// Emit
	...
	int2 NumCells;
	GetNumCells_Emitter_EmissionPositions(NumCells.x, NumCells.y);
	if (NumEmitted > 0 && In_SDFValue > In_SDFMinValue && In_SDFValue < In_SDFMaxValue)
	{
	  float PrevVal = 0;
...
void Grid3D_Secondary_EmissionPoints_Emitter_Func(inout FSimulationContext Context)
{
	...
	float Constant131 = 1;
	float Constant132 = 0;
	float2 Constant133 = float2(0,1e+06);
	float Constant134 = -1e+06;
	...
	float Result51 = length(CustomHlslADE88DC393864046574424524848D3E7AACA6FE3Emitter_SimGridReader_FuncOutput_curl);
...
void SimulateMapUpdate(inout FSimulationContext Context)
{
	...
	Context.MapUpdate.DynamicMaterialParameters.Param3WriteEnabled = Constant103;
...
void SimulateMapSimStage1_FindSecondaryEmissionPoints(inout FSimulationContext Context)
{
	...
	float Constant118 = 1;
	float Constant119 = 0;
	float2 Constant120 = float2(0,1e+06);
	float Constant121 = -1e+06;
	float Constant122 = 0.0001;
	float Constant123 = 0;
	float Constant124 = 0;
	float Constant125 = 0;
		...
		void SamplePreviousGridVector3Value_Emitter_SimGridReader_AttributeVelocity(float3 Unit, out float3 Value)
{
	Value = Emitter_SimGridReader_Grid.SampleLevel(Emitter_SimGridReader_GridSampler, Unit, 0).rgb;
		...
		void GetGridValue_Emitter_SimGridReader(int In_IndexX, int In_IndexY, int In_IndexZ, int In_AttributeIndex, out float Out_Val)
{
    Out_Val = 0;
