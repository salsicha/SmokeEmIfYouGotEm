...
#include "RaftSimLiquidStratifiedSource.h"
...
bool InstallArrays(UNiagaraSystem* System,const FState& State,FString& Error)
{
    ...
    if(FParse::Param(FCommandLine::Get(),TEXT("RaftSimRegionalStratifiedSource")))
    {
        int32 SourceSeed=0;FParse::Value(FCommandLine::Get(),TEXT("RaftSimRegionalSourceSeed="),SourceSeed);
        if(!RaftSimInstallLiquidStratifiedSource(System,Graphs,State.SourceWeights,uint32(SourceSeed)+uint32(State.Id)*7919u,Error)) return false;
    }
    Error.Reset();return true;
}
}
