...
The renderer disables legacy terrain and water overlay proxy geometry plus synthetic film-grain dither. The next source-terrain pass samples each review-gated 2017px DEM derivative bilinearly, feathers stitched-tile center seams, separates meter-scale macro relief from bounded multi-scale erosion residuals, and uses river-specific normal flattening. This makes South Fork banks, Grand Canyon walls, and the Pacuare gorge visibly source-shaped while preserving the analytic channel until conditioning and channel burning are approved. The current base review still reports edge and entropy failures on all six captures, broad low-gradient failures on five, and luminance failures on two Colorado views. All 20 flow captures fail edge density and entropy, 18 fail broad low-gradient coverage, and all 6 Colorado flow captures fail luminance.
...
`physics/src/raftsim/photoreal_review_rollups.py` synchronizes each recapture's quality summaries, handoff counts, performance-review counts, production-detail provenance, and source-terrain checkpoint into the source plan, procedural asset plan, art research, and gap register. The historical July 8 pixel checkpoint and the production-detail material checkpoint are explicitly superseded by the active source-terrain geometry checkpoint.
