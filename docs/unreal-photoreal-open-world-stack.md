...
The first native source-terrain candidates now live under `/Game/RaftSim/Maps/EnvironmentPreviews/LandscapeCandidates/`. `RaftSim.CreateLandscapeImportCandidateMaps` imports the three review-gated 1009x1009 16-bit PNG heightfields through Unreal's `LandscapeEditor` format API as 16x16-component `ALandscape` maps, builds four Nanite representations per river, audits all 256 source component and Nanite material bindings, and captures guide-seat plus river-eye evidence under `docs/environment-captures/photoreal_river_previews/landscape_candidates/`. Each candidate material now combines three source-conditioned macro Texture2D assets with three independent first-party close-range terrain Texture2D assets for albedo, normals, ambient occlusion, and roughness, using river-specific Landscape mapping and blend settings recorded in the generated manifest. The offscreen renderer uses a first render to request Landscape shader permutations, waits for compilation, recreates Landscape component state, and then records the evidence frame; this prevents UE 5.8's default grid from appearing in headless captures while material shaders are incomplete. The candidates remain isolated and review-gated because their analytic preview channel burn is not solver or accepted geospatial geometry, and visual inspection still shows sparse blockout terrain, coarse banks, review water, and no production biome dressing.
