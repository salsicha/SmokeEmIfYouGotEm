...
The same pass disables legacy terrain and water overlay proxy geometry plus synthetic film-grain dither. This removes metric-inflating cards, rails, center streaks, and noise while leaving integrated base terrain and water meshes visible. The current base review therefore reports the real blockout gaps: all six fail edge density, entropy, and broad low-gradient coverage, and two Colorado views fail luminance. All 20 flow captures fail edge density and entropy, 19 fail broad low-gradient coverage, and all 6 Colorado flow captures fail luminance. `photoreal_human_lifelike_review_handoff.json`, the Markdown packet, and the desktop/VR performance scaffolds remain synchronized but unapproved; they are not an invitation to review blocked screenshots as lifelike.
...
`physics/src/raftsim/photoreal_review_rollups.py` now synchronizes each recapture's quality summaries, handoff counts, performance-review counts, and production-detail provenance into the source plan, procedural asset plan, art research, and gap register. The historical July 8 zero-metric checkpoint remains explicitly superseded by the July 9 production-detail checkpoint.
