...
The first generated preview maps are now available under `/Game/RaftSim/Maps/EnvironmentPreviews/`, and rendered guide-seat captures are written to `docs/environment-captures/photoreal_river_previews/` by the `RaftSim.CreatePhotorealEnvironmentPreviewMaps` and `RaftSim.CapturePhotorealEnvironmentPreviews` editor automation. South Fork, Colorado, and Pacuare now have source-draped preview overlays, source-drape terrain vertex-color blending, generated valley terrain, curved river ribbons, foam/hydraulic cue strips, first-party translucent water glints/foam flecks/haze/spray-mist layers, boulder bars, clustered foliage proxies, and per-river light/fog variants; the automation also scopes UE 5.8 startup-world GC leak checks to nonfatal during generated-map/capture batches so evidence capture can complete while diagnostics remain logged. The latest July 8 pass disables the former visible tapered water-chroma overlay and separate water fleck cards, moves bounded chroma/noise/relief and fleck cues into integrated water meshes, adds a source-aware faceted-water blocker, smooths/demotes near-field cellular water relief in the guide-seat capture layer, adds smooth current-thread color diversity and Pacuare rainforest reflection bands, and applies capture post-processing directly to the scene-capture component. The stricter current review records zero automated blockers across all six guide-seat/river-eye captures, so the screenshots are candidates for human lifelike review. `docs/environment-captures/photoreal_river_previews/photoreal_human_lifelike_review_handoff.json` packages those candidates for seven-domain human review, `docs/environment-captures/photoreal_river_previews/photoreal_human_lifelike_review_packet.md` is the reviewer-facing Markdown packet with embedded images, source/flow context, metrics, review prompts, and signoff checkboxes, and `docs/environment-captures/photoreal_river_previews/photoreal_human_lifelike_review_results_template.json` is the canonical place to record reviewer evidence, blockers, follow-up actions, and final decisions. Final approval still requires replacing smooth procedural terrain, placeholder rocks/foliage, review-only material instances, and missing production foam/mist/lighting with full reviewed DEM/aerial imports plus rights-cleared or first-party production terrain, water, rock, foliage, foam/mist, lighting, guide/geospatial/hazard review, and performance evidence.

The first source-conditioned material replacement candidates now live under `unreal/Content/RaftSim/Rendering/SourceConditionedMaterialMaps/`. The manifest records 2048px macro albedo, material-zone, and packed AO/roughness/height maps derived from the active review-gated aerial drapes, DEM relief, water masks, and vegetation masks for all three rivers. Editor automation now imports them as review-only `Texture2D` assets, binds them to the atlas candidate material instances, exposes low-gain zone/macro-albedo/surface-response controls, regenerates the guide-seat and river-eye captures, and refreshes the automated/human-review handoff with the new hashes. The next rendering step is human art/guide/geospatial review and true production material promotion before any lifelike claim.
