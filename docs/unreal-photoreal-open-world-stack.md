...
The first source-conditioned material replacement candidates now live under `unreal/Content/RaftSim/Rendering/SourceConditionedMaterialMaps/`. The manifest records 2048px macro albedo, material-zone, and packed AO/roughness/height maps derived from the active review-gated aerial drapes, DEM relief, water masks, and vegetation masks for all three rivers. The next Unreal rendering step is to assign these maps to terrain, water-edge, vegetation, wet-bank, and relief material instances, regenerate captures, and compare them through the human review packet before any lifelike claim.
