...
- Dockable Slate tabs: replace launch stubs with editor panels for every registered tool surface and expose shared validation buttons in each tab.
...
One-click validation actions are source-controlled in `unreal/Content/RaftSim/Tools/tool_validation_actions.json` and exposed through `URaftSimToolValidationActionRegistry` plus `URaftSimToolValidationActionRunner`. These actions cover source checks, deterministic export, solver package regeneration, stitched-window validation, live-water smoke, round-trip validation, and report opening. The dockable Slate tabs now call the same execution path: required evidence is checked, action records are written in the project saved directory, report actions open the linked report, and live-water smoke dispatches the Unreal automation command.

Reviewed Unreal DataAssets are generated under `/Game/RaftSim/Tools/Reviewed` by `RaftSim.CreateReviewedDataAssets`, covering the tool registry, replay/debug viewer, rapid/river editor shell, feature tuning editor shell, and validation actions. Screenshot evidence for all five tabs is generated by `RaftSim.CaptureToolEvidence` under `docs/tool-captures/milestone25a/`.

The Milestone 25A tooling gate is recorded in `unreal/Content/RaftSim/Automation/polished_tooling_slice_gate.json` with reports in `physics/reports/milestone25a/`. `docs/raftsim-tools-workflow.md` is the user-facing workflow note for opening the tools, reviewing sample data, using validation actions, generating reviewed DataAssets, and capturing evidence. The gate now closes the first interactive tooling layer; the next polish layer is feature-specific editing widgets, rendered map/replay overlays, deterministic export implementations, and native automated video capture.
