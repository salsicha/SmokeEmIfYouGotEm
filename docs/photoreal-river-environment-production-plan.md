...
- The South Fork preview now uses the USDA/APFO NAIP sample as a denser terrain-conforming source-drape mosaic and `terrain/usgs_3dep_chili_bar_relief_preview_512.png` as preview-only bank/valley terrain relief in Unreal. This proves imagery and relief plumbing, but the inputs are small smoke-test rasters rather than complete orthomosaic, conditioned DEM, or terrain material passes.
...
- The preview generator now keeps source provenance out of the rendered guide-seat frame and records it in JSON instead. It also uses a lit preview material, computed procedural mesh normals, denser terrain-conforming source-drape mosaics, PCG sample foliage meshes where available, denser low-bank understory/scrub placeholders, and curved procedural foam/wave/eddy-line ribbons. This improves depth and review readability but is still not production photoreal vegetation, rock, water, or terrain material work.
...
- South Fork Unreal source layer: `physics/data/real_world/south_fork_american_chili_bar/imagery/usda_naip_chili_bar_sample_512.png` is sampled into a denser terrain-conforming source-drape mosaic for the generated preview map, `physics/data/real_world/south_fork_american_chili_bar/terrain/usgs_3dep_chili_bar_relief_preview_512.png` is sampled into preview terrain geometry, and `physics/data/real_world/south_fork_american_chili_bar/terrain/usgs_3dep_chili_bar_sample_256.tif` is recorded for the next heightfield-conditioning pass.
...
- Colorado source data: `physics/data/real_world/colorado_river_grand_canyon_rowing/production_source_pull_manifest.json` records the initial official pull; `imagery/usda_naip_lees_ferry_sample_512.png` is sampled into a denser terrain-conforming canyon source-drape mosaic, `terrain/usgs_3dep_lees_ferry_relief_preview_512.png` is sampled into preview canyon terrain geometry, and `terrain/usgs_3dep_lees_ferry_sample_256.tif` is recorded for the next canyon heightfield-conditioning pass.
