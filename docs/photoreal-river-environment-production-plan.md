...
- Pacuare source data: `physics/data/real_world/pacuare_river_costa_rica/production_source_pull_manifest.json` records the initial public/global pull plus the larger active preview source path; `imagery/pacuare_nasa_gibs_2025-04-02_demshade_source_drape_1024.png` is sampled into a denser terrain-conforming rainforest source-drape mosaic, `terrain/pacuare_dem_relief_preview_1024.png` is sampled into preview terrain geometry, `terrain/pacuare_copernicus_dem_corridor_heightfield_1009.png` is recorded for Unreal Landscape import tests, and `terrain/copernicus_dem_glo30_N09_W084.tif` plus `terrain/copernicus_dem_glo30_N10_W084.tif` are recorded for the next gorge heightfield-conditioning pass.
- These captures verify editor automation, guide-seat and raft-hidden river-eye camera paths, generated valley terrain meshes, terrain-conforming source drapes, lit and vertex-color preview materials, deterministic wet-bank/shoreline dressing, curved river ribbons, subtle current-streak proxies, procedural foam/wave/eddy-line ribbons, boulder bars, PCG sample foliage and bank understory/scrub placeholders, and per-river light/fog variants. They must be replaced by source-derived or rights-cleared photoreal passes before the goal is complete.
