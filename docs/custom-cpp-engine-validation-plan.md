...
Treat each blocked geometry family as its own acceptance lane. Wet/dry fixes must show bounded shoreline movement, dry-cell velocity masking, and mass drift. Bed-step strict parity is the finite-volume lane unless a future milestone explicitly redesigns reduced discontinuous-bed dynamics; reduced bed-step reports are still useful diagnostics, but they are not readiness blockers after the finite-volume promotion. Constriction fixes must preserve flux, velocity acceleration, and Froude-class transitions. Drop/ledge/tailwater fixes must show expected energy loss, hydraulic control, and downstream recovery. Cascading reach/drop fixes must compare reach-local outputs against stitched whole-window outputs so seams cannot hide errors.
