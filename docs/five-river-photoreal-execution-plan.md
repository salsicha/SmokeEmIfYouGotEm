...
- [ ] A1 South Fork American stationing and full-reach corridor repair.
  ...
  - [x] Diagnose the full-reach NHD route graph before promotion: `hydrography/full_reach_nhd_route_graph_diagnostic.json` records one connected 198-edge/199-node candidate pool, proves the current Coloma access seed lands at 5,250.419 m instead of the published 9,012.326 m checkpoint, records the missing Folsom anchor, and blocks editor/solver binding until anchor repair plus directed route clipping pass.
