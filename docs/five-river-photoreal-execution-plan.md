...
- [ ] A1 South Fork American stationing and full-reach corridor repair.
  ...
  - [x] Generate directed NHD station candidates for review: `review/full_reach_directed_station_candidates.json` records a unique 184-edge downstream NHD chain from Chili Bar with no branches or merges, projects the Coloma 5.6-mile checkpoint, the 20.5-21.0-mile downstream source window, and all 20 named rapids onto that chain as review-only points, and blocks editor geometry, solver windows, and catalog restationing until exact anchors and guide/geospatial review approve them.
