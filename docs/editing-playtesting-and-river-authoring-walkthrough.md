...
| Pacuare River | `unreal/Content/RaftSim/River/pacuare_river_third_target_editor_pass.json` | Rain-fed guided paddle-raft planning route with a review-gated preview centerline/stationing scaffold, archived SNIT/Direccion de Agua metadata leads, and selected SNIT per-layer terms/CRS metadata for annotations/source review only. Needs imported official hydrography, hydrology/rainfall, protected-area/source-rights, field-media, and rainforest/gorge fidelity review before solver generation. |
