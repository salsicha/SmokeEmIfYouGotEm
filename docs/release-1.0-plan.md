...
### 2026-07-18 — P2 slice three: A-3 flexible-raft D1–D4 in C++; D6 measured pairs closed
...
- **D6 closed with UE-measured results**: `RaftSim.D6.UEMeasuredExport` runs all 7 committed fixtures through the C++ path twice (compliant + rigid baseline) from the fixture input package, writes SHA-256-hashed telemetry plus both measured-results sidecars (`physics/reports/d6/ue/`), and asserts parity against the committed Python metrics at 1e-9 relative. Physics-side merge + comparison: **14/14 measured pairs recorded, all 7 fixtures `passed_numeric_equivalence` on the compliant target (worst |Δ| 2.3e-13), baseline deltas recorded** — `flexible_raft_d6_ue_comparison_report.json` status `measured_comparisons_passed_manual_promotion_required`. Chrono pairs remain optional/non-blocking per §6.
...
- `ARaftSimWaterSurfaceActor` (RaftSimRaft, ProceduralMeshComponent) renders the live solver free surface: a grid mesh whose vertices track the water adapter's surface height + normal each refresh, with Froude-driven foam in vertex colour, using the single-layer water material. Spawned automatically alongside the raft. `RaftSim.P2.WaterSurfaceRenders` passes; all four P2 automation tests green (surface, flip/recover, river window, tank float). **Phase 2 exit gate met**: raft on authoritative dynamics, live FV water it can flip in, visible deforming surface, D6 closed. Moving to P3 (Troublemaker vertical slice).
