...
- Flexible-raft reference physics D1–D5 (tube compliance, seat load, overwash flip, rock wrap/pin, telemetry), deterministic and tested. D6 now has seven genuine Unreal Chaos baseline records, seven independent Project Chrono compliant records, and a passing 14-record numeric comparison; named manual promotion review remains open.
...
| Cut from 1.0 | VR (→1.1; remove `bEnableHMD=True` from shipped config) · multiplayer · voice recognition · non-South-Fork rivers at production fidelity (runnable reference Free Runs shipped as-is per `physics/data/real_world/river_portfolio_plan.json`; production promotion stays gated) · Chrono runtime · Jolt · consoles/handheld · localization (FText hygiene only) · Wwise/FMOD · paid asset libraries · telemetry (local logs only) · player-facing editor tools. *(native Linux moved into scope 2026-08-06 — see Platforms.)* |
| Platforms | Windows x64 primary; macOS Apple Silicon secondary; **Linux x64 native (Vulkan) shipping tier** — re-baselined 2026-08-06: editor, Development and Shipping game targets build and pass the in-engine gates on a UE 5.8.1 source build, `unreal/Scripts/package_linux.sh` packages it, and the physics suite runs green-minus-recorded-baseline; the Proton verification report is retained as additional compatibility coverage on top of the native tier. |
...
| Perf targets | User revision September 12, 2026: 1080p30 High on RTX 3060 / M2 Pro; 1440p30 Epic on RTX 4070. Frame p95 ≤ 33.333 ms; no streaming hitch longer than two target frames (66.667 ms) full-reach. Game-thread ≤ 8 ms and solver ≤ 1.6 ms/tick sub-budgets unchanged. Historical 60 FPS reports below are retained, not current target authority. |
...
- **A-4 Crew** = locally generated UE 5.8 MetaHuman production roster with optimized
  assembled Blueprints, deterministic rafting pose leadership, and project-owned PPE/paddles.
  MetaHuman source/optimized `.uasset` binaries remain local-only under Epic's source-format
  redistribution terms; committed CC0 characters remain the complete fail-closed development
  fallback. Existing crew math (`RaftSimCrewStateContracts.cpp`) remains authoritative.
...
| D6 "14 missing measured pairs" | **Resolved July 28:** seven genuine Chaos records plus seven fixture-input-only Project Chrono 10.0.0 records are merged; the comparison passes. The A-3 UE port remains translation evidence only. Named manual promotion review stays in the P7 owner packet. |
...
| E1 platform matrix | Decided in §4; re-baselined 2026-08-06 (Win + macOS + native Linux; Proton report retained as extra coverage). |
...
**P6 — Platforms & packaging.** Cut `release/1.0`; Windows + macOS + native Linux packages via the P1 scripts (`package_win.ps1`, `package_mac.sh`, `package_linux.sh`; Linux solver lib via `build_solver_lib.sh` against the engine's bundled clang/libc++); determinism spot-check (same seed → same replay hashes) on all three platforms; macOS Metal verification (VSM-vs-cascade decision test: full-reach descent, no shadow artifacts, ≥55 fps High/1440p on M2 Pro; the known alpha-as-mask foliage checks); Linux Vulkan verification (in-engine gate pass + packaged full-descent soak on the Linux box); Proton verification report retained as extra coverage; QA sweep (scripted full-descent soaks all flows/platforms via the harness in packaged builds, input-device matrix, save migration, fresh-machine first run); release artifacts (GitHub Release draft with checksums, itch kit, Steam kit, trailer cut from in-engine captures, `docs/presskit/`).
*Exit:* all three platform builds pass QA from a fresh machine; artifacts staged; `v1.0.0-rc1` tagged. *(The release-candidate workflow's native-Linux RC lane still needs wiring in `.github/workflows/release-candidate.yml` and `Scripts/release_candidate.py` — currently macOS/Windows build lanes plus Linux Proton QA only.)*
...
**Locked-source commit gate (added 2026-08-06).** Any commit that touches a hash-locked source (a file whose SHA-256 is recorded in a review/evidence JSON) must refresh those locks in the same commit and pass the full physics suite with no failures beyond `Scripts/full_suite_baseline.json`. Enforced by `Scripts/check_locked_source_gate.py`: `--staged` as a pre-commit hook (install with `Scripts/install_git_hooks.sh`), `--all` in CI repo guards. Pre-existing drift is recorded explicitly in `Scripts/locked_source_debt.json` (298 entries across 49 reviews as of 2026-08-06) and both debt and baseline may only shrink.
...
### 2026-07-18 — P2 slice two: live solver water under the raft
- `URaftSimWaterRuntimeAdapter` now owns a `FRaftSimLiveWaterWindow`; `StepWater` ticks the genuine order-2 FV solver and `SampleWaterAtWorldPosition` bilinearly samples its fields (the constant-depth placeholder is gone). The raft actor stands up a 160×160 m flat-tank window via the bridge subsystem (report gate disabled for dev tank water only; river-water approval claims still gated). `RaftSim.P1.TestTankRaftFloatsAndPaddles` passes on live solver water with no crashes. Fixes en route: WaterState eta/wet initialization (SIGSEGV), UHT dtor placement, complete-type publicization of the window header.

### 2026-07-18 — P2 slice three: A-3 flexible-raft D1–D4 in C++; UE-port diagnostics added
- **A-3 port landed**: `RaftSimFlexibleRaftModel.{h,cpp}` (RaftSimPhysics) ports D1 compliant tube, D2 seat-load coupling, D3 overwash/flip, and D4 rock contact/wrap/pin/release from the Python reference with identical inputs/outputs and clamp constants (double precision, deterministic, first-wins nearest-segment ties). `URaftSimChronoRuntimeAdapter::StepRaftDynamics` (CustomReducedRigidBody) now evaluates the stack quasi-statically per fixed substep and applies retained-water and contact force/moment modifiers to the kinematic state, with persistent retained-volume/indentation memory and per-step telemetry; with no seats/water/obstacles configured it reduces to the previous behavior (P1 tank test untouched, still green). Actor swap onto the adapter is the next slice.
- **Correction recorded 2026-07-28 — these were port diagnostics, not D6 external measurements**: `RaftSim.D6.UEMeasuredExport` runs all seven fixtures twice through the same custom C++ D1–D4 implementation (`Compliant` and its analytical `RigidBaseline` mode). Its tight Python parity is useful translation evidence, but neither pass is Project Chrono/a reviewed independent compliant model and the rigid pass is not a simulated Unreal Chaos body. The outputs under `physics/reports/d6/ue/` therefore do not satisfy either independent D6 target and never authorize promotion.
- **Independent D6 target completed 2026-07-28**: `flexible_raft_d6_project_chrono_runner.py` consumes only the frozen fixture-input JSON, creates real Project Chrono 10.0.0 `ChSystemSMC`/`ChLinkTSDA` systems for tube and rock-contact compliance, runs all seven fixtures twice byte-identically, and writes seven hashed telemetry payloads. With the genuine Chaos sidecar, the merged 14-record report has zero missing targets, zero failed targets, and 74/74 compliant numeric metrics within tolerance. D6 stays unpromoted for named physics/integration/replay/guide-safety review.

### 2026-07-18 — P2 slice four: actor on adapter dynamics; cooked-field river windows
- **A-3 actor swap complete**: `ARaftSimRaftActor` no longer integrates anything itself — `BeginPlay` configures the bridge subsystem from the actor's properties (mass 220 kg, tube radius 0.28 m, 4.3×2.0 m footprint, buoyancy multiple, drag, heave/angular damping, inertia from the footprint), paddle/turn strokes route through `URaftSimChronoRuntimeAdapter::AddExternalImpulse`, and `Tick` just calls `TickBridge` and mirrors the adapter's kinematic state onto the actor transform. The adapter's `CustomReducedRigidBody` step gained the buoyancy support stage (gravity, six-point tube buoyancy sampled through a bridge-bound water probe, quadratic drag, heave damping, external impulses, angular damping) evaluated together with the D1–D4 modifiers in one semi-implicit 120 Hz substep; without a bound water probe the D1–D4-only behavior is unchanged. `RaftSim.P1.TestTankRaftFloatsAndPaddles` stays green through the bridge path.
- **A-1 river windows landed**: `FRaftSimLiveWaterWindow::CreateFromCookedFields` loads a `raftsim.cooked_flow_fields.v1` band (minimal in-module `.npy` reader for the float32/uint8 contract dtypes; per-file SHA-256 verified against the manifest with an in-module FIPS 180-4 implementation since UE lacks one on Mac), crops the requested world-space window, seeds bed/h/u/v/hu/hv/eta/wet, and runs the genuine order-2 HLL solver with the manifest's cook settings (roughness_scale 0.5, bed_slope_source_scale 0.75, CFL 0.45). Cut edges are transmissive; cooked bank edges stay banks. Manifest v1 does not record the seed scenario's Manning n, so `ConfigureRiverWindow` takes it as a parameter (South Fork median 0.041) — next cook should record it. **`RaftSim.P2.RiverWindowLoads` passes**: median_runnable full-axis window, 120 steps at 1/60 s, mass positive, no NaNs, wet fraction within 10% of the cooked seed. Moving-window recenter + scenario inflow/outflow forcing are the next water slice.

### 2026-07-18 — P2 flip/swim/recover loop (§5 A-5)
- `ARaftSimRaftActor` gained a capsize state machine (Upright/Capsized/Recovering): it latches on the adapter's D3 flip telemetry (negative flip margin) or a hard roll-over, ejects the crew as `FRaftSimSwimmerRescueFrame` swimmers that drift on the live water field (clamped to a physical max), and reseats them via the crew library's throw-line/reach-grab rescue math on guide re-flip. High-side input is now context-sensitive (re-right when capsized, else feed a D2 crew high-side action). `RaftSim.P2.RaftFlipsAndRecovers` automation test passes: forced upstream overwash → capsize → 5 swimmers → re-flip → all reseated → upright; P1 tank test still green. Re-flip rights the boat at the capsize location so the crew stay reachable through a runaway-sink guard.

### 2026-07-18 — P2 COMPLETE: water surface rendering v1
- `ARaftSimWaterSurfaceActor` (RaftSimRaft, ProceduralMeshComponent) renders the live solver free surface: a grid mesh whose vertices track the water adapter's surface height + normal each refresh, with Froude-driven foam in vertex colour, using the single-layer water material. Spawned automatically alongside the raft. `RaftSim.P2.WaterSurfaceRenders` passes; all four P2 automation tests green (surface, flip/recover, river window, tank float). **Phase 2 gameplay exit gate met**: raft on authoritative dynamics, live FV water it can flip in, and a visible deforming surface. The independent D6 release-calibration gate remained open.

### 2026-07-18 — P3 scoring core: run manager, encounter volumes, HUD (§A-6)
- Gameplay run layer in the game module: `ARaftSimEncounterVolume` (typed scout/hazard/line/finish triggers), `ARaftSimRunManager` (starts on leaving the eddy, accumulates clean-line/swim/boat-angle signals from the raft + hazard overlaps, finishes at the line, scores via the crew scoring library, saves best), `URaftSimRunHudWidget` (state/timer/incidents/swimmers/score + subtitles), `ARaftSimGuidePlayerController` (creates the HUD, game input), wired through the vertical-slice game mode. Added a raft position stability clamp (a raft cannot leave the world; guards rendering/gameplay against solver divergence). `RaftSim.P3.RunScoresAndSaves` passes: a run finishes, scores, and persists to the save game; P2 flip/surface tests still green.

### 2026-07-18 — P3 functional crew
- The raft now carries an AI crew: `IssueCrewCommand` (AllForward/Backward/TurnLeft/TurnRight/Stop/GetDown/HighSide) with a reaction latency and stroke cadence, applying combined paddle impulses through the adapter and coupling high-side/get-down to the D2 flexible crew actions (weight shift). Guide command inputs route through the pawn to the crew; visual paddlers sit on the tubes. `RaftSim.P3.CrewRespondsToCommands` passes (AllForward propels the raft); flip/run tests still green.

### 2026-07-18 — P3 capstone: Troublemaker map with live river water
- `ARaftSimRiverWaterConfig` (RaftSimWater) lets a map load a cooked steady-state flow window; the raft resolves it at BeginPlay and calls `ConfigureRiverWindow` (else the dev tank). `RaftSim.CreateTroublemakerMap` editor command + `bootstrap_troublemaker.py` generate `L_Troublemaker` (river config → median Troublemaker cooked fields, raft at the scout eddy, player start, vertical-slice game mode). Main menu gains "Run Troublemaker (South Fork)" and relabels the tank as "Training Eddy". `RaftSim.P3.TroublemakerRiverLoads` passes: the map loads a live wet finite river window (not the flat tank) and the raft rests on it. The Troublemaker cooked fields cook steady in bulk with an honestly-recorded unsteady pocket at the gut (the live window keeps evolving in-engine).

### 2026-07-18 — P3 audio pass 1
- `ARaftSimRunAudioDirector` (spawned by the game mode) drives a looping procedural water ambience whose loudness follows the live river roar built from the raft's flow speed + foam through the RaftSimAudio cue math (`BuildWaterAudioParameters`), with an impact layer on flips and pitch lift on rapid-feature intensity. No external assets. `RaftSim.P3.RunAudioReactsToFlow` passes: the ambience mix rises as the crew paddles the raft up to speed. Full run now has reactive sound. (Real sourced/authored sound assets remain a later polish pass.)
