...
- `geoclaw_cpp_comparison_summary.json`: 34 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- Preserve the reduced South Fork median-flow closure as a guardrail while the finite-volume median-flow row, high-flow real-world rows, reduced cascading rows, and raft-coupling failures remain visible.
