...
Milestone 18 source reports are now ahead of that packaged readiness snapshot: `physics/reports/milestone16/geoclaw_cpp_comparisons.json` has 5 of 40 GeoClaw/C++ threshold comparisons passing, `physics/reports/milestone16/raft_coupling_validation.json` has 11 of 50 raft-coupling comparisons passing, and `physics/reports/milestone16/regression_promotion_manifest.json` has 16 promoted artifacts. The refreshed `physics/reports/milestone16/full_cpp_validation_gate.json` remains blocked. Keep the packaged GeoClaw-to-Unreal readiness files unchanged until the explicit readiness-regeneration step.
...
- Finish the bed-step augmented topography/source-distribution fix; the current finite-volume checkpoint still fails `field_linf` and `slope_linf` even though probe, cross-section, mass, energy, Froude, and feature checks pass.
