...
- `geoclaw_cpp_comparison_summary.json`: 10 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- `runtime_profile_summary.json`: 20 of 20 promoted C++ profile repetitions pass local desktop, VR, and handheld water-solver budgets, and 10 of 10 deterministic replay groups match.
- `regression_promotion_summary.json`: 33 passing artifacts were promoted as regression fixtures or manifests in the latest packaged readiness snapshot.
- `geoclaw_to_unreal_readiness_report.json` and `.md`: final Milestone 16 decision; live custom water is blocked.

Milestone 18 source reports are now consumed by the packaged readiness snapshot: `physics/reports/milestone18/remaining_geometry_closure.json` passes with six of six geometry families promotion-ready, `physics/reports/milestone16/geoclaw_cpp_comparisons.json` records 10 of 40 threshold comparisons passing, and `physics/reports/milestone16/raft_coupling_validation.json` has 15 of 50 raft-coupling comparisons passing. The refreshed `physics/reports/milestone16/full_cpp_validation_gate.json` and packaged GeoClaw-to-Unreal readiness report remain blocked by aggregate GeoClaw/C++ comparison and raft-coupling failures, not by the geometry-family gate.
