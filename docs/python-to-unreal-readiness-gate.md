...
- `geoclaw_cpp_comparison_summary.json`: 24 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- Preserve the generic profile catalog path and reduced hydraulic-hole/downstream-boil closure as guardrails while the finite-volume hole/boil row and broader rapid-feature rows remain visible.
