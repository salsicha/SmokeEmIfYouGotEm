...
- `geoclaw_cpp_comparison_summary.json`: 32 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- Preserve the reduced South Fork low-flow closure as a guardrail while the finite-volume low-flow row, median/high real-world rows, reduced cascading rows, and raft-coupling failures remain visible.
