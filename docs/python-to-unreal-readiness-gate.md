...
Milestone 18 source reports are now ahead of that packaged readiness snapshot: `physics/reports/milestone16/geoclaw_cpp_comparisons.json` has 6 of 40 GeoClaw/C++ threshold comparisons passing, `physics/reports/milestone16/raft_coupling_validation.json` has 11 of 50 raft-coupling comparisons passing, and `physics/reports/milestone16/regression_promotion_manifest.json` has 20 promoted artifacts, including 3 stitched reach/drop geometry artifacts. The refreshed `physics/reports/milestone16/full_cpp_validation_gate.json` remains blocked. Keep the packaged GeoClaw-to-Unreal readiness files unchanged until the explicit readiness-regeneration step.
...
- Keep constriction parity blocked until the corrected west `user`-boundary reference passes geometry-aware throat reconstruction; scalar source scaling can make mass and energy pass, but current finite-volume Roe still fails field-shape, probe, cross-section, Froude, and feature-strength checks.
- Keep drop/ledge blocked until the corrected west `user`-boundary reference passes water-shape reconstruction around the hydraulic control and downstream recovery; current finite-volume HLL/Roe lanes pass conservation, wet-mask, slope, cross-section, Froude, and feature checks with feature forcing off, but still fail field/probe thresholds.
