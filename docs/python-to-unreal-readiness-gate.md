...
- `geoclaw_cpp_comparison_summary.json`: 39 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- Preserve the reduced South Fork cascading low-flow stitched whole-window closure as a guardrail while the high reduced cascading row and raft-coupling failures remain visible.
- Preserve the reduced South Fork cascading median-flow stitched whole-window closure as a guardrail while the high reduced cascading row and raft-coupling failures remain visible.
