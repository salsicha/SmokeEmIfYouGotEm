...
- `geoclaw_cpp_comparison_summary.json`: 36 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- Preserve the reduced South Fork high-flow closure as a guardrail while the finite-volume high-flow row, reduced cascading rows, and raft-coupling failures remain visible.
