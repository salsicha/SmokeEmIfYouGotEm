...
- `geoclaw_cpp_comparison_summary.json`: 38 of 40 GeoClaw/C++ threshold comparisons pass, so the comparison gate blocks.
...
- Preserve the reduced South Fork cascading low-flow stitched whole-window closure as a guardrail while the median/high reduced cascading rows and raft-coupling failures remain visible.
